...
/* workaround for some ANR CRs. if suppliant is blocked longer than 10s, wifi
 * hal will tell wifiMonitor to teminate. for the case which can block
 * supplicant 10s is to del key more than 5 times. the root cause is that there
 * is no resource in TC4, so del key command was not able to set, and then oid
 * timeout was happed. if we found the root cause why fw couldn't release TC
 * resouce, we will remove this workaround
 */
...
typedef u64 wifi_timestamp; /* In microseconds (us) */
typedef u64 wifi_timespan; /* In nanoseconds  (ns) */

typedef u8 mac_addr[6];
typedef u32 wifi_channel; /* indicates channel frequency in MHz */
typedef s32 wifi_rssi;
...
typedef struct _PARAM_WIFI_GSCAN_CAPABILITIES_STRUCT_T {
	u32 max_scan_cache_size; /*total space allocated for scan (in bytes) */
	u32 max_scan_buckets; /*maximum number of channel buckets */
	u32 max_ap_cache_per_scan; /*maximum number of APs that can be stored
	                            * per scan */
	u32 max_rssi_sample_size; /*number of RSSI samples used for averaging
	                           * RSSI */
	u32 max_scan_reporting_threshold; /*max possible report_threshold as
	                                   * described in wifi_scan_cmd_params
	                                   */
	u32 max_hotlist_aps; /*maximum number of entries for hotlist APs */
	u32 max_significant_wifi_change_aps; /*maximum number of entries for
	                                      * significant wifi change APs */
	u32 max_bssid_history_entries; /*number of BSSID/RSSI entries that
	                                * device can hold */
} PARAM_WIFI_GSCAN_CAPABILITIES_STRUCT_T,
*P_PARAM_WIFI_GSCAN_CAPABILITIES_STRUCT_T;
...
typedef struct _PARAM_WIFI_GSCAN_BUCKET_SPEC {
	u32 bucket; /* bucket index, 0 based */
	WIFI_BAND band; /*when UNSPECIFIED, use channel lis */
	u32 period; /* desired period, in millisecond; if this is too  low, the
	             * firmware should choose to generate results as fast as it
	             * can instead of failing the command
	             */
	/* report_events semantics -
	 *  0 => report only when scan history is % full
	 *  1 => same as 0 + report a scan completion event after scanning this
	 * bucket 2 => same as 1 + forward scan results (beacons/probe responses
	 * + IEs) in real time to HAL 3 => same as 2 + forward scan results
	 * (beacons/probe responses + IEs) in real time to supplicant as well
	 * (optional) .
	 */
	u8 report_events;
...
typedef struct _PARAM_WIFI_GSCAN_CMD_PARAMS {
	u32 base_period; /* base timer period in ms */
	u32 max_ap_per_scan;
	/* number of APs to store in each scan in the
	 * BSSID/RSSI history buffer (keep the highest RSSI APs)
	 */
	u32 report_threshold; /* in %, when scan buffer is this much full, wake
	                       * up AP */
	u32 num_scans;
	u32 num_buckets;
...
typedef struct _PARAM_WIFI_GSCAN_RESULT {
	wifi_timestamp ts; /* time since boot (in microsecond) when the result
	                    * was */
	/* retrieved */
	u8 ssid[32 + 1]; /* null terminated */
	mac_addr bssid;
	...
	u8 ie_data[1]; /* blob of all the information elements found in the */
...
typedef struct _PARAM_WIFI_BSSID_HOTLIST {
	u32 lost_ap_sample_size;
	u32 num_ap; /* number of hotlist APs */
...
typedef struct _PARAM_WIFI_SIGNIFICANT_CHANGE {
	u16 rssi_sample_size; /* number of samples for averaging RSSI */
	u16 lost_ap_sample_size; /* number of samples to confirm AP loss */
	u16 min_breaching; /* number of APs breaching threshold */
	u16 num_ap; /* max 64 */
...
int mtk_cfg80211_NLA_PUT(struct sk_buff *skb,
			 int attrtype,
			 int attrlen,
			 const void *data);

int mtk_cfg80211_nla_put_type(struct sk_buff *skb,
			      ENUM_NLA_PUT_DATE_TYPE type,
			      int attrtype,
			      const void *value);

int mtk_cfg80211_vendor_get_capabilities(struct wiphy *wiphy,
...
int mtk_cfg80211_vendor_set_significant_change(struct wiphy *wiphy,
					       struct wireless_dev *wdev,
					       const void *data,
					       int data_len);
...
int mtk_cfg80211_vendor_enable_full_scan_results(struct wiphy *wiphy,
						 struct wireless_dev *wdev,
						 const void *data,
						 int data_len);

int mtk_cfg80211_vendor_get_scan_results(struct wiphy *wiphy,
					 struct wireless_dev *wdev,
					 const void *data,
					 int data_len);
...
int mtk_cfg80211_vendor_event_scan_results_available(struct wiphy *wiphy,
						     struct wireless_dev *wdev,
						     u32 num);

int mtk_cfg80211_vendor_event_full_scan_results(struct wiphy *wiphy,
						struct wireless_dev *wdev,
						P_PARAM_WIFI_GSCAN_RESULT pdata,
						u32 data_len);

int mtk_cfg80211_vendor_event_significant_change_results(struct wiphy *wiphy,
							 struct wireless_dev *
							 wdev,
							 P_PARAM_WIFI_CHANGE_RESULT
							 pdata,
							 u32 data_len);

int mtk_cfg80211_vendor_event_hotlist_ap_found(struct wiphy *wiphy,
					       struct wireless_dev *wdev,
					       P_PARAM_WIFI_GSCAN_RESULT pdata,
					       u32 data_len);

int mtk_cfg80211_vendor_event_hotlist_ap_lost(struct wiphy *wiphy,
					      struct wireless_dev *wdev,
					      P_PARAM_WIFI_GSCAN_RESULT pdata,
					      u32 data_len);
