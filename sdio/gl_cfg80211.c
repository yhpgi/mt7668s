...
int mtk_cfg80211_change_iface(
		struct wiphy *wiphy, struct net_device *ndev, enum nl80211_iftype type, struct vif_params *params)
#else
int mtk_cfg80211_change_iface(
		struct wiphy *wiphy, struct net_device *ndev, enum nl80211_iftype type, u32 *flags, struct vif_params *params)
#endif
{
		...
		struct key_params *params)
{
	...
	if (pairwise) {
		...
		RX_RESET_CNT(&prGlueInfo->prAdapter->rRxCtrl, RX_BMC_NO_KEY_COUNT);
		RX_RESET_CNT(&prGlueInfo->prAdapter->rRxCtrl, RX_BMC_KEY_ERROR_COUNT);
		...
		struct wiphy *wiphy, struct net_device *ndev, u8 key_index, bool unicast, bool multicast)
{
	...
	rStatus = kalIoctl(
			prGlueInfo, wlanoidSetDefaultKey, &rDefaultKey, sizeof(PARAM_DEFAULT_KEY_T), FALSE, FALSE, TRUE, &u4BufLen);
	if (rStatus == WLAN_STATUS_SUCCESS)
		i4Rst = 0;
...
int mtk_cfg80211_auth(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_auth_request *req)
{
	...
	if (req->key_len != 0) {
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidSetAddWep, prWepKey, prWepKey->u4Length, FALSE, FALSE, TRUE, &u4BufLen);
	...
	DBGLOG(REQ, WARN, "req IE len %d, ssid %16llx, ssid_len %d\n", req->bss->ies->len,
			ssid_hash_64(SSID_IE(req->bss->ies->data)->aucSSID), SSID_IE(req->bss->ies->data)->ucLength);
#else
	DBGLOG(REQ, WARN, "req IE len %d, ssid %.*s, ssid_len %d\n", req->bss->ies->len,
			SSID_IE(req->bss->ies->data)->ucLength, SSID_IE(req->bss->ies->data)->aucSSID,
			SSID_IE(req->bss->ies->data)->ucLength);
#endif
	if (req->bss->ies->len != 0 && IE_ID(req->bss->ies->data) == ELEM_ID_SSID) {
	...
	if (!prConnSettings->fgIsConnInitialized || fgNewAuthParam) {
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidSetConnect, (void *)&rNewSsid, sizeof(PARAM_CONNECT_T), FALSE, FALSE,
				TRUE, &u4BufLen);
		...
		prStaRec = cnmGetStaRecByAddress(
				prGlueInfo->prAdapter, prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex, rNewSsid.pucBssid);
		if (prStaRec) {
...
int mtk_cfg80211_connect(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_connect_params *sme)
{
	...
	if (sme->ie && sme->ie_len > 0) {
...
#if CFG_SUPPORT_WAPI
		rStatus =
				kalIoctl(prGlueInfo, wlanoidSetWapiAssocInfo, pucIEStart, sme->ie_len, FALSE, FALSE, FALSE, &u4BufLen);
		...
		if (wextSrchDesiredWPSIE(pucIEStart, sme->ie_len, 0xDD, (PUINT_8 *)&prDesiredIE)) {
			...
			rStatus = kalIoctl(prGlueInfo, wlanoidSetWSCAssocInfo, prDesiredIE, IE_SIZE(prDesiredIE), FALSE, FALSE,
					FALSE, &u4BufLen);
			if (rStatus != WLAN_STATUS_SUCCESS)
		...
		if (wextSrchDesiredHS20IE(pucIEStart, sme->ie_len, (PUINT_8 *)&prDesiredIE)) {
			rStatus = kalIoctl(
					prGlueInfo, wlanoidSetHS20Info, prDesiredIE, IE_SIZE(prDesiredIE), FALSE, FALSE, TRUE, &u4BufLen);
#if 0
			if (rStatus != WLAN_STATUS_SUCCESS)
		...
		if (wextSrchDesiredInterworkingIE(pucIEStart, sme->ie_len, (PUINT_8 *)&prDesiredIE)) {
			rStatus = kalIoctl(prGlueInfo, wlanoidSetInterworkingInfo, prDesiredIE, IE_SIZE(prDesiredIE), FALSE, FALSE,
					TRUE, &u4BufLen);
#if 0
			if (rStatus != WLAN_STATUS_SUCCESS)
		...
		if (wextSrchDesiredRoamingConsortiumIE(pucIEStart, sme->ie_len, (PUINT_8 *)&prDesiredIE)) {
			rStatus = kalIoctl(prGlueInfo, wlanoidSetRoamingConsortiumIEInfo, prDesiredIE, IE_SIZE(prDesiredIE), FALSE,
					FALSE, TRUE, &u4BufLen);
#if 0
			if (rStatus != WLAN_STATUS_SUCCESS)
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetAuthMode, &eAuthMode, sizeof(eAuthMode), FALSE, FALSE, FALSE, &u4BufLen);
	if (rStatus != WLAN_STATUS_SUCCESS)
	...
	rStatus = kalIoctl(
			prGlueInfo, wlanoidSetEncryptionStatus, &eEncStatus, sizeof(eEncStatus), FALSE, FALSE, FALSE, &u4BufLen);
	if (rStatus != WLAN_STATUS_SUCCESS)
	...
	rNewSsid.u4SsidLen	  = sme->ssid_len;
	rStatus				  = kalIoctl(
						  prGlueInfo, wlanoidSetConnect, (PVOID)&rNewSsid, sizeof(PARAM_CONNECT_T), FALSE, FALSE, TRUE, &u4BufLen);
...
int mtk_cfg80211_join_ibss(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_ibss_params *params)
{
	...
	if (params->channel_fixed) {
		...
		rStatus = kalIoctl(
				prGlueInfo, wlanoidSetFrequency, &u4ChnlFreq, sizeof(u4ChnlFreq), FALSE, FALSE, FALSE, &u4BufLen);
		if (rStatus != WLAN_STATUS_SUCCESS)
			return -EFAULT;
	}
	...
	rStatus =
			kalIoctl(prGlueInfo, wlanoidSetSsid, (PVOID)&rNewSsid, sizeof(PARAM_SSID_T), FALSE, FALSE, TRUE, &u4BufLen);
...
int mtk_cfg80211_set_power_mgmt(struct wiphy *wiphy, struct net_device *ndev, bool enabled, int timeout)
{
	...
	if ((prBssInfo->eBand == BAND_2G4) && (prGlueInfo->prAdapter->rWifiVar.ucEnforce2G < Param_PowerModeMax)) {
		eEnforcePowerMode = (PARAM_POWER_MODE)prGlueInfo->prAdapter->rWifiVar.ucEnforce2G;
	} else if ((prBssInfo->eBand == BAND_5G) && (prGlueInfo->prAdapter->rWifiVar.ucEnforce5G < Param_PowerModeMax)) {
...
int mtk_cfg80211_set_power_mgmt(struct wiphy *wiphy, struct net_device *ndev, bool enabled, int timeout)
{
	...
	if ((prGlueInfo->prAdapter->prAisBssInfo->ePowerModeFromUser != rPowerMode.ePowerMode) &&
		...
		prGlueInfo->prAdapter->prAisBssInfo->ePowerModeFromUser = rPowerMode.ePowerMode;
	}
	...
	if (prParams) {
		...
		case HS20_CMD_ID_SET_BSSID_POOL:
			...
			rstatus = kalIoctl(prGlueInfo, (PFN_OID_HANDLER_FUNC)wlanoidSetHS20BssidPool,
					&prParams->hs20_set_bssid_pool, sizeof(struct param_hs20_set_bssid_pool), FALSE, FALSE, TRUE,
					&u4SetInfoLen);
			break;
		default:
...
int mtk_cfg80211_testmode_set_key_ext(IN struct wiphy *wiphy, IN void *data, IN int len)
{
	...
	if (prIWEncExt->alg == IW_ENCODE_ALG_SMS4) {
		...
		if (prIWEncExt->ext_flags & IW_ENCODE_EXT_SET_TX_KEY && !(prIWEncExt->ext_flags & IW_ENCODE_EXT_GROUP_KEY)) {
		...
		rstatus = kalIoctl(prGlueInfo, wlanoidSetAddKey, prWpiKey, sizeof(PARAM_KEY_T), FALSE, FALSE, TRUE, &u4BufLen);
...
int mtk_cfg80211_testmode_get_sta_statistics(
		IN struct wiphy *wiphy, IN void *data, IN int len, IN P_GLUE_INFO_T prGlueInfo)
{
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	UINT_32		u4BufLen;
	UINT_32		u4LinkScore;
	...
	/* Sta queue length */
	if (unlikely(nla_put(skb, NL80211_TESTMODE_STA_STATISTICS_TC_QUE_LEN_ARRAY, sizeof(rQueryStaStatistics.au4TcQueLen),
	...
	/* Reserved field */
	if (unlikely(nla_put(skb, NL80211_TESTMODE_STA_STATISTICS_RESERVED_ARRAY, sizeof(rQueryStaStatistics.au4Reserved),
...
int mtk_cfg80211_testmode_sw_cmd(IN struct wiphy *wiphy, IN void *data, IN int len)
{
	...
	if (prParams) {
		if (prParams->set == 1) {
			rstatus = kalIoctl(prGlueInfo, (PFN_OID_HANDLER_FUNC)wlanoidSetSwCtrlWrite, &prParams->adr, (UINT_32)8,
...
static int mtk_wlan_cfg_testmode_cmd(struct wiphy *wiphy, void *data, int len)
{
	...
	case TESTMODE_CMD_ID_STR_CMD:
		i4Status = mtk_cfg80211_process_str_cmd(prGlueInfo, (PUINT_8)(prParams + 1), len - sizeof(*prParams));
		break;
	default:
		i4Status = -EINVAL;
		break;
...
int mtk_cfg80211_sched_scan_start(
		IN struct wiphy *wiphy, IN struct net_device *ndev, IN struct cfg80211_sched_scan_request *request)
{
	P_GLUE_INFO_T			   prGlueInfo = NULL;
	WLAN_STATUS				   rStatus;
	UINT_32					   i, u4BufLen;
...
int mtk_cfg80211_assoc(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_assoc_request *req)
{
	...
	if (!prConnSettings->fgIsConnInitialized) {
		...
		rStatus = kalIoctl(
				prGlueInfo, wlanoidSetBssid, (void *)req->bss->bssid, MAC_ADDR_LEN, FALSE, FALSE, TRUE, &u4BufLen);
		...
		prStaRec = cnmGetStaRecByAddress(
				prGlueInfo->prAdapter, prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex, req->bss->bssid);
...
#else
	rStatus =
			kalIoctl(prGlueInfo, wlanoidSetBssid, (void *)req->bss->bssid, MAC_ADDR_LEN, FALSE, FALSE, TRUE, &u4BufLen);
		...
		struct wiphy *wiphy, struct net_device *ndev, const u8 *mac, struct station_parameters *params)
{
	...
	if (params->ht_capa != NULL) {
		...
		kalMemCopy(
				rCmdUpdate.rHtCap.rMCS.arRxMask, params->ht_capa->mcs.rx_mask, sizeof(rCmdUpdate.rHtCap.rMCS.arRxMask));
	...
	if (params->vht_capa != NULL) {
		...
		rCmdUpdate.rVHtCap.rVMCS.u2RxMcsMap	 = params->vht_capa->supp_mcs.rx_mcs_map;
		rCmdUpdate.rVHtCap.rVMCS.u2RxHighest = params->vht_capa->supp_mcs.rx_highest;
		rCmdUpdate.rVHtCap.rVMCS.u2TxMcsMap	 = params->vht_capa->supp_mcs.tx_mcs_map;
		rCmdUpdate.rVHtCap.rVMCS.u2TxHighest = params->vht_capa->supp_mcs.tx_highest;
	...
	if (prAisBssInfo->fgTdlsIsChSwProhibited) {
		...
		rStatus = kalIoctl(prGlueInfo, TdlsSendChSwControlCmd, &TdlsSendChSwControlCmd, sizeof(CMD_TDLS_CH_SW_T), FALSE,
				FALSE, FALSE,
				/* FALSE,    //6628 -> 6630  fgIsP2pOid-> x */
				&u4BufLen);
	}
...
INT_32 mtk_cfg80211_process_str_cmd(P_GLUE_INFO_T prGlueInfo, PUINT_8 cmd, INT_32 len)
{
	...
	if (strnicmp(cmd, "NEIGHBOR-REQUEST", 16) == 0) {
		...
		rStatus = kalIoctl(
				prGlueInfo, wlanoidSendNeighborRequest, (void *)pucSSID, u4SSIDLen, FALSE, FALSE, TRUE, &u4SetInfoLen);
	} else
		return -EOPNOTSUPP;
#endif
	...
	if (strnicmp(cmd, "BSS-TRANSITION-QUERY", 20) == 0) {
		...
		if (len > 20 && (strnicmp(cmd + 20, " reason=", 8) == 0))
			pucReason = cmd + 28;
		rStatus = kalIoctl(prGlueInfo, wlanoidSendBTMQuery, (void *)pucReason, 1, FALSE, FALSE, TRUE, &u4SetInfoLen);
	} else
		return -EOPNOTSUPP;
#endif
...
void mtk_cfg80211_set_wakeup(struct wiphy *wiphy, bool enabled)
{
	...
	if (prGlueInfo && prGlueInfo->prAdapter) {
		...
		DBGLOG(REQ, WARN, "not applied (%p, %p)\n", prGlueInfo, prGlueInfo ? prGlueInfo->prAdapter : NULL);
	}
}
