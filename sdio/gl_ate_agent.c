...
typedef struct _ATE_PRIV_CMD {
	u8 *name;
	int (*set_proc)(struct net_device *prNetDev, u8 *prInBuf);
...
int Set_ResetStatCounter_Proc(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO,
	       "MT6632 : ATE_AGENT iwpriv Set_ResetStatCounter_Proc\n");
...
int SetATEDa(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	/* xx:xx:xx:xx:xx:xx */
	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x", &addr[0], &addr[1], &addr[2],
		    &addr[3], &addr[4], &addr[5]);
	if (rv == 6) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 : ATE_AGENT iwpriv SetATEDa Sa:%02x:%02x:%02x:%02x:%02x:%02x\n",
		       addr[0],
		       addr[1],
		       addr[2],
		       addr[3],
		...
		i4Status = MT_ATESetMACAddress(
			prNetDev, RF_AT_FUNCID_SET_MAC_ADDRESS, addr2);
	} else {
		return -EINVAL;
	}
	return i4Status;
...
int SetATESa(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 6) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 : ATE_AGENT iwpriv SetATESa Sa:%02x:%02x:%02x:%02x:%02x:%02x\n",
		       addr[0],
		       addr[1],
		       addr[2],
		       addr[3],
		...
		i4Status = MT_ATESetMACAddress(prNetDev, RF_AT_FUNCID_SET_TA,
					       addr2);
	} else {
		return -EINVAL;
	}
...
int SetATETxCount(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	s32 i4Status;
	s32 rv;
	u8 addr[MAC_ADDR_LEN] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
	...
	i4Status = MT_ATESetMACAddress(prNetDev, RF_AT_FUNCID_SET_MAC_ADDRESS,
				       addr);
...
int Set_TxBfProfileTag_InValid(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTag_InValid prInBuf = %s, ucInValid = %d\n",
		       prInBuf,
		       ucInValid);
...
int Set_TxBfProfileTag_PfmuIdx(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTag_PfmuIdx prInBuf = %s, ucProfileIdx = %d\n",
		       prInBuf,
		       ucProfileIdx);
...
int Set_TxBfProfileTag_BfType(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTag_BfType prInBuf = %s, ucBFType = %d\n",
		       prInBuf,
		       ucBFType);
...
int Set_TxBfProfileTag_DBW(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTag_DBW prInBuf = %s, ucBW = %d\n",
		       prInBuf,
		       ucBW);
...
int Set_TxBfProfileTag_SuMu(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTag_SuMu prInBuf = %s, ucSuMu = %d\n",
		       prInBuf,
		       ucSuMu);
...
int Set_TxBfProfileTag_Mem(struct net_device *prNetDev, u8 *prInBuf)
{
	u32 aucInput[8];
	s32 i4Status = 0;
	u8 aucMemAddrColIdx[4], aucMemAddrRowIdx[4];
	s32 rv;
	...
	rv = sscanf(prInBuf, "%d:%d:%d:%d:%d:%d:%d:%d", &aucInput[0],
		    &aucInput[1], &aucInput[2], &aucInput[3], &aucInput[4],
		    &aucInput[5], &aucInput[6], &aucInput[7]);
	...
	if (rv == 8) {
		...
		aucMemAddrColIdx[0] = (u8)aucInput[0];
		aucMemAddrRowIdx[0] = (u8)aucInput[1];
		aucMemAddrColIdx[1] = (u8)aucInput[2];
		aucMemAddrRowIdx[1] = (u8)aucInput[3];
		aucMemAddrColIdx[2] = (u8)aucInput[4];
		aucMemAddrRowIdx[2] = (u8)aucInput[5];
		aucMemAddrColIdx[3] = (u8)aucInput[6];
		aucMemAddrRowIdx[3] = (u8)aucInput[7];
...
{
	u32 aucInput[6];
	u8 ucNrow, ucNcol, ucNgroup, ucLM, ucCodeBook, ucHtcExist;
	s32 i4Status = 0;
	s32 rv;
	...
	rv = sscanf(prInBuf, "%d:%d:%d:%d:%d:%d", &aucInput[0], &aucInput[1],
		    &aucInput[2], &aucInput[3], &aucInput[4], &aucInput[5]);
	...
	if (rv == 6) {
		DBGLOG(RFTEST,
		       ...
		       aucInput[4],
		       aucInput[5]);
		ucNrow = (u8)aucInput[0];
		ucNcol = (u8)aucInput[1];
		ucNgroup = (u8)aucInput[2];
		ucLM = (u8)aucInput[3];
		ucCodeBook = (u8)aucInput[4];
		ucHtcExist = (u8)aucInput[5];

		i4Status = TxBfProfileTag_Matrix(prNetDev, &g_rPfmuTag1, ucNrow,
						 ucNcol, ucNgroup, ucLM,
...
{
	u32 aucInput[4];
	u8 ucSNR_STS0, ucSNR_STS1, ucSNR_STS2, ucSNR_STS3;
	s32 i4Status = 0;
	s32 rv;
	...
	rv = sscanf(prInBuf, "%d:%d:%d:%d", &aucInput[0], &aucInput[1],
		    &aucInput[2], &aucInput[3]);
	if (rv == 4) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 : ATE_AGENT iwpriv Set_TxBfProfileTag_SNR aucInput:%d:%d:%d:%d\n",
		       aucInput[0],
		       aucInput[1],
		       aucInput[2],
		       aucInput[3]);

		ucSNR_STS0 = (u8)aucInput[0];
		ucSNR_STS1 = (u8)aucInput[1];
		ucSNR_STS2 = (u8)aucInput[2];
		ucSNR_STS3 = (u8)aucInput[3];

		i4Status = TxBfProfileTag_SNR(prNetDev, &g_rPfmuTag1,
					      ucSNR_STS0, ucSNR_STS1,
					      ucSNR_STS2, ucSNR_STS3);
	} else {
		return -EINVAL;
	}
...
int Set_TxBfProfileTag_SmartAnt(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTag_SmartAnt prInBuf = %s, ucSmartAnt = %d\n",
		       prInBuf,
		       ucSmartAnt);
...
int Set_TxBfProfileTag_SeIdx(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 TxBfProfileTag_SeIdx prInBuf = %s, ucSeIdx = %d\n",
		       prInBuf,
		       ucSeIdx);
		i4Status =
			TxBfProfileTag_SeIdx(prNetDev, &g_rPfmuTag2, ucSeIdx);
	} else {
		return -EINVAL;
	}
...
int Set_TxBfProfileTag_RmsdThrd(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTag_RmsdThrd prInBuf = %s, ucRmsdThrd = %d\n",
		       prInBuf,
		       ucRmsdThrd);
...
int Set_TxBfProfileTag_McsThrd(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 6) {
		...
		ucMcsLss[0] = (u8)aucInput[0];
		ucMcsSss[0] = (u8)aucInput[1];
		ucMcsLss[1] = (u8)aucInput[2];
		ucMcsSss[1] = (u8)aucInput[3];
		ucMcsLss[2] = (u8)aucInput[4];
		ucMcsSss[2] = (u8)aucInput[5];
...
int Set_TxBfProfileTag_TimeOut(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTag_TimeOut prInBuf = %s, ucTimeOut = %d\n",
		       prInBuf,
		       ucTimeOut);
...
int Set_TxBfProfileTag_DesiredBW(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTag_DesiredBW prInBuf = %s, ucDesiredBW = %d\n",
		       prInBuf,
		       ucDesiredBW);
...
int Set_TxBfProfileTag_DesiredNc(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTag_DesiredNc prInBuf = %s, ucDesiredNc = %d\n",
		       prInBuf,
		       ucDesiredNc);
...
int Set_TxBfProfileTag_DesiredNr(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTag_DesiredNr prInBuf = %s, ucDesiredNr = %d\n",
		       prInBuf,
		       ucDesiredNr);
...
int Set_TxBfProfileTagWrite(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfileTagWrite prInBuf = %s, profileIdx = %d\n",
		       prInBuf,
		       profileIdx);
...
{
	u32 profileIdx, fgBFer, subcarrierIdxMsb, subcarrierIdxLsb;
	s32 i4Status = 0;
	s32 rv;
...
int Set_TxBfProfileDataWrite(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	rv = sscanf(prInBuf,
		    ...
		    &au4Phi[1], &au4Psi[1], &au4Phi[2], &au4Psi[2], &au4Phi[3],
		    &au4Psi[3], &au4Phi[4], &au4Psi[4], &au4Phi[5], &au4Psi[5],
...
int Set_TxBfProfilePnRead(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_TxBfProfilePnRead prInBuf = %s, profileIdx = %d\n",
		       prInBuf,
		       profileIdx);
...
int Set_TxBfProfilePnWrite(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d",
		    &ucProfileIdx, &u2bw, &au2XSTS[0], &au2XSTS[1], &au2XSTS[2],
		    &au2XSTS[3], &au2XSTS[4], &au2XSTS[5], &au2XSTS[6],
		    &au2XSTS[7], &au2XSTS[8], &au2XSTS[9], &au2XSTS[10],
		    &au2XSTS[11]);
	if (rv == 14) {
		DBGLOG(RFTEST,
		       ERROR,
		       ...
		       au2XSTS[10],
		       au2XSTS[11]);
		i4Status = TxBfProfilePnWrite(prNetDev, ucProfileIdx, u2bw,
					      au2XSTS);
	} else {
		return -EINVAL;
	}
...
{
	u32 ucSuMu, ucNumSta, ucSndInterval, ucWLan0, ucWLan1, ucWLan2, ucWLan3;
	s32 i4Status = 0;
	s32 rv;
	...
	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x:%x", &ucSuMu, &ucNumSta,
...
{
	u32 u4WlanId, u4ETxBf, u4ITxBf, u4MuTxBf;
	s32 i4Status = 0;
	s32 rv;
	...
	rv = sscanf(prInBuf, "%d:%d:%d:%d", &u4WlanId, &u4ETxBf, &u4ITxBf,
		    &u4MuTxBf);
	if (rv == 4) {
		DBGLOG(RFTEST,
		       ERROR,
...
{
	s32 au4Mac[MAC_ADDR_LEN];
	s32 u4Type, u4Wtbl, u4Ownmac, u4PhyMode, u4Bw, u4Nss, u4PfmuId, u4Mode,
	    u4Marate, u4SpeIdx, ucaid, u4Rv;
	s8 aucMac[MAC_ADDR_LEN];
	s32 i4Status = 0;
	s32 i = 0;
	s32 rv;
	...
	rv = sscanf(prInBuf,
		    ...
		    &au4Mac[5], &u4Type, &u4Wtbl, &u4Ownmac, &u4PhyMode, &u4Bw,
		    &u4Nss, &u4PfmuId, &u4Mode, &u4Marate, &u4SpeIdx, &ucaid,
		    &u4Rv);
	if (rv == 18) {
		DBGLOG(RFTEST,
		       ERROR,
		...
		i4Status = TxBfManualAssoc(prNetDev, aucMac, u4Type, u4Wtbl,
					   u4Ownmac, u4Mode, u4Bw, u4Nss,
...
int Set_TxBfPfmuMemAlloc(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 2) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 TxBfPfmuMemAlloc ucSuMuMode = %d, ucWlanIdx = %d",
		       ucSuMuMode,
		       ucWlanIdx);
...
int Set_TxBfPfmuMemRelease(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT6632 TxBfPfmuMemRelease ucWlanId = %d",
		       ucWlanId);
...
{
	u32 u4OwnMacIdx, fgBand;
	u32 OwnMacAddr[MAC_ADDR_LEN];
	...
	rv = sscanf(prInBuf, "%d:%x:%x:%x:%x:%x:%x:%d", &u4OwnMacIdx,
...
{
	u32 u4OwnMacIdx, u4BssIdx;
	u32 au4BssId[MAC_ADDR_LEN];
	...
	rv = sscanf(prInBuf, "%d:%d:%x:%x:%x:%x:%x:%x", &u4OwnMacIdx, &u4BssIdx,
		    &au4BssId[0], &au4BssId[1], &au4BssId[2], &au4BssId[3],
		    &au4BssId[4], &au4BssId[5]);
	if (rv == 8) {
		DBGLOG(RFTEST,
		       ERROR,
...
{
	u32 u4WlanId, u4BssId, u4Aid;
	u32 au4MacAddr[MAC_ADDR_LEN];
	u8 aucMacAddr[MAC_ADDR_LEN];
	s32 i4Status = 0;
	u32 i;
	s32 rv;
	...
	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x:%x:%x:%x", &u4WlanId, &u4BssId,
		    &u4Aid, &au4MacAddr[0], &au4MacAddr[1], &au4MacAddr[2],
	...
	if (rv == 9) {
		...
		i4Status = StaRecCmmUpdate(prNetDev, u4WlanId, u4BssId, u4Aid,
					   aucMacAddr);
	} else {
		return -EINVAL;
	}
...
int Set_StaRecBfUpdate(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 23) {
		/*
		 * DBGLOG(RFTEST, ERROR,
		 *"MT6632 Set_StaRecBfUpdate prInBuf = %s, u4WlanId = %x,
		 * u4BssId = %x, u4Aid = %x, aucMacAddr[0]:%x, aucMacAddr[1]:%x,
		 * aucMacAddr[2]:%x, aucMacAddr[3]:%x, aucMacAddr[4]:%x,
		 *   aucMacAddr[5]:%x",
		...
		i4Status = StaRecBfUpdate(prNetDev, rStaRecBfUpdArg, aucMemRow,
					  aucMemCol);
	} else {
		return -EINVAL;
	}
...
int Set_MUGetInitMCS(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		...
		DBGLOG(RFTEST, ERROR,
		       "MT6632 Set_MUGetInitMCS prInBuf = %s, u4groupIdx = %x",
		       prInBuf, u4groupIdx);
		...
		i4Status = kalIoctl(prGlueInfo, wlanoidMuMimoAction,
				    &rMuMimoActionInfo,
				    sizeof(rMuMimoActionInfo), true, true, true,
				    &u4BufLen);
	} else {
		return -EINVAL;
	}
...
int Set_MUCalInitMCS(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	u32 u4NumOfUser, u4Bandwidth, u4NssOfUser0, u4NssOfUser1,
	    u4PfMuIdOfUser0, u4PfMuIdOfUser1, u4NumOfTxer, u4SpeIndex,
	    u4GroupIndex;
	s32 rv;
	...
	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x:%x:%x:%x", &u4NumOfUser,
	...
	if (rv == 9) {
		...
		i4Status = kalIoctl(prGlueInfo, wlanoidMuMimoAction,
				    &rMuMimoActionInfo,
				    sizeof(rMuMimoActionInfo), false, false,
				    true, &u4BufLen);
	} else {
		return -EINVAL;
	}
...
int Set_MUCalLQ(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x:%x:%x:%x:%x", &u4NumOfUser,
		    &u4Bandwidth, &u4NssOfUser0, &u4NssOfUser1,
		    &u4PfMuIdOfUser0, &u4PfMuIdOfUser1, &u4NumOfTxer,
		    &u4SpeIndex, &u4GroupIndex);
	if (rv == 9) {
...
int Set_MUGetLQ(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	/* { */
	/* DBGLOG(RFTEST, ERROR, "MT6632 Set_MUGetLQ prInBuf = %s, u4Type = %x",
	 * prInBuf, u4Type); */
...
int Set_MUSetSNROffset(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR,
		       "MT6632 Set_MUSetSNROffset prInBuf = %s, u4Val = %x",
		       prInBuf, u4Val);
...
int Set_MUSetZeroNss(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR,
		       "MT6632 Set_MUSetZeroNss prInBuf = %s, u4Val = %x",
		       prInBuf, u4Val);
...
int Set_MUSetSpeedUpLQ(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR,
		       "MT6632 Set_MUSetSpeedUpLQ prInBuf = %s, u4Val = %x",
		       prInBuf, u4Val);
...
int Set_MUSetMUTable(struct net_device *prNetDev, u8 *prTable)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	PARAM_CUSTOM_MUMIMO_ACTION_STRUCT_T rMuMimoActionInfo;
	s32 i4Status = 0;
	...
	i4Status = kalIoctl(prGlueInfo, wlanoidMuMimoAction, &rMuMimoActionInfo,
			    sizeof(rMuMimoActionInfo), false, false, true,
			    &u4BufLen);
	/* } */
	/* else */
	/* { */
...
int Set_MUSetGroup(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	u32 aucUser0MacAddr[PARAM_MAC_ADDR_LEN],
	...
	if (sscanf(prInBuf,
		...
		DBGLOG(RFTEST,
		       ...
		       rMuMimoActionInfo.unMuMimoParam.rMuSetGroup.u4User0MuPfId,
		       rMuMimoActionInfo.unMuMimoParam.rMuSetGroup.u4User1MuPfId,
		       rMuMimoActionInfo.unMuMimoParam.rMuSetGroup
		       .u4User0InitMCS,
		       rMuMimoActionInfo.unMuMimoParam.rMuSetGroup
		       .u4User1InitMCS,
		       aucUser0MacAddr[0],
		...
		for (i = 0; i < PARAM_MAC_ADDR_LEN; i++) {
			rMuMimoActionInfo.unMuMimoParam.rMuSetGroup
			.aucUser0MacAddr[i] = aucUser0MacAddr[i];
			rMuMimoActionInfo.unMuMimoParam.rMuSetGroup
			.aucUser1MacAddr[i] = aucUser1MacAddr[i];
		}
...
int Set_MUGetQD(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	u32 u4SubcarrierIndex, u4Length;
	...
	if (sscanf(prInBuf, "%x:%x", &u4SubcarrierIndex, &u4Length) == 2) {
		DBGLOG(RFTEST,
		       ERROR,
		       "MT6632 Set_MUGetQD prInBuf = %s, u4SubcarrierIndex = %x, u4Length = %x",
		       prInBuf,
		       u4SubcarrierIndex,
		       u4Length);
...
int Set_MUSetEnable(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR,
		       "MT6632 Set_MUSetEnable prInBuf = %s, u4Val = %x",
		       prInBuf, u4Val);
...
int Set_MUTriggerTx(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	u32 u4IsRandomPattern, u4MsduPayloadLength0, u4MsduPayloadLength1,
	    u4MuPacketCount, u4NumOfSTAs;
	u32 au4MacAddrs[2][6];
	...
	if (sscanf(prInBuf,
		   ...
		   &au4MacAddrs[0][3], &au4MacAddrs[0][4], &au4MacAddrs[0][5],
		   &au4MacAddrs[1][0], &au4MacAddrs[1][1], &au4MacAddrs[1][2],
		   &au4MacAddrs[1][3], &au4MacAddrs[1][4],
		   &au4MacAddrs[1][5]) == 17) {
		DBGLOG(RFTEST,
		       ERROR,
		...
		.fgIsRandomPattern = u4IsRandomPattern;
		rMuMimoActionInfo.unMuMimoParam.rMuTriggerMuTx
		.u4MsduPayloadLength0 = u4MsduPayloadLength0;
		rMuMimoActionInfo.unMuMimoParam.rMuTriggerMuTx
		.u4MsduPayloadLength1 = u4MsduPayloadLength1;
		...
		for (i = 0; i < 2; i++) {
			for (j = 0; j < PARAM_MAC_ADDR_LEN; j++)
				rMuMimoActionInfo.unMuMimoParam.rMuTriggerMuTx
				.aucMacAddrs[i][j] = au4MacAddrs[i][j];
		}
...
int SetTxTargetPower(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO,
	       "MT6632 : ATE_AGENT iwpriv Set Tx Target Power, buf: %s\n",
	       prInBuf);
	...
	DBGLOG(REQ, INFO,
	       "MT6632 : ATE_AGENT iwpriv Set Tx Target Power, prInBuf: %s\n",
	       prInBuf);
	DBGLOG(INIT, ERROR,
	       "MT6632 : ATE_AGENT iwpriv Set Tx Target Power :%02x\n", addr);
...
int SetRddReport(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO,
	       "MT6632 : ATE_AGENT iwpriv Set RDD Report, prInBuf: %s\n",
	       prInBuf);
	DBGLOG(INIT, ERROR,
	       "MT6632 : ATE_AGENT iwpriv Set RDD Report : Band %d\n", dbdcIdx);
...
int SetByPassCac(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO,
	       "MT6632 : ATE_AGENT iwpriv Set By Pass Cac, buf: %s\n", prInBuf);

	rv = kstrtoint(prInBuf, 0, &i4ByPassCacTime);

	DBGLOG(REQ, INFO,
	       "MT6632 : ATE_AGENT iwpriv Set By Pass Cac, prInBuf: %s\n",
	       prInBuf);
	DBGLOG(INIT, ERROR,
	       "MT6632 : ATE_AGENT iwpriv Set By Pass Cac : %dsec\n",
	       i4ByPassCacTime);
	...
	u4ByPassCacTime = (u32)i4ByPassCacTime;
...
int SetRadarDetectMode(struct net_device *prNetDev, u8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO,
	       "MT6632 : ATE_AGENT iwpriv Set Radar Detect Mode, buf: %s\n",
	       prInBuf);

	rv = kstrtoint(prInBuf, 0, &radarDetectMode);

	DBGLOG(REQ, INFO,
	       "MT6632 : ATE_AGENT iwpriv Set Radar Detect Mode, prInBuf: %s\n",
	       prInBuf);
	DBGLOG(INIT,
	       ERROR,
	       "MT6632 : ATE_AGENT iwpriv Set Radar Detect Mode : %d\n",
	       radarDetectMode);
	...
	ucRadarDetectMode = (u8)radarDetectMode;

	p2pFuncSetRadarDetectMode(ucRadarDetectMode);

	if (rv == 0) {
		i4Status =
			MT_ATESetRadarDetectMode(prNetDev, ucRadarDetectMode);
	} else {
		return -EINVAL;
	}
...
int AteCmdSetHandle(struct net_device *prNetDev, u8 *prInBuf, u32 u4InBufLen)
{
	...
	while ((this_char = strsep((char **)&prInBuf, ",")) != NULL) {
		if (!*this_char)
			continue;
		DBGLOG(RFTEST, ERROR,
		       "MT6632 : ATE_AGENT iwpriv this_char = %s\n", this_char);
		DBGLOG(RFTEST, INFO,
		       "MT6632 : ATE_AGENT iwpriv this_char = %s\n", this_char);
		...
		DBGLOG(REQ, INFO,
		       "MT6632 : ATE_AGENT iwpriv cmd = %s, value = %s\n",
		       this_char, value);
