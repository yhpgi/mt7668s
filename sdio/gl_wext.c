...
const long channel_freq[] = { 2412, 2417, 2422, 2427, 2432, 2437, 2442,
...
static const struct iw_priv_args rIwPrivTable[] = {
	...
	{ IOCTL_SET_STRUCT, IW_PRIV_TYPE_CHAR | sizeof(NDIS_TRANSPORT_STRUCT),
	  0, "" },
	{ IOCTL_GET_STRUCT, 0,
	  IW_PRIV_TYPE_CHAR | sizeof(NDIS_TRANSPORT_STRUCT), "" },
	...
	{ PRIV_CMD_CSUM_OFFLOAD, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,
	  "set_tcp_csum" },
#endif

	{ PRIV_CMD_POWER_MODE, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,
	  "set_power_mode" },
	{ PRIV_CMD_POWER_MODE, 0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1,
	  "get_power_mode" },

	{ PRIV_CMD_WMM_PS, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 3, 0,
	  "set_wmm_ps" },

	{ PRIV_CMD_TEST_MODE, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,
	  "set_test_mode" },
	{ PRIV_CMD_TEST_CMD, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 2, 0,
	  "set_test_cmd" },
	...
	{ PRIV_CMD_ACCESS_MCR, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 2, 0,
	  "set_mcr" },
	...
	{ PRIV_CMD_SW_CTRL, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 2, 0,
	  "set_sw_ctrl" },
	{ PRIV_CMD_SW_CTRL, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1,
	  IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_sw_ctrl" },

#if CFG_SUPPORT_BCM && CFG_SUPPORT_BCM_BWCS
	{ PRIV_CUSTOM_BWCS_CMD, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,
	  "set_bwcs" },
	...
	{ PRIV_CMD_BAND_CONFIG, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,
	  "set_band" },
	{ PRIV_CMD_BAND_CONFIG, 0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1,
	  "get_band" },
	...
	{ PRIV_CMD_P2P_MODE, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 2, 0,
	  "set_p2p_mode" },
#endif
	{ PRIV_CMD_MET_PROFILING, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 2, 0,
	  "set_met_prof" },
};
...
const struct iw_handler_def wext_handler_def = {
	...
	.num_private_args =
		(__u16)sizeof(rIwPrivTable) / sizeof(struct iw_priv_args),
#endif
...
static void wext_support_ioctl_SIOCSIWGENIE(IN P_GLUE_INFO_T prGlueInfo,
					    IN char *prExtraBuf,
					    IN u32 u4ExtraSize);

static void wext_support_ioctl_SIOCSIWPMKSA_Action(IN struct net_device *prDev,
						   IN char *prExtraBuf,
						   IN int ioMode,
						   OUT int *ret);
 ...
 *******************************************************************************
 */
void MAP_CHANNEL_ID_TO_KHZ(u32 ch, u32 khz)
{
	switch (ch) {
	case 1:
		khz = 2412000;
 ...
 */
/*----------------------------------------------------------------------------*/
u8 wextSrchDesiredWPAIE(IN u8 *pucIEStart, IN s32 i4TotalIeLen,
			IN u8 ucDesiredElemId, OUT u8 **ppucDesiredIE)
{
	s32 i4InfoElemLen;
	...
	while (i4TotalIeLen >= 2) {
		i4InfoElemLen = (s32)pucIEStart[1] + 2;
 ...
 */
/*----------------------------------------------------------------------------*/
u8 wextSrchDesiredWPSIE(IN u8 *pucIEStart, IN s32 i4TotalIeLen,
			IN u8 ucDesiredElemId, OUT u8 **ppucDesiredIE)
{
	s32 i4InfoElemLen;
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_name(IN struct net_device *prNetDev,
			 IN struct iw_request_info *prIwrInfo, OUT char *pcName,
			 IN char *pcExtra)
{
	...
	if (netif_carrier_ok(prNetDev)) {
		rStatus = kalIoctl(prGlueInfo, wlanoidQueryNetworkTypeInUse,
				   &eNetWorkType, sizeof(eNetWorkType), true,
				   false, false, &u4BufLen);

		switch (eNetWorkType) {
		case PARAM_NETWORK_TYPE_DS:
			strncpy(pcName, "IEEE 802.11b",
				sizeof(((struct iwreq *)0)->u.name));
			break;

		case PARAM_NETWORK_TYPE_OFDM24:
			strncpy(pcName, "IEEE 802.11bgn",
				sizeof(((struct iwreq *)0)->u.name));
			break;

		case PARAM_NETWORK_TYPE_AUTOMODE:
		case PARAM_NETWORK_TYPE_OFDM5:
			strncpy(pcName, "IEEE 802.11abgn",
				sizeof(((struct iwreq *)0)->u.name));
			break;

		case PARAM_NETWORK_TYPE_FH:
		default:
			strncpy(pcName, "IEEE 802.11",
				sizeof(((struct iwreq *)0)->u.name));
			break;
		}
	} else {
		strncpy(pcName, "Disconnected",
			sizeof(((struct iwreq *)0)->u.name));
	}
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_set_freq(IN struct net_device *prNetDev,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_freq(IN struct net_device *prNetDev,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_set_mode(IN struct net_device *prNetDev,
			 IN struct iw_request_info *prIwReqInfo,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_mode(IN struct net_device *prNetDev,
			 IN struct iw_request_info *prIwReqInfo,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_range(IN struct net_device *prNetDev,
			  IN struct iw_request_info *prIwrInfo,
			  IN struct iw_point *prData, OUT char *pcExtra)
{
	...
	prRange->event_capa[0] =
		(IW_EVENT_CAPA_K_0 | IW_EVENT_CAPA_MASK(SIOCGIWAP) |
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidQuerySupportedRates, &aucSuppRate,
			   sizeof(aucSuppRate), true, false, false, &u4BufLen);

	for (i = 0; i < IW_MAX_BITRATES && i < PARAM_MAX_LEN_RATES_EX; i++) {
		if (aucSuppRate[i] == 0)
			break;
		prRange->bitrate[i] =
			(aucSuppRate[i] & 0x7F) * 500000; /* 0.5Mbps
		                                           */
	}
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_set_ap(IN struct net_device *prDev,
		       IN struct iw_request_info *prIwrInfo,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_ap(IN struct net_device *prNetDev,
		       IN struct iw_request_info *prIwrInfo,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_set_mlme(IN struct net_device *prNetDev,
			 IN struct iw_request_info *prIwrInfo,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_set_scan(IN struct net_device *prNetDev,
			 IN struct iw_request_info *prIwrInfo,
			 IN union iwreq_data *prData, IN char *pcExtra)
{
	...
	/* retrieve SSID */
	if (prData) {
		essid_len = ((struct iw_scan_req *)(((struct iw_point *)prData)
						    ->pointer))
			    ->essid_len;
	}
#endif
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetBssidListScan, pcExtra,
			   essid_len, false, false, false, &u4BufLen);

	/* wait_for_completion_interruptible_timeout(&prGlueInfo->rScanComp, 2 *
	 * KAL_HZ); */
 ...
 * of pcExtra buffer before used, and is updated after filling scan results.
 * \param[out] pcExtra Pointer to buffer which is allocated by caller of this
 *                     function, wext_support_ioctl() or ioctl_standard_call()
 * in wireless.c.
 *
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_scan(IN struct net_device *prNetDev,
			 IN struct iw_request_info *prIwrInfo,
			 IN OUT struct iw_point *prData, IN char *pcExtra)
{
	...
	if (u4AllocBufLen < sizeof(PARAM_BSSID_LIST_EX_T)) {
		DBGLOG(INIT, WARN, "[wifi] invalid input length:%d < %d\n",
		       prData->length, sizeof(PARAM_BSSID_LIST_EX_T));
		return -EINVAL;
	}
	...
	/* wait_for_completion_interruptible_timeout(&prGlueInfo->rScanComp, 4 *
	 * KAL_HZ); */
	...
	if (rStatus == WLAN_STATUS_INVALID_LENGTH) {
		...
		goto error;
#else
		/* Realloc a larger query buffer here, but don't write too much
		 * to extra
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidQueryBssidList, prList,
				   u4AllocBufLen, true, false, false,
				   &u4BufLen);
	...
	for (i = 0; i < prList->u4NumberOfItems; ++i) {
		...
		for (j = 0; j < PARAM_MAX_LEN_RATES_EX; ++j) {
			u8 curRate = prBss->rSupportedRates[j] & 0x7F;
		...
		if (wextSrchDesiredWPAIE(
			    &prBss->aucIEs[sizeof(PARAM_FIXED_IEs)],
			    prBss->u4IELength - sizeof(PARAM_FIXED_IEs), 0x30,
			    (u8 **)&prDesiredIE)) {
			iwEvent.cmd = IWEVGENIE;
			iwEvent.u.data.flags = 1;
			iwEvent.u.data.length =
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_set_essid(IN struct net_device *prNetDev,
			  IN struct iw_request_info *prIwrInfo,
			  IN struct iw_point *prEssid, IN char *pcExtra)
{
	...
	if (prGlueInfo->rWpaInfo.u4WpaVersion == IW_AUTH_WPA_VERSION_DISABLED) {
		...
		case IW_AUTH_KEY_MGMT_802_1X:
			eAuthMode = (prGlueInfo->rWpaInfo.u4WpaVersion ==
	...
	if (kalIoctl(prGlueInfo, wlanoidSetSsid, (void *)&rNewSsid,
		     sizeof(PARAM_SSID_T), false, false, true,
 ...
 * information. \param[out] pcExtra Pointer to buffer srtoring essid string.
 *
 * \retval 0 If netif_carrier_ok.
 * \retval -ENOTCONN Otherwise.
 *
...
static int wext_get_essid(IN struct net_device *prNetDev,
			  IN struct iw_request_info *prIwrInfo,
			  IN struct iw_point *prEssid, OUT char *pcExtra)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidQuerySsid, prSsid,
			   sizeof(PARAM_SSID_T), true, false, false, &u4BufLen);
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_rate(IN struct net_device *prNetDev,
			 IN struct iw_request_info *prIwrInfo,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_set_rts(IN struct net_device *prNetDev,
			IN struct iw_request_info *prIwrInfo,
			IN struct iw_param *prRts, IN char *pcExtra)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetRtsThreshold, &u4RtsThresh,
			   sizeof(u4RtsThresh), false, false, false, &u4BufLen);
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_rts(IN struct net_device *prNetDev,
			IN struct iw_request_info *prIwrInfo,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_frag(IN struct net_device *prNetDev,
			 IN struct iw_request_info *prIwrInfo,
...
static int wext_set_txpow(IN struct net_device *prNetDev,
			  IN struct iw_request_info *prIwrInfo,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_txpow(IN struct net_device *prNetDev,
			  IN struct iw_request_info *prIwrInfo,
			  OUT struct iw_param *prTxPow, IN char *pcExtra)
{
	...
	prTxPow->value = 100;
	prTxPow->fixed = 1;
	/* prTxPow->disabled = (ePowerState != ParamDeviceStateD3) ? false :
	 * true; */
 ...
 * information. \param[in] pcExtra Buffer to store key content.
 *
 * \retval 0 Success.
 *
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_encode(IN struct net_device *prNetDev,
			   IN struct iw_request_info *prIwrInfo,
 ...
 * information. \param[in] pcExtra Pointer to key string buffer.
 *
 * \retval 0 Success.
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_set_encode(IN struct net_device *prNetDev,
			   IN struct iw_request_info *prIwrInfo,
			   IN struct iw_point *prEnc, IN char *pcExtra)
{
	...
	if ((prEnc->flags & IW_ENCODE_MODE) == IW_ENCODE_DISABLED) {
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidSetAuthMode, &eAuthMode,
				   sizeof(eAuthMode), false, false, false,
				   &u4BufLen);

		eEncStatus = ENUM_ENCRYPTION_DISABLED;

		rStatus = kalIoctl(prGlueInfo, wlanoidSetEncryptionStatus,
				   &eEncStatus, sizeof(eEncStatus), false,
				   false, false, &u4BufLen);
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_set_power(IN struct net_device *prNetDev,
			  IN struct iw_request_info *prIwrInfo,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_get_power(IN struct net_device *prNetDev,
			  IN struct iw_request_info *prIwrInfo,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_set_auth(IN struct net_device *prNetDev,
			 IN struct iw_request_info *prIwrInfo,
 ...
 */
/*----------------------------------------------------------------------------*/
static int wext_set_encode_ext(IN struct net_device *prNetDev,
			       IN struct iw_request_info *prIwrInfo,
			       IN struct iw_point *prEnc, IN char *pcExtra)
{
	u8 wepBuf[48];
...

	{
		if ((prEnc->flags & IW_ENCODE_MODE) == IW_ENCODE_DISABLED) {
			...
			memcpy(prRemoveKey->arBSSID, prIWEncExt->addr.sa_data,
			       6);
			/*
		...
		case IW_ENCODE_ALG_WEP:
			...
			if (prIWEncExt->key_len == 5 ||
				...
				rStatus = kalIoctl(
					prGlueInfo, wlanoidSetEncryptionStatus,
					&eEncStatus,
					sizeof(ENUM_PARAM_ENCRYPTION_STATUS_T),
					false, false, false, &u4BufLen);
...
#endif
		{
			...
			if (prIWEncExt->alg == IW_ENCODE_ALG_TKIP &&
				...
				memcpy(prKey->aucKeyMaterial, prIWEncExt->key,
				       16);
				memcpy(((u8 *)prKey->aucKeyMaterial) + 16,
				       prIWEncExt->key + 24, 8);
				memcpy((prKey->aucKeyMaterial) + 24,
				       prIWEncExt->key + 16, 8);
			} else {
				/* aucKeyMaterial is defined as a 32-elements
				 * array */
				if (prIWEncExt->key_len > 32) {
					DBGLOG(REQ,
					       ERROR,
					       "prIWEncExt->key_len: %d is too long!\n",
					       prIWEncExt->key_len);
					ret = -EFAULT;
					goto freeBuf;
				}
			...
			rStatus = kalIoctl(prGlueInfo, wlanoidSetAddKey, prKey,
					   prKey->u4Length, false, false, true,
					   &u4BufLen);
			    ...
			    IN struct iw_point *prData)
{
	 ...
	 * and "COUNTRY JP"
	 */
	if (GLUE_CHK_PR2(prNetDev, prData) == false || !prData->pointer ||
	    prData->length < 10)
		return -EINVAL;

	prGlueInfo = *((P_GLUE_INFO_T *)netdev_priv(prNetDev));

	aucCountry[0] = *((u8 *)prData->pointer + 8);
	aucCountry[1] = *((u8 *)prData->pointer + 9);
...
{
	u16 u2BufferSize = prData->length;
 ...
 */
/*----------------------------------------------------------------------------*/
int wext_support_ioctl(IN struct net_device *prDev, IN struct ifreq *prIfReq,
		       IN int i4Cmd)
{
	...
	/* case SIOCGIWSTATS:
	 *  get statistics, intercepted by wireless_process_ioctl() in
	 * wireless.c, redirected to dev_iwstats(),
	...
	case SIOCSIWAP: /* 0x8B14, set access point MAC addresses (BSSID) */
		if (iwr->u.ap_addr.sa_data[0] == 0 &&
			...
			** wpa_driver_wext_deinit(), do nothing here or disassoc
			* again?
			*/
			ret = 0;
			break;
	...
	case SIOCSIWSCAN: /* 0x8B18, scan request */
		if (iwr->u.data.pointer == NULL) {
			...
			if (req->essid_len > MAX_SSID_LEN) {
				ret = -EFAULT;
			} else if (kalMemCopy(prExtraBuf, req->essid,
					      req->essid_len)) {
				ret = -EFAULT;
			} else {
				ret = wext_set_scan(
	...
	case SIOCGIWESSID: /* 0x8B1B, get SSID */
		...
		if (iwr->u.essid.length != IW_ESSID_MAX_SIZE &&
			...
			DBGLOG(INIT, INFO,
			       "[wifi] iwr->u.essid.length:%d error\n",
			       iwr->u.essid.length);
	...
	case SIOCSIWENCODE: /* 0x8B2A, set encoding token & mode */
		...
		if (ret == 0) {
			ret = wext_set_encode(prDev, NULL, &iwr->u.encoding,
					      prExtraBuf);
		}
	...
	case SIOCSIWENCODEEXT: /* 0x8B34, set extended encoding token & mode */
		if (iwr->u.encoding.pointer) {
			...
			/* check here was less than < before, but changed to !=
			 * lest malicious big length according to driver code,
			 * max key len is only 32, so check that too.
			 */
			if (u4ExtraSize < sizeof(struct iw_encode_ext) ||
					    ...
					    IN u32 u4ExtraSize)
{
	...
	if (wextSrchDesiredWPSIE(prExtraBuf, u4ExtraSize, 0xDD,
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidSetWSCAssocInfo,
				   prDesiredIE, IE_SIZE(prDesiredIE), false,
				   false, true, &u4BufLen);
		if (rStatus != WLAN_STATUS_SUCCESS) {
			/* do nothing */
		}
						   ...
						   IN int ioMode, OUT int *ret)
{
	...
	switch (ioMode) {
	case IW_PMKSA_ADD:
		prPmkid = (P_PARAM_PMKID_T)kalMemAlloc(
			8 + sizeof(PARAM_BSSID_INFO_T), VIR_MEM_TYPE);
		if (!prPmkid) {
			DBGLOG(INIT, INFO,
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidSetPmkid, prPmkid,
				   sizeof(PARAM_PMKID_T), false, false, true,
				   &u4BufLen);
		if (rStatus != WLAN_STATUS_SUCCESS)
 ...
 * valid. Using of kernel symbol wireless_send_event(), which is defined in
 *      <net/iw_handler.h> after WE-14 (2.4.20).
 */
/*----------------------------------------------------------------------------*/
void wext_indicate_wext_event(IN P_GLUE_INFO_T prGlueInfo,
			      ...
			      IN unsigned int u4dataLen)
{
	...
	case IWEVASSOCREQIE:
		...
		if (((pucDesiredIE[1] + 2) * 2) > (len - 1)) {
			DBGLOG(INIT, INFO,
			       "size of pucDesiredIE[1] exceeds buffer size\n");
			goto skip_indicate_event;
		}

		for (i = 0; i < pucDesiredIE[1] + 2; ++i) {
			ret = snprintf(pucExtraInfo, len, "%02x",
				       pucDesiredIE[i]);
			if (ret < 0) {
				DBGLOG(INIT, ERROR,
				       "%s:%d:snprintf return %d\n",
				       __FUNCTION__, __LINE__, ret);
				goto skip_indicate_event;
			}
			len -= ret;
	...
	case IWEVMICHAELMICFAILURE:
		...
		if (pucData) {
			...
			P_PARAM_AUTH_REQUEST_T pAuthReq =
				(P_PARAM_AUTH_REQUEST_T)pucData;
			/* under WE-18, only IWEVCUSTOM can be used */
			u4Cmd = IWEVCUSTOM;
			pucExtraInfo = aucExtraInfoBuf;

			i4BufLen =
				scnprintf(pucExtraInfo, sizeof(aucExtraInfoBuf),
					  "MLME-MICHAELMICFAILURE.indication ");

			i4BufLen += scnprintf(
				(pucExtraInfo + i4BufLen),
				(sizeof(aucExtraInfoBuf) - i4BufLen), "%s",
				(pAuthReq->u4Flags ==
	...
	case IWEVPMKIDCAND:
		if (prGlueInfo->rWpaInfo.u4WpaVersion ==
			...
			P_PARAM_PMKID_CANDIDATE_T prPmkidCand =
				(P_PARAM_PMKID_CANDIDATE_T)pucData;
			...
			kalMemCopy(pucExtraInfo, (u8 *)&rPmkidCand,
