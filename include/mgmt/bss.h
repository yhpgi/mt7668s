 ...
 *
 *    The file contains the function declarations and defines for used in
 * BSS/IBSS.
 */
...
#include "wlan_def.h"
extern const u8 *apucNetworkType[NETWORK_TYPE_NUM];
...
/* #define MAX_LEN_TIM_PARTIAL_BMP     (((MAX_ASSOC_ID + 1) + 7) / 8) */ /* Required
 *                                                                          bits
 *                                                                          =
 * (MAX_ASSOC_ID
 + 1)
...
WLAN_STATUS bssUpdateBeaconContent(IN P_ADAPTER_T prAdapter, IN u8 uBssIndex);
