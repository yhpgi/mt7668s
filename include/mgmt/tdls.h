...
#define TDLSEX_STA_REC_IDX_GET(__prAdapter__, __MsduInfo__)	       \
	{							       \
		...
		__StaRec__ = cnmGetStaRecByAddress(		       \
...
typedef struct _PARAM_CUSTOM_TDLS_CMD_STRUCT_T {
	u8 ucFmeType; /* TDLS_FRM_ACTION_ID */
	...
	u8 arRspAddr[TDLS_FME_MAC_ADDR_LEN];
	u8 arBssid[TDLS_FME_MAC_ADDR_LEN];
...
typedef struct _TDLS_CMD_LINK_MGT_T {
	...
	u16 u2StatusCode;
	u32 u4SecBufLen;
	u8 aucSecBuf[TDLS_SEC_BUF_LENGTH];
} TDLS_CMD_LINK_MGT_T;
...
typedef struct _TDLS_CMD_PEER_UPDATE_T {
	...
	u8 aucSupChan[TDLS_CMD_PEER_UPDATE_SUP_CHAN_MAX];

	u16 u2StatusCode;

	u8 aucSupRate[TDLS_CMD_PEER_UPDATE_SUP_RATE_MAX];
	u16 u2SupRateLen;
	...
	u8 aucExtCap[TDLS_CMD_PEER_UPDATE_EXT_CAP_MAXLEN];
	u16 u2ExtCapLen;
...
#define TDLS_CMD_CORE_RESERVED_SIZE    50
	union {
		...
		u8 Reserved[TDLS_CMD_CORE_RESERVED_SIZE];
	} Content;
} TDLS_CMD_CORE_T;
...
u32 TdlsFrameGeneralIeAppend(ADAPTER_T *prAdapter,
...
u32 TdlsexLinkOper(P_ADAPTER_T prAdapter,
		   void *pvSetBuffer,
		   u32 u4SetBufferLen,
		   u32 *pu4SetInfoLen);

u32 TdlsexLinkMgt(P_ADAPTER_T prAdapter,
		  void *pvSetBuffer,
		  u32 u4SetBufferLen,
		  u32 *pu4SetInfoLen);

void TdlsexEventHandle(P_GLUE_INFO_T prGlueInfo, u8 *prInBuf, u32 u4InBufLen);
