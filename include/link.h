...
#define LINK_MGMT_GET_ENTRY(prLinkMgmt, prEntry, EntryType, memType) \
	do { \
		LINK_REMOVE_HEAD(&((P_LINK_MGMT_T)prLinkMgmt)->rFreeLink, prEntry, EntryType *); \
		if (!prEntry) \
			prEntry = kalMemAlloc(sizeof(EntryType), memType); \
		if (prEntry) { \
			kalMemZero(prEntry, sizeof(EntryType)); \
			LINK_INSERT_TAIL(&((P_LINK_MGMT_T)prLinkMgmt)->rUsingLink, &prEntry->rLinkEntry); \
		} \
	} while (0)
...
#define LINK_MGMT_UNINIT(prLinkMgmt, EntryType, memType) \
	do { \
		...
		P_LINK_T   prUsingList = &((P_LINK_MGMT_T)prLinkMgmt)->rUsingLink; \
