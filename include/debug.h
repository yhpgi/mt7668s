...
#define DIV2INT(_dividend,						\
		_divisor)		    ((_divisor) ? (_dividend) /	\
					     (_divisor) : 0)
...
#define DBGLOG(_Module, _Class, _Fmt, ...)			    \
	do {							    \
		if ((aucDebugModule[DBG_ ## _Module ## _IDX] &	    \
		     DBG_CLASS_ ## _Class) == 0)		    \
		break;						    \
		LOG_FUNC("[%u]%s:(" #_Module " " #_Class ") " _Fmt, \
			 KAL_GET_CURRENT_THREAD_ID(), __func__,	    \
			 ## __VA_ARGS__);			    \
	} while (0)
#define DBGLOG_RATELIMIT(_Module, _Class, _Fmt, ...)		    \
	do {							    \
...
#define DBGLOG_MEM8(_Module, _Class, _StartAddr, _Length)		       \
	{								       \
		if (aucDebugModule[DBG_ ## _Module ## _IDX] &		       \
			...
			dumpMemory8((u8 *)(_StartAddr), (u32)(_Length));       \
		}							       \
	}
#define DBGLOG_MEM32(_Module, _Class, _StartAddr, _Length)		       \
	{								       \
		if (aucDebugModule[DBG_ ## _Module ## _IDX] &		       \
		    DBG_CLASS_ ## _Class) {				       \
			LOG_FUNC("%s:(" #_Module " " #_Class ")\n", __func__); \
			dumpMemory32((u32 *)(_StartAddr), (u32)(_Length));     \
		}							       \
	}
#endif
