...
#define LOOK_AHEAD_LEN						  (ETH_HLEN +	     \
								       IP_HEADER_LEN \
								       +	     \
								       SOURCE_PORT_LEN)
...
/* Bluetooth SNAP */
#define ETH_SNAP_OFFSET						  ( \
		ETHER_HEADER_LEN + ETH_LLC_LEN)
...
#define ETH_SNAP_BT_SIG_OUI				      \
	{						      \
		ETH_SNAP_BT_SIG_OUI_0, ETH_SNAP_BT_SIG_OUI_1, \
		ETH_SNAP_BT_SIG_OUI_2			      \
	}
...
#define WLAN_MAC_HEADER_A4_QOS_HTC_LEN \
	36 /* Address 4, QoS Control and HTC included */
...
#define DEFAULT_LISTEN_INTERVAL_BY_DTIM_PERIOD \
	2 /* In unit of AP's DTIM interval, */
...
#define RATE_BASIC_BIT \
	BIT(7) /* mask bit for the rate belonging to the BSSBasicRateSet */
...
#define CWMIN_ERP_0 \
	31 /* 802.11g aCWmin(0) - for only have 1/2/5/11Mbps Rates */
...
#define MASK_FRAME_TYPE						  (MASK_FC_TYPE	\
								       |	\
								       MASK_FC_SUBTYPE)
#define MASK_TO_DS_FROM_DS					  (MASK_FC_TO_DS \
								       |	 \
								       MASK_FC_FROM_DS)
...
#define MAC_FRAME_ASSOC_REQ					  ( \
		MAC_FRAME_TYPE_MGT | 0x0000)
#define MAC_FRAME_ASSOC_RSP					  ( \
		MAC_FRAME_TYPE_MGT | 0x0010)
#define MAC_FRAME_REASSOC_REQ					  ( \
		MAC_FRAME_TYPE_MGT | 0x0020)
#define MAC_FRAME_REASSOC_RSP					  ( \
		MAC_FRAME_TYPE_MGT | 0x0030)
#define MAC_FRAME_PROBE_REQ					  ( \
		MAC_FRAME_TYPE_MGT | 0x0040)
#define MAC_FRAME_PROBE_RSP					  ( \
		MAC_FRAME_TYPE_MGT | 0x0050)
#define MAC_FRAME_BEACON					  ( \
		MAC_FRAME_TYPE_MGT | 0x0080)
#define MAC_FRAME_ATIM						  ( \
		MAC_FRAME_TYPE_MGT | 0x0090)
#define MAC_FRAME_DISASSOC					  ( \
		MAC_FRAME_TYPE_MGT | 0x00A0)
#define MAC_FRAME_AUTH						  ( \
		MAC_FRAME_TYPE_MGT | 0x00B0)
#define MAC_FRAME_DEAUTH					  ( \
		MAC_FRAME_TYPE_MGT | 0x00C0)
#define MAC_FRAME_ACTION					  ( \
		MAC_FRAME_TYPE_MGT | 0x00D0)
#define MAC_FRAME_ACTION_NO_ACK					  ( \
		MAC_FRAME_TYPE_MGT | 0x00E0)

#define MAC_FRAME_CONTRL_WRAPPER				  ( \
		MAC_FRAME_TYPE_CTRL | 0x0070)
#define MAC_FRAME_BLOCK_ACK_REQ					  ( \
		MAC_FRAME_TYPE_CTRL | 0x0080)
#define MAC_FRAME_BLOCK_ACK					  ( \
		MAC_FRAME_TYPE_CTRL | 0x0090)
#define MAC_FRAME_PS_POLL					  ( \
		MAC_FRAME_TYPE_CTRL | 0x00A0)
#define MAC_FRAME_RTS						  ( \
		MAC_FRAME_TYPE_CTRL | 0x00B0)
#define MAC_FRAME_CTS						  ( \
		MAC_FRAME_TYPE_CTRL | 0x00C0)
#define MAC_FRAME_ACK						  ( \
		MAC_FRAME_TYPE_CTRL | 0x00D0)
#define MAC_FRAME_CF_END					  ( \
		MAC_FRAME_TYPE_CTRL | 0x00E0)
#define MAC_FRAME_CF_END_CF_ACK					  ( \
		MAC_FRAME_TYPE_CTRL | 0x00F0)

#define MAC_FRAME_DATA						  ( \
		MAC_FRAME_TYPE_DATA | 0x0000)
#define MAC_FRAME_DATA_CF_ACK					  ( \
		MAC_FRAME_TYPE_DATA | 0x0010)
#define MAC_FRAME_DATA_CF_POLL					  ( \
		MAC_FRAME_TYPE_DATA | 0x0020)
#define MAC_FRAME_DATA_CF_ACK_CF_POLL				  ( \
		MAC_FRAME_TYPE_DATA | 0x0030)
#define MAC_FRAME_NULL						  ( \
		MAC_FRAME_TYPE_DATA | 0x0040)
#define MAC_FRAME_CF_ACK					  ( \
		MAC_FRAME_TYPE_DATA | 0x0050)
#define MAC_FRAME_CF_POLL					  ( \
		MAC_FRAME_TYPE_DATA | 0x0060)
#define MAC_FRAME_CF_ACK_CF_POLL				  ( \
		MAC_FRAME_TYPE_DATA | 0x0070)
#define MAC_FRAME_QOS_DATA					  ( \
		MAC_FRAME_TYPE_DATA | 0x0080)
#define MAC_FRAME_QOS_DATA_CF_ACK				  ( \
		MAC_FRAME_TYPE_DATA | 0x0090)
#define MAC_FRAME_QOS_DATA_CF_POLL				  ( \
		MAC_FRAME_TYPE_DATA | 0x00A0)
#define MAC_FRAME_QOS_DATA_CF_ACK_CF_POLL			  ( \
		MAC_FRAME_TYPE_DATA | 0x00B0)
#define MAC_FRAME_QOS_NULL					  ( \
		MAC_FRAME_TYPE_DATA | 0x00C0)
#define MAC_FRAME_QOS_CF_POLL					  ( \
		MAC_FRAME_TYPE_DATA | 0x00E0)
#define MAC_FRAME_QOS_CF_ACK_CF_POLL				  ( \
		MAC_FRAME_TYPE_DATA | 0x00F0)
...
#define CAP_INFO_BSS_TYPE					  (CAP_INFO_ESS	\
								       |	\
								       CAP_INFO_IBSS)
...
#define CAP_CF_STA_NOT_ON_LIST \
								  CAP_INFO_CF_POLL_REQ
/* STA: CF-Pollable, requesting on the CF-Polling list */
#define CAP_CF_STA_ON_LIST \
								  CAP_INFO_CF_POLLABLE
...
#define CAP_CF_AP_DELIVERY_ONLY	\
								  CAP_INFO_CF_POLL_REQ
/* AP: PC at AP for delivery and polling */
#define CAP_CF_AP_DELIVERY_POLLING \
								  CAP_INFO_CF_POLLABLE
...
#define REASON_CODE_DEAUTH_LEAVING_BSS \
	3 /* Deauth because sending STA is leaving BSS */
#define REASON_CODE_DISASSOC_INACTIVITY				  4 /* Disassoc due to inactivity */
#define REASON_CODE_DISASSOC_AP_OVERLOAD \
	5 /* Disassoc because AP is unable to handle all assoc STAs */
...
#define REASON_CODE_DISASSOC_LEAVING_BSS \
	8 /* Disassoc because sending STA is leaving BSS */
#define REASON_CODE_ASSOC_BEFORE_AUTH \
	9 /* STA requesting (re)assoc is not auth with responding STA */
...
#define REASON_CODE_MULTICAST_CIPHER_NOT_VALID \
	18 /* Multicast Cipher is not valid */
#define REASON_CODE_UNICAST_CIPHER_NOT_VALID \
...
#define REASON_CODE_INVALID_RSNE_CAPABILITIES \
	22 /* Invalid RSNE Capabilities \
	    */
#define REASON_CODE_IEEE_802_1X_AUTH_FAILED \
	23 /* IEEE 802.1X Authentication failed */
#define REASON_CODE_CIPHER_REJECT_SEC_POLICY \
	24 /* Cipher suite rejected because of the security policy */
#define REASON_CODE_DISASSOC_UNSPECIFIED_QOS \
...
#define REASON_CODE_PEER_WHILE_LEAVING \
	36 /* QSTA is leaving the QBSS or resetting */
#define REASON_CODE_PEER_REFUSE_DLP \
	37 /* Peer does not want to use this mechanism */
#define REASON_CODE_PEER_SETUP_REQUIRED	\
	38 /* Frames received but a setup is reqired */
#define REASON_CODE_PEER_TIME_OUT				  39 /* Time out */
#define REASON_CODE_PEER_CIPHER_UNSUPPORTED \
	45 /* Peer does not support the requested cipher suite */
#define REASON_CODE_BEACON_TIMEOUT \
	100 /* for beacon timeout, defined by mediatek */
/* 7.3.1.8 AID field */
...
#define STATUS_CODE_CAP_NOT_SUPPORTED \
	10 /* Cannot support all requested cap in the Cap Info field */
/* Reassoc denied due to inability to confirm that assoc exists */
#define STATUS_CODE_REASSOC_DENIED_WITHOUT_ASSOC		  11
#define STATUS_CODE_ASSOC_DENIED_OUTSIDE_STANDARD \
	12 /* Assoc denied due to reason outside the scope of this std. */
...
#define STATUS_CODE_AUTH_REJECTED_CHAL_FAIL \
	15 /* Auth rejected because of challenge failure */
...
#define STATUS_CODE_ASSOC_DENIED_NO_PBCC \
	20 /* Assoc denied due to requesting STA not supporting PBCC */
/* Assoc denied due to requesting STA not supporting channel agility */
#define STATUS_CODE_ASSOC_DENIED_NO_CH_AGILITY			  21
#define STATUS_CODE_ASSOC_REJECTED_NO_SPEC_MGT \
	22 /* Assoc rejected because Spectrum Mgt capability is required */
...
#define STATUS_CODE_ASSOC_REJECTED_TEMPORARILY \
	30 /*  IEEE 802.11w, Assoc denied due to the SA query */
/* IEEE 802.11w, Assoc denied due to the MFP select policy */
#define STATUS_CODE_ROBUST_MGMT_FRAME_POLICY_VIOLATION		  31
#endif
#define STATUS_CODE_UNSPECIFIED_QOS_FAILURE \
...
#define STATUS_CODE_UNSUPPORTED_RSN_IE_VERSION \
	44 /* Unsupported RSN information element version */
#define STATUS_CODE_INVALID_RSN_IE_CAP \
	45 /* Invalid RSN information element capabilities */
#define STATUS_CODE_CIPHER_SUITE_REJECTED \
...
#define STATUS_CODE_DIRECT_LINK_NOT_ALLOWED \
	48 /* Direct Link is not allowed in the BSS by policy */
#define STATUS_CODE_DESTINATION_STA_NOT_PRESENT	\
	49 /* Destination STA is not present within this QBSS */
#define STATUS_CODE_DESTINATION_STA_NOT_QSTA \
	50 /* Destination STA is not a QSTA */
...
#define STATUS_CODE_CCX_CCKM_REASSOC_FAILURE \
	0xFFF4 /* CCX CCKM reassociation failure */
...
#define CATEGORY_UNPROTECTED_WNM_ACTION	\
...
/* Country IE should contain at least 3-bytes country code string and one
 * subband triplet. */
...
#define ELEM_MAX_LEN_SUPPORTED_CHANNELS				  (MAX_CHN_NUM * \
								       2)
...
#define TS_INFO_TRAFFIC_TYPE_MASK \
	BIT(0) /* WMM: 0 (Asynchronous TS of low-duty cycles) */
...
#define HT_CAP_INFO_RX_STBC_3_SS \
								  HT_CAP_INFO_RX_STBC

#define ELEM_MAX_LEN_VHT_CAP					  (14 -		     \
								       ELEM_HDR_LEN) \
	/* sizeof(IE_VHT_CAP_T)-2 */
/* 8.4.2.161 VHT Operation element */
#define ELEM_MAX_LEN_VHT_OP					  (7 -		     \
								       ELEM_HDR_LEN) \
	/* sizeof(IE_VHT_OP_T)-2 */
...
#define										    \
	VHT_CAP_INFO_COMPRESSED_STEERING_NUMBER_OF_BEAMFORMER_ANTENNAS_2_SUPPOERTED \
	BIT(13)
#define										    \
	VHT_CAP_INFO_COMPRESSED_STEERING_NUMBER_OF_BEAMFORMER_ANTENNAS_3_SUPPOERTED \
	BIT(14)
#define										    \
...
#define AMPDU_PARAM_MAX_AMPDU_LEN_256K				  (BIT(2) | \
								       BIT(0))
...
#define AMPDU_PARAM_MSS_4_US					  (BIT(4) | \
								       BIT(2))
#define AMPDU_PARAM_MSS_8_US					  (BIT(4) | \
								       BIT(3))
...
#define ELEM_MAX_LEN_OBSS_SCAN					  (16 -	\
								       ELEM_HDR_LEN)

/* 7.3.2.60 20/40 BSS Coexistence element */
#define ELEM_MAX_LEN_20_40_BSS_COEXIST				  (3 - \
								       ELEM_HDR_LEN)
...
#define ELEM_MAX_LEN_INTERWORKING				  (11 -	\
								       ELEM_HDR_LEN)

/* 802.11u 7.3.2.93 Advertisement Protocol IE */
#define ELEM_MAX_LEN_ADV_PROTOCOL				  (4 - \
								       ELEM_HDR_LEN)

/* 802.11u 7.3.2.96 Roaming Consortium IE */
#define ELEM_MAX_LEN_ROAMING_CONSORTIUM				  (19 -	\
								       ELEM_HDR_LEN)
...
#define ACTION_ADDTS_RSP_FRAME_LEN \
	(24 + 4 + 63) /* WMM Status Code: 1; WMM TSPEC IE: 63 */
...
#define ACTION_HT_ANT_SEL_INDICES_FB \
...
/* VERSION(2 octets for WPA) / SUBTYPE(1 octet)-VERSION(1 octet) fields for WMM
 * in WFA IE */
...
#define WMM_QC_ACK_POLICY_NOT_ACKNOWLEDGE			  (1 <<	\
		WMM_QC_ACK_POLICY_OFFSET)
...
typedef struct _ETH_FRAME_HEADER_T {
	u8 aucDestAddr[MAC_ADDR_LEN];
	u8 aucSrcAddr[MAC_ADDR_LEN];
	u16 u2TypeLen;
...
typedef struct _WLAN_MAC_HEADER_T {
	u16 u2FrameCtrl;
	u16 u2DurationID;
	u8 aucAddr1[MAC_ADDR_LEN];
	u8 aucAddr2[MAC_ADDR_LEN];
	u8 aucAddr3[MAC_ADDR_LEN];
	u16 u2SeqCtrl;
...
typedef struct _CTRL_PSPOLL_FRAME_T {
	...
	u8 aucTA[MAC_ADDR_LEN]; /* TA */
} __KAL_ATTRIB_PACKED__ CTRL_PSPOLL_FRAME_T, *P_CTRL_PSPOLL_FRAME_T;

/* BAR */
typedef struct _CTRL_BAR_FRAME_T {
	u16 u2FrameCtrl; /* Frame Control */
	u16 u2DurationID; /* Duration */
	u8 aucDestAddr[MAC_ADDR_LEN]; /* RA */
	u8 aucSrcAddr[MAC_ADDR_LEN]; /* TA */
	u16 u2BarControl;
	u8 aucBarInfo[2]; /* Variable size */
...
typedef struct _WLAN_BEACON_FRAME_T {
	...
	u8 aucInfoElem[1]; /* Various IEs, start from SSID */
...
typedef struct _WLAN_DISASSOC_FRAME_T {
	...
	u16 u2ReasonCode; /* Reason code */
	u8 aucInfoElem[1]; /* Various IEs, possible no. */
...
typedef struct _WLAN_ASSOC_REQ_FRAME_T {
	...
	u16 u2CapInfo; /* Capability information */
	u16 u2ListenInterval; /* Listen interval */
	u8 aucInfoElem[1]; /* Information elements, include WPA IE */
...
typedef struct _WLAN_ASSOC_RSP_FRAME_T {
	...
	u16 u2CapInfo; /* Capability information */
	u16 u2StatusCode; /* Status code */
	u16 u2AssocId; /* Association ID */
	u8 aucInfoElem[1]; /* Information elements, such as */
...
typedef struct _WLAN_REASSOC_REQ_FRAME_T {
	...
	u16 u2CapInfo; /* Capability information */
	u16 u2ListenInterval; /* Listen interval */
	u8 aucCurrentAPAddr[MAC_ADDR_LEN]; /* Current AP address */
	u8 aucInfoElem[1]; /* Information elements, include WPA IE */
...
typedef WLAN_ASSOC_RSP_FRAME_T WLAN_REASSOC_RSP_FRAME_T,
	*P_WLAN_REASSOC_RSP_FRAME_T;
...
typedef struct _WLAN_AUTH_FRAME_T {
	...
	u16 u2AuthAlgNum; /* Authentication algorithm number */
	u8 aucAuthData[AUTH_DATA_MAX_LEN];
	u8 aucInfoElem[1]; /* Various IEs for Fast BSS Transition */
...
typedef struct _IE_SUPPORTED_RATE_T {
	u8 ucId;
	u8 ucLength;
	u8 aucSupportedRates[ELEM_MAX_LEN_SUP_RATES];
...
typedef struct _IE_CHALLENGE_TEXT_T {
	u8 ucId;
	u8 ucLength;
	u8 aucChallengeText[ELEM_MAX_LEN_CHALLENGE_TEXT];
...
typedef struct _COUNTRY_INFO_TRIPLET_T {
	u8 ucParam1; /*!< If param1 >= 201, this triplet is referred to as */
...
typedef struct _COUNTRY_INFO_SUBBAND_TRIPLET_T {
	u8 ucFirstChnlNum; /*!< First Channel Number */
	u8 ucNumOfChnl; /*!< Number of Channels */
	s8 cMaxTxPwrLv; /*!< Maximum Transmit Power Level */
} __KAL_ATTRIB_PACKED__ COUNTRY_INFO_SUBBAND_TRIPLET_T,
*P_COUNTRY_INFO_SUBBAND_TRIPLET_T;

typedef struct _COUNTRY_INFO_REGULATORY_TRIPLET_T {
	u8 ucRegExtId; /*!< Regulatory Extension Identifier, should */
	/* be greater than or equal to 201 */
	u8 ucRegClass; /*!< Regulatory Class */
	u8 ucCoverageClass; /*!< Coverage Class, unsigned 1-octet value 0~31 */
	/* , 32~255 reserved */
} __KAL_ATTRIB_PACKED__ COUNTRY_INFO_REGULATORY_TRIPLET_T,
*P_COUNTRY_INFO_REGULATORY_TRIPLET_T;
...
typedef struct _IE_POWER_CONSTRAINT_T {
	u8 ucId;
	u8 ucLength;
	u8 ucLocalPowerConstraint; /* Unit: dBm */
...
typedef struct _IE_POWER_CAP_T {
	u8 ucId;
	u8 ucLength;
	s8 cMinTxPowerCap; /* Unit: dBm */
	s8 cMaxTxPowerCap; /* Unit: dBm */
...
typedef struct _IE_TPC_REPORT_T {
	u8 ucId;
	u8 ucLength;
	s8 cTxPower; /* Unit: dBm */
	s8 cLinkMargin; /* Unit: dB */
...
typedef struct _IE_MEASUREMENT_REQ_T {
	...
	u8 ucToken;
	u8 ucRequestMode;
	u8 ucMeasurementType;
	u8 aucRequestFields[1];
...
typedef struct _IE_NEIGHBOR_REPORT_T {
	u8 ucId; /* Element ID */
	u8 ucLength; /* Length */
	u8 aucBSSID[MAC_ADDR_LEN]; /* OUI */
	u32 u4BSSIDInfo; /* Type */
	u8 ucOperClass; /* Hotspot Configuration */
	u8 ucChnlNumber;
	u8 ucPhyType;
	u8 aucSubElem[0];
...
typedef struct _ACTION_NEIGHBOR_REPORT_FRAME_T {
	...
	u8 ucCategory; /* Category */
	u8 ucAction; /* Action Value */
	u8 ucDialogToken; /* Dialog Token */
	u8 aucInfoElem[1]; /* subelements */
} __KAL_ATTRIB_PACKED__ ACTION_NEIGHBOR_REPORT_FRAME_T,
*P_ACTION_NEIGHBOR_REPORT_FRAME_T;
...
typedef struct _RM_CHNL_LOAD_REQ_T {
	...
	u8 ucChannel;
	u16 u2RandomInterval;
	u16 u2Duration;
	u8 aucSubElements[1];
} __KAL_ATTRIB_PACKED__ RM_CHNL_LOAD_REQ_T, *P_RM_CHNL_LOAD_REQ_T;

typedef RM_CHNL_LOAD_REQ_T RM_NOISE_HISTOGRAM_REQ_T,
	*P_RM_NOISE_HISTOGRAM_REQ_T;
...
typedef struct _RM_LCI_REQ_T {
	u8 ucLocationSubject;
	u8 ucLatitudeResolution;
	u8 ucLongitudeResolution;
	u8 ucAltitudeResolution;
	u8 aucSubElements[1];
...
typedef struct _IE_HT_OP_T {
	...
	u16 u2Info2;
	u16 u2Info3;
	u8 aucBasicMcsSet[16];
} __KAL_ATTRIB_PACKED__ IE_HT_OP_T, *P_IE_HT_OP_T;

/*8.4.2.160.3 VHT Supported MCS Set field*/
typedef struct _VHT_SUPPORTED_MCS_FIELD {
	u16 u2RxMcsMap;
	u16 u2RxHighestSupportedDataRate;
	u16 u2TxMcsMap;
	u16 u2TxHighestSupportedDataRate;
...
typedef struct _IE_VHT_OP_MODE_NOTIFICATION_T {
	...
	u8 ucLength;
	u8 ucOperatingMode;
} __KAL_ATTRIB_PACKED__ IE_VHT_OP_MODE_NOTIFICATION_T,
*P_IE_VHT_OP_MODE_NOTIFICATION_T;

/*8.4.2.22 Secondary Channel Offset element*/
typedef struct _IE_SECONDARY_OFFSET_T {
	u8 ucId;
	u8 ucLength;
	u8 ucSecondaryOffset;
...
typedef struct _RSN_INFO_ELEM_T {
	...
	u8 ucLength;
	u16 u2Version;
	u32 u4GroupKeyCipherSuite;
	u16 u2PairwiseKeyCipherSuiteCount;
	u8 aucPairwiseKeyCipherSuite1[4];
...
typedef struct _IE_INTOLERANT_CHNL_REPORT_T {
	...
	u8 ucRegulatoryClass;
	u8 aucChannelList[1];
} __KAL_ATTRIB_PACKED__ IE_INTOLERANT_CHNL_REPORT_T,
...
typedef struct _IE_OBSS_SCAN_PARAM_T {
	u8 ucId;
	u8 ucLength;
	u16 u2ScanPassiveDwell;
	u16 u2ScanActiveDwell;
	u16 u2TriggerScanInterval;
	u16 u2ScanPassiveTotalPerChnl;
	u16 u2ScanActiveTotalPerChnl;
	u16 u2WidthTransDelayFactor;
...
typedef struct _WLAN_ACTION_FRAME {
	...
	u8 ucCategory; /* Category */
	u8 ucAction; /* Action Value */
	u8 ucActionDetails[1]; /* Action details */
...
typedef struct _ACTION_SM_REQ_FRAME {
	...
	u8 ucAction; /* Action Value */
	u8 ucDialogToken; /* Dialog Token */
	u8 aucInfoElem[1]; /* Information elements  */
...
typedef struct _ACTION_TPC_REPORT_FRAME {
	...
	u8 ucElemId; /* Element ID */
	u8 ucLength; /* Length */
	u8 ucTransPwr; /* Transmit Power */
	u8 ucLinkMargin; /* Link Margin */
...
typedef struct _ACTION_CHANNEL_SWITCH_FRAME {
	...
	u8 ucCategory; /* Category */
	u8 ucAction; /* Action Value */
	u8 aucInfoElem[5]; /* Information elements */
} __KAL_ATTRIB_PACKED__ ACTION_CHANNEL_SWITCH_FRAME,
*P_ACTION_CHANNEL_SWITCH_FRAME;
...
typedef struct _ACTION_ADDTS_RSP_FRAME {
	...
	u8 ucAction; /* Action Value */
	u8 ucDialogToken; /* Dialog Token */
	u8 ucStatusCode; /* WMM Status Code is of one byte */
	u8 aucInfoElem[1]; /* Information elements, such as */
	/* TS Delay, and etc. */
...
typedef struct _ACTION_ADDBA_REQ_FRAME_T {
	...
	u8 aucBAParameterSet[2]; /* BA policy, TID, buffer size */
	u8 aucBATimeoutValue[2];
	u8 aucBAStartSeqCtrl[2]; /* SSN */
...
typedef struct _ACTION_DELBA_FRAME_T {
	...
	u8 ucCategory; /* Category */
	u8 ucAction; /* Action Value */
	u16 u2DelBaParameterSet; /* Bit 11 Initiator, Bits 12-15 TID */
	u16 u2ReasonCode; /* 7.3.1.7 */
...
typedef struct _ACTION_RM_REQ_FRAME {
	...
	u8 ucAction; /* Action Value */
	u8 ucDialogToken; /* Dialog Token */
	u16 u2Repetitions; /* Number of repetitions */
	u8 aucInfoElem[1]; /* Measurement Request elements, such as */
...
typedef struct _ACTION_RM_REPORT_FRAME {
	...
	u8 ucAction; /* Action Value */
	u8 ucDialogToken; /* Dialog Token */
	u8 aucInfoElem[1]; /* Measurement Report elements, such as */
...
} __KAL_ATTRIB_PACKED__ ACTION_20_40_COEXIST_FRAME,
...
typedef struct _ACTION_SA_QUERY_FRAME {
	...
	u8 ucTransId[ACTION_SA_QUERY_TR_ID_LEN]; /* Transaction id */
...
typedef struct _ACTION_NOTIFY_CHNL_WIDTH_FRAME {
	...
	u8 ucCategory; /* Category */
	u8 ucAction; /* Action Value */
	u8 ucChannelWidth; /* Channel Width */
} __KAL_ATTRIB_PACKED__ ACTION_NOTIFY_CHNL_WIDTH_FRAME,
...
typedef struct _ACTION_WNM_TIMING_MEAS_REQ_FRAME {
	...
	u8 ucCategory; /* Category */
	u8 ucAction; /* Action Value */
	u8 ucTrigger; /* Trigger */
} __KAL_ATTRIB_PACKED__ ACTION_WNM_TIMING_MEAS_REQ_FRAME,
...
typedef struct _ACTION_UNPROTECTED_WNM_TIMING_MEAS_FRAME {
	...
	u8 ucMaxToDErr; /* Maximum of ToD Error [10ns] */
	u8 ucMaxToAErr; /* Maximum of ToA Error [10ns] */
...
typedef struct _ACTION_OP_MODE_NOTIFICATION_FRAME {
	...
	u8 ucCategory; /* Category */
	u8 ucAction; /* Action Value */
	u8 ucOperatingMode; /* Operating Mode */
} __KAL_ATTRIB_PACKED__ ACTION_OP_MODE_NOTIFICATION_FRAME,
*P_ACTION_OP_MODE_NOTIFICATION_FRAME;
...
typedef struct _ACTION_SM_POWER_SAVE_FRAME {
	...
	u8 ucCategory; /* Category */
	u8 ucAction; /* Action Value */
	u8 ucSmPowerCtrl; /* SM Power Control (see 8.4.1.22) */
} __KAL_ATTRIB_PACKED__ ACTION_SM_POWER_SAVE_FRAME,
*P_ACTION_SM_POWER_SAVE_FRAME;
...
typedef struct _ACTION_NOTIFY_CHANNEL_WIDTH_FRAME {
	...
	u8 ucCategory; /* Category */
	u8 ucAction; /* Action Value */
	u8 ucChannelWidth; /* Channel Width (see 8.4.1.21) */
} __KAL_ATTRIB_PACKED__ ACTION_NOTIFY_CHANNEL_WIDTH_FRAME,
*P_ACTION_NOTIFY_CHANNEL_WIDTH_FRAME;
...
typedef struct _WAPI_INFO_ELEM_T {
	...
	u8 ucLength;
	u16 u2Version;
	u16 u2AuthKeyMgtSuiteCount;
	u8 aucAuthKeyMgtSuite1[4];
...
typedef struct _SUB_IE_BSS_TERM_DURATION_T {
	...
	u8 aucTermTsf[8];
	u16 u2Duration;
} __KAL_ATTRIB_PACKED__ SUB_IE_BSS_TERM_DURATION_T,
*P_SUB_IE_BSS_TERM_DURATION_T;
...
#define RCPI_TO_dBm(_rcpi)					     \
	((PARAM_RSSI)(((_rcpi) > RCPI_HIGH_BOUND ? RCPI_HIGH_BOUND : \
		       (_rcpi)) >>				     \
		      1) -					     \
...
#define dBm_TO_RCPI(_dbm)						\
	(RCPI)(((((PARAM_RSSI)(_dbm) + NDBM_LOW_BOUND_FOR_RCPI) << 1) >	\
		RCPI_HIGH_BOUND) ?					\
	       RCPI_HIGH_BOUND :					\
	       ((((PARAM_RSSI)(_dbm) + NDBM_LOW_BOUND_FOR_RCPI)		\
		 << 1) < RCPI_LOW_BOUND ?				\
		RCPI_LOW_BOUND :					\
		(((PARAM_RSSI)(_dbm) + NDBM_LOW_BOUND_FOR_RCPI)		\
		 << 1)))
...
#define UNEQUAL_SSID(pucSsid1, ucSsidLen1, pucSsid2, ucSsidLen2)	       \
	((ucSsidLen1 > ELEM_MAX_LEN_SSID) ||				       \
	 (ucSsidLen2 > ELEM_MAX_LEN_SSID) || ((ucSsidLen1) != (ucSsidLen2)) || \
	 kalMemCmp(pucSsid1, pucSsid2, ucSsidLen1))

/* The macro to copy the SSID, the length of pucDestSsid should have at least 32
 * bytes */
...
#define COPY_IE(pucDestIE, pucSrcIE)			    \
	do {						    \
		kalMemCopy((u8 *)pucDestIE, (u8 *)pucSrcIE, \
			   IE_SIZE(pucSrcIE));		    \
	} while (false)
