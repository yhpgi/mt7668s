...
#endif
#else
	if (prGlueInfo == NULL || prGlueInfo->prAdapter == NULL || prGlueInfo->prAdapter->prP2pInfo == NULL) {
...
#endif
{
	...
	do {
		...
		prNewNetDevice = alloc_netdev_mq(
				sizeof(NETDEV_PRIVATE_GLUE_INFO), name, NET_NAME_PREDICTABLE, ether_setup, CFG_MAX_TXQ_NUM);
#else
		...
		*((P_GLUE_INFO_T *)netdev_priv(prNewNetDevice)) = prGlueInfo;
		...
		wlanBindBssIdxToNetInterface(prGlueInfo, prP2pRoleFsmInfo->ucBssIndex, (PVOID)prP2pInfo->aprRoleHandler);
		...
		prSwitchModeMsg = (P_MSG_P2P_SWITCH_OP_MODE_T)cnmMemAlloc(
				prGlueInfo->prAdapter, RAM_TYPE_MSG, sizeof(MSG_P2P_SWITCH_OP_MODE_T));
		...
		prMsgActiveBss = (P_MSG_P2P_ACTIVE_DEV_BSS_T)cnmMemAlloc(
				prGlueInfo->prAdapter, RAM_TYPE_MSG, sizeof(MSG_P2P_ACTIVE_DEV_BSS_T));
...
int mtk_p2p_cfg80211_del_iface(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	...
	wlanBindBssIdxToNetInterface(
			prGlueInfo, prP2pRoleFsmInfo->ucBssIndex, (PVOID)prGlueInfo->prP2PInfo[0]->prDevHandler);
...
int mtk_p2p_cfg80211_set_default_key(
		struct wiphy *wiphy, struct net_device *netdev, u8 key_index, bool unicast, bool multicast)
{
	P_GLUE_INFO_T		prGlueInfo = NULL;
...
int mtk_p2p_cfg80211_scan(struct wiphy *wiphy, struct cfg80211_scan_request *request)
{
	...
	do {
		...
		for (u4Idx = 0; u4Idx < request->n_ssids; u4Idx++) {
			COPY_SSID(prSsidStruct->aucSsid, prSsidStruct->ucSsidLen, request->ssids->ssid, request->ssids->ssid_len);
...
int mtk_p2p_cfg80211_set_txpower(
		struct wiphy *wiphy, struct wireless_dev *wdev, enum nl80211_tx_power_setting type, int mbm)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	...
	do {
		...
		if (chandef) {
			...
			DBGLOG(P2P, INFO, "=============== role(%d) b=%d f=%d w=%d s1=%d s2=%d ============ \n", ucRoleIdx,
					chandef->chan->band, chandef->chan->center_freq, chandef->width, chandef->center_freq1,
		...
		prP2pBcnUpdateMsg = (P_MSG_P2P_BEACON_UPDATE_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG,
		...
		prP2pStartAPMsg =
				(P_MSG_P2P_START_AP_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG, sizeof(MSG_P2P_START_AP_T));
		...
		struct wiphy *wiphy, struct net_device *dev, struct cfg80211_chan_def *chandef, unsigned int cac_time_ms)
{
	...
	do {
		...
		memcpy(prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->chan, chandef->chan, sizeof(struct ieee80211_channel));
		...
		prP2pDfsCacMsg = (P_MSG_P2P_DFS_CAC_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG, sizeof(*prP2pDfsCacMsg));
...
int mtk_p2p_cfg80211_channel_switch(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_csa_settings *params)
{
	...
	do {
		...
		memcpy(prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->chan, params->chandef.chan, sizeof(struct ieee80211_channel));
		...
		if ((params->beacon_csa.head_len != 0) || (params->beacon_csa.tail_len != 0)) {
			u4Len = sizeof(MSG_P2P_BEACON_UPDATE_T) + params->beacon_csa.head_len + params->beacon_csa.tail_len;
			prP2pBcnUpdateMsg = (P_MSG_P2P_BEACON_UPDATE_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG, u4Len);
...
int mtk_p2p_cfg80211_change_beacon(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_beacon_data *info)
{
	...
	do {
		...
		if ((info->head_len != 0) || (info->tail_len != 0)) {
			u4Len = sizeof(MSG_P2P_BEACON_UPDATE_T) + info->head_len + info->tail_len + info->assocresp_ies_len;
		...
		struct wiphy *wiphy, struct wireless_dev *wdev, struct cfg80211_mgmt_tx_params *params, u64 *cookie)
{
	...
	do {
		...
		*cookie = prGlueInfo->u8Cookie++;
		...
		prMgmtFrame =
				cnmMgtPktAlloc(prGlueInfo->prAdapter, (INT_32)(params->len + sizeof(UINT_64) + MAC_TX_RESERVED_FIELD));
...
int mtk_p2p_cfg80211_change_iface(
		IN struct wiphy *wiphy, IN struct net_device *ndev, IN enum nl80211_iftype type, IN struct vif_params *params)
#else
...
void mtk_p2p_cfg80211_mgmt_frame_register(
		IN struct wiphy *wiphy, struct wireless_dev *wdev, IN u16 frame_type, IN bool reg)
{
#if 0
...
int mtk_p2p_cfg80211_testmode_get_best_channel(IN struct wiphy *wiphy, IN void *data, IN int len)
{
	 ...
	 */
	rlmDomainGetChnlList(prGlueInfo->prAdapter, BAND_2G4, TRUE, MAX_2G_BAND_CHN_NUM, &ucNumOfChannel, aucChannelList);
	...
	if (prQueryLteChn == NULL) {
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidQueryLteSafeChannel, prQueryLteChn, sizeof(PARAM_GET_CHN_INFO), TRUE,
				FALSE, TRUE, &u4BufLen);
		if (rStatus != WLAN_STATUS_SUCCESS) {
		...
		u4LteSafeChnBitMask_2G =
				prQueryLteChn->rLteSafeChnList.au4SafeChannelBitmask[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_2G_BASE_1 - 1];
