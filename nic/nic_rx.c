...
P_SW_RFB_T incRxDefragMPDU(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSWRfb, OUT P_QUE_T prReturnedQue)
{
	...
	if (prSWRfb->ucSecMode == CIPHER_SUITE_TKIP || prSWRfb->ucSecMode == CIPHER_SUITE_TKIP_WO_MIC ||
			prSWRfb->ucSecMode == CIPHER_SUITE_CCMP || prSWRfb->ucSecMode == CIPHER_SUITE_CCMP_W_CCX ||
			prSWRfb->ucSecMode == CIPHER_SUITE_CCMP_256 || prSWRfb->ucSecMode == CIPHER_SUITE_GCMP_128 ||
			prSWRfb->ucSecMode == CIPHER_SUITE_GCMP_256) {
		ucSecMode = prSWRfb->ucSecMode;
		if (!qmRxPNtoU64(prSWRfb->prRxStatusGroup1->aucPN, CCMPTSCPNNUM, &u8PN)) {
	...
	if (prFragInfo->pr1stFrag != (SW_RFB_T *)NULL) {
		...
		if (ucFragNo != prFragInfo->ucNextFragNo
			...
			DBGLOG(RX, INFO, "u2SeqNo = %04x, NextFragNo = %02x, FragNo = %02x\n", prFragInfo->u2SeqNo,
	...
	if (fgFirst) {
		...
		SET_EXPIRATION_TIME(prFragInfo->rReceiveLifetimeLimit, TU_TO_SYSTIME(DOT11_RECEIVE_LIFETIME_TU_DEFAULT));

		prFragInfo->pr1stFrag = prSWRfb;

		prFragInfo->pucNextFragStart = (PUINT_8)prSWRfb->pucRecvBuff + HAL_RX_STATUS_GET_RX_BYTE_CNT(prRxStatus);
		...
		DBGLOG(RX, LOUD, "First: SeqCtrl = %04x, SeqNo = %04x, NextFragNo = %02x\n", u2SeqCtrl, prFragInfo->u2SeqNo,
...
BOOLEAN nicRxIsDuplicateFrame(IN OUT P_SW_RFB_T prSwRfb)
{
	...
	if (RXM_IS_QOS_DATA_FRAME(u2FrameCtrl)) { /* WLAN header shall exist when doing duplicate detection */
		if (prSwRfb->ucTid < CFG_RX_MAX_BA_TID_NUM && prSwRfb->prStaRec->aprRxReorderParamRefTbl[prSwRfb->ucTid]) {
...
VOID nicRxProcessPktWithoutReorder(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if (!timerPendingTimer(&prAdapter->rPacketDelaySetupTimer)) {
		if (nicRxSetupRFB(prAdapter, prSwRfb)) {
			DBGLOG(RX, WARN, "Allocate SwRfb packet buf failed, Start ReturnIndicatedRfb Timer (%u)\n",
...
VOID nicRxProcessDataPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	/* non BA session */
		...
		if (!HAL_RX_STATUS_IS_ICV_ERROR(prRxStatus) && HAL_RX_STATUS_IS_TKIP_MIC_ERROR(prRxStatus)) {
			...
			if (HAL_RX_STATUS_GET_PAYLOAD_FORMAT(prRxStatus) == RX_PAYLOAD_FORMAT_FIRST_SUB_AMSDU) {
				DBGLOG(RX, INFO, "LLC_MIS:%d, EthType:0x%x\n", HAL_RX_STATUS_IS_LLC_MIS(prRxStatus), pu2EtherType);
				fgDrop = FALSE;
			}
#endif
	...
	if (HAL_RX_STATUS_IS_CIPHER_MISMATCH(prRxStatus) && (prSwRfb->fgDataFrame == TRUE)) {
		...
		DBGLOG_RATELIMIT(RSN, EVENT, "HAL_RX_STATUS_IS_CIPHER_MISMATCH, htr:%d, HdrLen:%d\n",
				HAL_RX_STATUS_IS_HEADER_TRAN(prRxStatus), HAL_RX_STATUS_GET_HEADER_LEN(prRxStatus));
		...
		if (prSwRfb->u2HeaderLen >= ETH_HLEN && (*pu2EtherType == NTOHS(ETH_P_1X)
			...
			fgDrop = TRUE;
			DBGLOG_RATELIMIT(RSN, EVENT, "Drop plain text during security connection\n");
		}
	}
	...
	if ((prSwRfb->fgIsBC || prSwRfb->fgIsMC) && (prSwRfb->fgDataFrame == TRUE)) {
		UINT_8		 ucBssIndex = secGetBssIdxByWlanIdx(prAdapter, HAL_RX_STATUS_GET_WLAN_IDX(prRxStatus));
		...
		if (prBssInfo && IS_BSS_AIS(prBssInfo)) {
			...
			if (HAL_RX_STATUS_IS_ICV_ERROR(prRxStatus)) {
				...
				if (RX_GET_CNT(&prAdapter->rRxCtrl, RX_BMC_KEY_ERROR_COUNT) == prAdapter->rWifiVar.u4BmcKeyErrorTh)
			...
			if ((fgTriggerBCNTimeout) && (prAdapter->rWifiVar.u4BmcKeyErrorTh)) {
				DBGLOG(QM, EVENT,
...
VOID nicRxProcessEventPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	if ((prEvent->u2PacketLength - EVENT_HDR_WITHOUT_RXD_SIZE) > prSwRfb->u2PacketLen - sizeof(WIFI_EVENT_T)) {
		DBGLOG(NIC, ERROR, "RX EVENT: ID[0x%02X] SEQ[%u], invalid payload LEN[%u] > %u\n", prEvent->ucEID,
				prEvent->ucSeqNum, prEvent->u2PacketLength - EVENT_HDR_WITHOUT_RXD_SIZE,
...
VOID nicRxProcessEventPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
...
#if DBG
	{
		...
		if (authSendDeauthFrame(prAdapter, NULL, NULL, prSwRfb, REASON_CODE_CLASS_3_ERR, (PFN_TX_DONE_HANDLER)NULL) ==
				WLAN_STATUS_SUCCESS) {
	...
	case EVENT_ID_TDLS:
		...
		TdlsexEventHandle(prAdapter->prGlueInfo, (UINT_8 *)prEvent->aucBuffer, (UINT_32)(prEvent->u2PacketLength - 8));
		break;
		...
		if (prEvent->ucS2DIndex == S2D_INDEX_EVENT_N2H) {
			if (!prAdapter->fgN9AssertDumpOngoing) {
				...
				if (prAdapter->fgN9CorDumpFileOpend) {
					if (kalWriteCorDumpFile(prEvent->aucBuffer, prEvent->u2PacketLength, TRUE) != WLAN_STATUS_SUCCESS) {
			...
			while (QUEUE_IS_NOT_EMPTY(prTempRfbList)) {
				...
				case RX_PKT_TYPE_SW_DEFINED:
					...
					/* case HIF_RX_PKT_TYPE_MANAGEMENT: */
						...
						DBGLOG_MEM8(RX, ERROR, (PUINT_8)prSwRfb->pvHeader, prSwRfb->u2PacketLen);
...
/* Sigma */
		...
		if (prAdapter->fgIsP2PRegistered) {
			...
			if (prBssInfo)
				p2pFuncValidateRxActionFrame(prAdapter, prSwRfb, (prBssInfo->ucBssIndex == P2P_DEV_BSS_INDEX),
	...
	case CATEGORY_RM_ACTION:
		...
		case RM_ACTION_REIGHBOR_RESPONSE:
			rlmProcessNeighborReportResponse(prAdapter, prActFrame, prSwRfb->u2PacketLen);
			break;
		}
		break;
#endif
