...
u32 g_arMissTimeout[CFG_STA_REC_NUM][CFG_RX_MAX_BA_TID_NUM];

const u8 aucTid2ACI[TX_DESC_TID_NUM] = {
...
const u8 aucACI2TxQIdx[WMM_AC_INDEX_NUM] = {
...
const u8 *apucACI2Str[WMM_AC_INDEX_NUM] = { "BE", "BK", "VI", "VO" };

u8 arNetwork2TcResource[HW_BSSID_NUM + 1][NET_TC_NUM] = {
	/* HW Queue Set 1 */
	/* AC_BE, AC_BK, AC_VI, AC_VO, MGMT, BMC */
	{ TC1_INDEX, TC0_INDEX, TC2_INDEX, TC3_INDEX, TC4_INDEX,
	  BMC_TC_INDEX }, /* AIS */
	{ TC1_INDEX, TC0_INDEX, TC2_INDEX, TC3_INDEX, TC4_INDEX,
	  BMC_TC_INDEX }, /* P2P/BoW */
	{ TC1_INDEX, TC0_INDEX, TC2_INDEX, TC3_INDEX, TC4_INDEX,
	  BMC_TC_INDEX }, /* P2P/BoW */
	{ TC1_INDEX, TC0_INDEX, TC2_INDEX, TC3_INDEX, TC4_INDEX,
	  BMC_TC_INDEX }, /* P2P/BoW */
	{ TC1_INDEX, TC0_INDEX, TC2_INDEX, TC3_INDEX, TC4_INDEX,
	  BMC_TC_INDEX }, /* P2P_DEV */
...
const u8 arTcRemapTable[HW_WMM_NUM][TC_NUM_PER_WMM] = {
...
const u8 aucWmmAC2TcResourceSet1[WMM_AC_INDEX_NUM] = { TC1_INDEX, TC0_INDEX,
						       TC2_INDEX, TC3_INDEX };

#if NIC_TX_ENABLE_SECOND_HW_QUEUE
const u8 aucWmmAC2TcResourceSet2[WMM_AC_INDEX_NUM] = { TC7_INDEX, TC6_INDEX,
						       TC8_INDEX, TC9_INDEX };
#endif
/*******************************************************************************
...
#define qmHandleRxPackets_AOSP_1						      \
	do {									      \
		...
		if (prCurrSwRfb->fgFragFrame) {					      \
			prCurrSwRfb = incRxDefragMPDU(prAdapter, prCurrSwRfb,	      \
						      prReturnedQue);		      \
			if (prCurrSwRfb) {					      \
				prRxStatus = prCurrSwRfb->prRxStatus;		      \
				DBGLOG(QM, TRACE,				      \
				       "defragmentation RxStatus=%x\n",		      \
				       prRxStatus);				      \
			}							      \
		}								      \
		if (prCurrSwRfb) {						      \
			...
			if (HAL_RX_STATUS_GET_SEC_MODE(prRxStatus) ==		      \
				...
				if (prCurrSwRfb->prStaRec) {			      \
					...
					u8 *pucMicKey = NULL;			      \
					ucBssIndex = prCurrSwRfb->prStaRec	      \
						     ->ucBssIndex;		      \
					...
					if (prBssInfo->eCurrentOPMode ==	      \
						...
						prCurrSwRfb->ucTid =		      \
							(u8)(			      \
								HAL_RX_STATUS_GET_TID \
								(		      \
									prRxStatus)); \
					} else {				      \
						ASSERT(false);			      \
						/* pucMicKey = \
						 * &prCurrSwRfb->prStaRec->aucRxMicKey[0]; \
						 */ \
					}	    \
					/* SW TKIP MIC verify, adopt new \
					 * function call for MIC calculation \
					 */ \
					/* TODO:[6630] Need to Check Header \
					 * Translation Case */					     \
					if (pucMicKey == NULL) {				     \
						DBGLOG(RX,					     \
						       ERROR,					     \
						       "Mark NULL the Packet for TKIP Key Error\n"); \
						fgMicErr = true;				     \
					} else if (						     \
						tkipMicDecapsulateInRxHdrTransMode		     \
						(						     \
							prCurrSwRfb,				     \
							pucMicKey) ==				     \
						false) {					     \
...
#define RX_DIRECT_REORDER_UNLOCK(pad, dbg)				 \
	do {								 \
		...
		DBGLOG(QM, EVENT, "RX_DIRECT_REORDER_UNLOCK %u\n",	 \
		       __LINE__);					 \
		spin_unlock_bh(						 \
			&_glue->rSpinLock[SPIN_LOCK_RX_DIRECT_REORDER]); \
	} while (0)
...
void qmInit(IN P_ADAPTER_T prAdapter, IN u8 isTxResrouceControlEn)
{
	...
	u32 u4TotalGurantedTcResource = 0;
#endif
	...
	prQM->u4ResidualTcResource =
		u4TotalTcResource - u4TotalGurantedTcResource;
	...
	prQM->u4CurrentStaRecIndexToEnqueue = 0;
	{
		...
		/* Irrelevant in case this STA is an AIS AP (see
	...
	prQM->rLastTxPktDumpTime = (u32)kalGetTimeTick();
}
...
void qmTestCases(IN P_ADAPTER_T prAdapter)
{
	...
	if (QM_TEST_STA_REC_DEACTIVATION) {
		/* Note that QM_STA_REC_HARD_CODING shall be set to 1 for this
		 * test */
...
void qmActivateStaRec(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	if (prStaRec->fgIsValid) { /* The STA_REC has been activated */
		...
		qmDeactivateStaRec(prAdapter, prStaRec); /* To flush TX/RX
		                                          * queues and del RX BA
		                                          * agreements */
	}
...
void qmDeactivateStaRec(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	if (HAL_IS_TX_DIRECT(prAdapter)) {
		...
		prFlushedTxPacketList =
			qmFlushStaTxQueues(prAdapter, prStaRec->ucIndex);
	...
	for (i = 0; i < CFG_RX_MAX_BA_TID_NUM; i++) {
		...
		qmDelRxBaEntry(prAdapter, prStaRec->ucIndex, (u8)i, false);
	}
...
P_MSDU_INFO_T qmFlushTxQueues(IN P_ADAPTER_T prAdapter)
{
	...
	for (ucQueArrayIdx = 0; ucQueArrayIdx < NUM_OF_PER_TYPE_TX_QUEUES;
	     ucQueArrayIdx++) {
...
P_MSDU_INFO_T qmFlushStaTxQueues(IN P_ADAPTER_T prAdapter, IN u32 u4StaRecIdx)
{
	...
	for (ucQueArrayIdx = 0; ucQueArrayIdx < NUM_OF_PER_STA_TX_QUEUES;
	     ucQueArrayIdx++) {
	...
	for (i = 0; i < CFG_NUM_OF_RX_BA_AGREEMENTS; i++) {
		if (QUEUE_IS_NOT_EMPTY(&(prQM->arRxBaTable[i].rReOrderQue))) {
			if (!prSwRfbListHead) {
				...
				prSwRfbListHead = (P_SW_RFB_T)QUEUE_GET_HEAD(
					&(prQM->arRxBaTable[i].rReOrderQue));
				prSwRfbListTail = (P_SW_RFB_T)QUEUE_GET_TAIL(
					&(prQM->arRxBaTable[i].rReOrderQue));
			} else {
				/* Concatenate the MSDU_INFO list with the
				 * existing list */
			     ...
			     IN u32 u4Tid)
{
	...
	/* Note: For each queued packet, prCurrSwRfb->eDst equals
			      ...
			      IN P_MSDU_INFO_T prMsduInfo, OUT u8 *pucTC)
{
	...
	u8 fgCheckACMAgain;
	u8 ucTC, ucQueIdx = WMM_AC_BE_INDEX;
	P_BSS_INFO_T prBssInfo;
	u8 aucNextUP[WMM_AC_INDEX_NUM] = {
	...
	if (prMsduInfo->ucUserPriority < 8) {
		...
		/* QM_DBG_CNT_15 */ /* QM_DBG_CNT_16 */
		/* QM_DBG_CNT_17 */ /* QM_DBG_CNT_18 */
		/* QM_DBG_CNT_19 */ /* QM_DBG_CNT_20 */
		/* QM_DBG_CNT_21 */ /* QM_DBG_CNT_22 */
	}
				 ...
				 IN P_MSDU_INFO_T prMsduInfoListHead)
{
	...
	do {
		...
		prBssInfo = GET_BSS_INFO_BY_INDEX(
			prAdapter, prCurrentMsduInfo->ucBssIndex);
		...
		if (!fgDropPacket) {
			...
			case STA_REC_INDEX_BMCAST:
#if (HIF_TX_RSRC_WMM_ENHANCE == 1)
				prTxQue =
					&prQM->arTxQueue[prCurrentMsduInfo
							 ->ucBssIndex];
#else
				prTxQue =
					&prQM->arTxQueue[TX_QUEUE_INDEX_BMCAST];
#endif
				ucTC = arNetwork2TcResource[prCurrentMsduInfo
							    ->ucBssIndex]
				       [NET_TC_BMC_INDEX];
				...
				/* Always set BMC packet retry limit to
				 * unlimited */
			...
			default:
				prTxQue = qmDetermineStaTxQueue(
					prAdapter, prCurrentMsduInfo, &ucTC);
				break; /*default */
			} /* switch (prCurrentMsduInfo->ucStaRecIndex) */
		...
		if (prTxQue != &rNotEnqueuedQue) {
			/* Check and trigger fast TC resource adjustment for
			 * queued packets */
	...
	if (QUEUE_IS_NOT_EMPTY(&rNotEnqueuedQue)) {
		QM_TX_SET_NEXT_MSDU_INFO(
			(P_MSDU_INFO_T)QUEUE_GET_TAIL(&rNotEnqueuedQue), NULL);
		prMsduInfoReleaseList =
			(P_MSDU_INFO_T)QUEUE_GET_HEAD(&rNotEnqueuedQue);
	}
#if QM_ADAPTIVE_TC_RESOURCE_CTRL
			    ...
			    IN P_MSDU_INFO_T prMsduInfo)
{
...
#if CFG_SUPPORT_TDLS
			prTempStaRec = cnmGetTdlsPeerByAddress(
	...
	/* 4 <3> Not BMCAST, No AP --> Compare DA (i.e., to see whether this is
	 * a unicast frame to a client) */
	...
	prMsduInfo->ucStaRecIndex = (u8)prQM->u4CurrentStaRecIndexToEnqueue;
#endif
}
				       ...
				       IN u32 u4TotalQuota)
{
	...
	u32 u4CurStaForwardFrameCount; /* To remember the total forwarded
	                                * packets for a STA */
	u32 u4MaxForwardFrameCountLimit; /* The maximum number of packets a STA
	                                  * can forward */
	u32 u4AvaliableResource; /* The TX resource amount */
	u32 u4MaxResourceLimit;

	u8 fgEndThisRound;
	P_QUE_MGT_T prQM = &prAdapter->rQM;

	u8 *pucPsStaFreeQuota;
#if (HIF_TX_RSRC_WMM_ENHANCE == 1)
	u8 ucWmmIdx, ucFlagHit, ucAcIdx, ucAcIdxVldCnt, ucStaLoopCnt;
	u8 ucDebugTc;
#endif
	...
	if (ucTC == TC3_INDEX && prAdapter->fgIsUcastBurstMode) {
		u32 i;
		u32 u4TxCheckPeer = 0, u4ActivePeer = 0;
		u32 u4CurrTime = 0, u4ArrivalTime = 0;
		u8 fgTxTimeout = false;

		u4CurrTime = (u32)kalGetTimeTick();
		for (i = 0; i < CFG_STA_REC_NUM; i++) {
			...
			if (prStaRec->fgIsInUse) {
				...
				if (QUEUE_IS_NOT_EMPTY(prCurrQueue)) {
					u4TxCheckPeer |= BIT(i); // record
					// active and
					// has pkt peer
					prDequeuedPkt =
						(P_MSDU_INFO_T)QUEUE_GET_HEAD(
							prCurrQueue);
					u4ArrivalTime =
						GLUE_GET_PKT_ARRIVAL_TIME(
							prDequeuedPkt->prPacket);

					/* check timeout */
					if ((u4CurrTime > u4ArrivalTime) &&
					    ((u4CurrTime - u4ArrivalTime) >
					     prAdapter->ucUnicastBurstTimeout))
						...
						fgTxTimeout = true;
						 ...
						 ((u4CurrTime +
						   (0xFFFFFFFF -
						    u4ArrivalTime)) >
						  prAdapter
						  ->ucUnicastBurstTimeout))
						fgTxTimeout = true;
				}
				DBGLOG(TX,
				       ERROR,
				       "[Act:0x%x/TxChk:0x%x] u4ArrivalTime:%d u4CurrTime:%d\n",
				       u4ActivePeer,
				       u4TxCheckPeer,
				       u4ArrivalTime,
				       u4CurrTime);
		...
		if ((u4TxCheckPeer != u4ActivePeer) && !fgTxTimeout) {
			DBGLOG(TX,
			       ERROR,
			       "[UC buffering!]u4TxCheckPeer:0x%x u4ActivePeer:0x%x fgTxTimeout=%d",
			       u4TxCheckPeer,
			       u4ActivePeer,
			       fgTxTimeout);
			return 0;
	...
	while (ucLoop < CFG_STA_REC_NUM) {
		...
		if (QUEUE_IS_NOT_EMPTY(prCurrQueue)) {
			...
			/* 4 <2.2> Update forward frame/page count limit
			 * for this STA */
			...
			if (prStaRec->fgIsInPS) {
				if (prStaRec->fgIsQoS &&
					...
					/* ASSERT(prStaRec->ucFreeQuotaForDelivery
					 * == 0); */
					u4MaxForwardFrameCountLimit =
						prStaRec->
			...
			if (prAdapter->rWifiVar.fgDbDcModeEn) {
				u4MaxResourceLimit = gmGetDequeueQuota(
					prAdapter, prStaRec, prBssInfo,
					u4TotalQuota);
			}
#endif
#endif
			...
			/* Three cases to break: (1) No resource (2) No
			 * packets (3) Fairness */
					 ...
					 IN u32 u4TotalQuota)
{
	...
	u8 fgChangeDeQFunc = true;
	u8 fgGlobalQueFirst = true;
	...
	u4LeftResource =
		pfnDeQFunc[0](prAdapter, prQue, ucTC, u4AvaliableResource,
	...
	u4LeftResource = pfnDeQFunc[1](prAdapter, prQue, ucTC, u4LeftResource,
				       u4MaxResourceLimit);
				      ...
				      IN u32 u4TotalQuota)
{
	...
	while (!QUEUE_IS_EMPTY(prCurrQueue)) {
		...
		if (IS_BSS_ACTIVE(prBssInfo)) {
			if (!prBssInfo->fgIsNetAbsent) {
				prDequeuedPkt->ucWmmQueSet =
					prBssInfo->ucWmmQueSet; /* to record WMM
				                                 * Set */
	...
	if (QUEUE_IS_NOT_EMPTY(prMergeQue)) {
		...
		QM_TX_SET_NEXT_MSDU_INFO(
			(P_MSDU_INFO_T)QUEUE_GET_TAIL(prCurrQueue), NULL);
	}
				 ...
				 IN P_TX_TCQ_STATUS_T prTcqStatus)
{
	...
	for (i = TC0_INDEX; i < TC_NUM; i++) {
		if ((i == TC4_INDEX) || (i == TC5_INDEX) || (i == TC10_INDEX) ||
		    (i == TC15_INDEX)) {
	...
	for (i = TC3_INDEX; i >= TC0_INDEX; i--) {
		...
		if (prAdapter->rWifiVar.u4MaxTxDeQLimit) {
			...
			u4MaxQuotaLimit = (u32)prTcqStatus->au4MaxNumOfPage[i];
		}
		...
		if ((i == ucBmcTcIdx) || (i == arTcRemapTable[1][ucBmcTcIdx]) ||
	...
	if (QUEUE_IS_NOT_EMPTY(&rReturnedQue)) {
		prReturnedPacketListHead =
			(P_MSDU_INFO_T)QUEUE_GET_HEAD(&rReturnedQue);
		QM_TX_SET_NEXT_MSDU_INFO(
			(P_MSDU_INFO_T)QUEUE_GET_TAIL(&rReturnedQue), NULL);
	}
			   ...
			   IN P_TX_TCQ_STATUS_T prTcqStatus)
{
	...
	/* 4 <2> Adjust TcqStatus according to the updated
	 * prQM->au4CurrentTcResource */
	else {
		s32 i4TotalExtraQuota = 0;
		s32 ai4ExtraQuota[QM_ACTIVE_TC_NUM];
		u8 fgResourceRedistributed = true;
		...
		for (i = 0; i < QM_ACTIVE_TC_NUM; i++) {
			ai4ExtraQuota[i] =
				(s32)prTcqStatus->au4MaxNumOfBuffer[i] -
				(s32)prQM->au4CurrentTcResource[i];

			if (ai4ExtraQuota[i] > 0) { /* The resource shall be
		...
		/* Distribute quotas to TCs which need extra resource according
		 * to prQM->au4CurrentTcResource */
#if (HIF_TX_RSRC_WMM_ENHANCE == 1)
		ASSERT(prAdapter);
		...
		/* In case some TC is waiting for TX Done, continue to adjust TC
		 * quotas upon TX Done */
...
void qmCalAveQLen(P_QUE_MGT_T prQM, u8 u4Tc, u32 u4CurrQueLen)
{
	if (prQM->au4AverageQueLen[u4Tc] == 0) {
		...
		prQM->au4AverageQueLen[u4Tc] -= (prQM->au4AverageQueLen[u4Tc] >>
...
#endif
#if (HIF_TX_RSRC_WMM_ENHANCE == 1)
	u32 arStaQNum[TC_NUM] = { 0 }, ucBcmCnt;
	u8 ucWmmSet = 0;
		...
		if (u4Tc < NUM_OF_PER_STA_TX_QUEUES) {
			for (u4StaRecIdx = 0; u4StaRecIdx < CFG_STA_REC_NUM;
				...
				if (prStaRec) {
					...
					/* If the STA is activated, get the
					 * queue length */
					if ((prStaRec->fgIsValid) &&
					    (!prBssInfo->fgIsNetAbsent)) {
						u4CurrQueLen +=
				  ...
				  IN u32 *pu4ShareCount)
{
	...
	u8 ucIdx;
#if (HIF_TX_RSRC_WMM_ENHANCE == 1)
	u32 au4AdjTc[] = { TC3_INDEX, TC2_INDEX, TC1_INDEX, TC0_INDEX };
#else
	u32 au4AdjTc[] = { TC3_INDEX, TC2_INDEX, TC5_INDEX, TC1_INDEX,
			   TC0_INDEX };
#endif
	u32 u4AdjTcSize = (sizeof(au4AdjTc) / sizeof(u32));
	u32 u4ResidualResource = *pu4ResidualResource;
	u32 u4ShareCount = *pu4ShareCount;
#if (HIF_TX_RSRC_WMM_ENHANCE == 1)
	u8 ucHifWmmSet, ucRealTc;
#endif
...
#if (HIF_TX_RSRC_WMM_ENHANCE == 1)
		for (ucHifWmmSet = 0; ucHifWmmSet < HIF_WMM_SET_NUM;
		     ucHifWmmSet++) {
...
/*----------------------------------------------------------------------------*/
	void qmReassignTcResource(IN P_ADAPTER_T prAdapter)
	{
		...
		u32 u4TcIdx;
		s32 ai4TcResDemand[QM_ACTIVE_TC_NUM];
		u32 u4ShareCount = 0;
		u32 u4Share = 0;
		P_QUE_MGT_T prQM = &prAdapter->rQM;
		u32 u4ActiveTcCount = 0;
		u32 u4LastActiveTcIdx = TC3_INDEX;

		/* Note: After the new assignment is obtained, set
		 * prQM->fgTcResourcePostAnnealing to true to start the TC-quota
		 * adjusting procedure, which will be invoked upon every TX Done
		 */
		...
		for (u4TcIdx = 0; u4TcIdx < QM_ACTIVE_TC_NUM; u4TcIdx++) {
			...
			/* Define: extra_demand = que_length + min_reserved_quota -
			 * current_quota */
			ai4TcResDemand[u4TcIdx] =
				((s32)(QM_GET_TX_QUEUE_LEN(prAdapter, u4TcIdx) +
				       prQM->au4MinReservedTcResource[u4TcIdx])
				 -
				 (s32)prQM->au4CurrentTcResource[u4TcIdx]);
			...
			/* If there are queued packets, allocate extra resource for the
			 * TC (for TCP consideration) */
		...
		if (i4TotalResourceDemand <= 0) {
			...
			u4ResidualResource = (u32)(-i4TotalResourceDemand);
			u4Share = (u4ResidualResource / u4ShareCount);

			/* 4 <2.2> Satisfy every TC and share the residual resource
			 * evenly */
			for (u4TcIdx = 0; u4TcIdx < QM_ACTIVE_TC_NUM;
			     u4TcIdx++) {
				/* Skip TC4 (not adjustable) */
				if (u4TcIdx == TC4_INDEX)
					continue;
		...
		/* 4 <3> Case 2: Demand > Total Resource --> Guarantee a minimum amount
		 * of resource for each TC */
		else {
			u4ShareCount = 0;
			u4ResidualResource = prQM->u4ResidualTcResource;

			/* 4 <3.1> Allocated resouce amount  = minimum of (guaranteed,
			 * total demand) */
			for (u4TcIdx = 0; u4TcIdx < QM_ACTIVE_TC_NUM;
			     u4TcIdx++) {
				/* Skip TC4 (not adjustable) */
				...
				/* The demand can be fulfilled with the guaranteed
				 * resource amount */
				if ((prQM->au4CurrentTcResource[u4TcIdx] +
					...
					prQM->au4CurrentTcResource[u4TcIdx] +=
						ai4TcResDemand[u4TcIdx];
					u4ResidualResource +=
						(prQM->au4GuaranteedTcResource[
							 u4TcIdx] -
						 prQM->au4CurrentTcResource[
							 u4TcIdx]);
					ai4TcResDemand[u4TcIdx] = 0;
				}
				/* The demand can not be fulfilled with the guaranteed
				 * resource amount */
				else {
					ai4TcResDemand[u4TcIdx] -=
						(prQM->au4GuaranteedTcResource[
		...
		DBGLOG(QM,
		       ...
		       prQM->au4CurrentTcResource[7],
		       prQM->au4CurrentTcResource[8],
		       prQM->au4CurrentTcResource[9],
		...
		DBGLOG(QM,
		       ...
		       prQM->au4CurrentTcResource[12],
		       prQM->au4CurrentTcResource[13],
		       prQM->au4CurrentTcResource[14],
		       prQM->au4CurrentTcResource[15],
		       prQM->au4CurrentTcResource[16]);
#endif
#endif
	}
...
/*----------------------------------------------------------------------------*/
	void qmDoAdaptiveTcResourceCtrl(IN P_ADAPTER_T prAdapter)
	{
		...
		do {
			...
			for (u4Tc = 0; u4Tc < QM_ACTIVE_TC_NUM; u4Tc++) {
				if (QM_GET_TX_QUEUE_LEN(prAdapter,
							u4Tc) >= 100) {
					DBGLOG(QM,
					       LOUD,
	...
	void qmCheckForFastTcResourceCtrl(IN P_ADAPTER_T prAdapter, IN u8 ucTc)
	{
		...
		/* Trigger TC resource adjustment if there is a requirement coming for a
		 * empty TC */
			     ...
			     IN P_SW_RFB_T prSwRfbListHead)
{
	...
	do {
		...
		/* prHifRxHdr = prCurrSwRfb->prHifRxHdr; // TODO: (Tehuang) Use
		 * macro to obtain the pointer */
		prRxStatus = prCurrSwRfb->prRxStatus;
		...
		if (prAdapter->rRxCtrl.u4RxPktsDumpTypeMask &
			...
			DBGLOG_MEM8(SW4, TRACE, (u8 *)prCurrSwRfb->pvHeader,
		...
		if (HAL_RX_STATUS_GET_HEADER_TRAN(prRxStatus) == true) {
			...
			if (prCurrSwRfb->prStaRec == NULL) {
				...
				if (prCurrSwRfb->prRxStatusGroup4 == NULL) {
					DBGLOG_MEM8(
						SW4, TRACE,
						(u8 *)prCurrSwRfb->prRxStatus,
						prCurrSwRfb->prRxStatus
						->u2RxByteCount);
					ASSERT(0);
				}
			...
			if (prAdapter->rRxCtrl.rFreeSwRfbList.u4NumElem >
				...
				prBssInfo = GET_BSS_INFO_BY_INDEX(prAdapter,
								  ucBssIndex);
				/* DBGLOG_MEM8(QM, TRACE,prCurrSwRfb->pvHeader,
				 * 16); */
				/*  */

				/* if ((OP_MODE_ACCESS_POINT !=
				...
				if (prBssInfo->eCurrentOPMode ==
					...
					} else if (secLookupStaRecIndexFromTA(
							   prAdapter,
							   pucEthDestAddr) !=
						   STA_REC_INDEX_NOT_FOUND) {
						prCurrSwRfb->eDst =
							RX_PKT_DESTINATION_FORWARD;
						/* TODO : need to check the dst
						 * mac is valid */
						/* If src mac is invalid, the
						 * packet will be freed in fw */
					}
				}
			} else {
			...
			u16 u2AisFrameCtrl = 0;
			P_WLAN_MAC_HEADER_T prWlanHeader = NULL;
			P_BSS_INFO_T prAisBssInfo = NULL;
			u8 fgIsProtected = false;

			prAisBssInfo = prAdapter->prAisBssInfo;

			prWlanHeader =
				(P_WLAN_MAC_HEADER_T)prCurrSwRfb->pvHeader;
			u2AisFrameCtrl = prWlanHeader->u2FrameCtrl;
			fgIsProtected =
				(u2AisFrameCtrl & MASK_FC_PROTECTED_FRAME) ?
				true :
				false;
			...
			if (prAisBssInfo) {
				if ((prAisBssInfo->prStaRecOfAP) &&
				    ...
				    EQUAL_MAC_ADDR(prWlanHeader->aucAddr2,
		...
		if (prCurrSwRfb->prStaRec) {
			...
			prBssInfoRly =
				GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndexRly);
			...
			if (fgIsBMC && prBssInfoRly &&
			    (IS_BSS_AIS(prBssInfoRly) ||
			     IS_BSS_P2P(prBssInfoRly)) &&
		...
		if (prCurrSwRfb->fgReorderBuffer && !fgIsBMC && fgIsHTran) {
			...
			/* Check Class Error */
			if (secCheckClassError(prAdapter, prCurrSwRfb,
				...
				if (prReorderQueParm &&
				    prReorderQueParm->fgIsValid && !fgIsBMC) {
					qmProcessPktWithReordering(
						prAdapter, prCurrSwRfb,
						prReturnedQue);
				} else {
			...
			case MAC_FRAME_BLOCK_ACK_REQ:
				qmProcessBarFrame(prAdapter, prCurrSwRfb,
						  prReturnedQue);
				RX_INC_CNT(&prAdapter->rRxCtrl,
					   RX_BAR_DROP_COUNT);
				break;
	...
	if (QUEUE_IS_NOT_EMPTY(prReturnedQue)) {
		QM_TX_SET_NEXT_MSDU_INFO(
			(P_SW_RFB_T)QUEUE_GET_TAIL(prReturnedQue), NULL);
	}
		       ...
		       OUT P_QUE_T prReturnedQue)
{
	...
	prSwRfb->ucTid = (*((u16 *)((u8 *)prBarCtrlFrame +
				    CTRL_BAR_BAR_CONTROL_OFFSET))) >>
			 BAR_CONTROL_TID_INFO_OFFSET;
	prSwRfb->u2SSN = (*((u16 *)((u8 *)prBarCtrlFrame +
	...
	/* Incorrect STA_REC index */
	prSwRfb->ucStaRecIdx = secLookupStaRecIndexFromTA(
		prAdapter, prBarCtrlFrame->aucSrcAddr);
	if (prSwRfb->ucStaRecIdx >= CFG_STA_REC_NUM) {
		DBGLOG(QM,
		       WARN,
	...
	u4SSN = (u32)(prSwRfb->u2SSN);
	u4WinStart = (u32)(prReorderQueParm->u2WinStart);
	u4WinEnd = (u32)(prReorderQueParm->u2WinEnd);

	if (qmCompareSnIsLessThan(u4WinStart, u4SSN)) {
		prReorderQueParm->u2WinStart = (u16)u4SSN;
		prReorderQueParm->u2WinEnd =
			((prReorderQueParm->u2WinStart) +
			 (prReorderQueParm->u2WinSize) - 1) %
			MAX_SEQ_NO_COUNT;
			...
			OUT P_QUE_T prReturnedQue)
{
	...
	(((u4WinStart <= u4SeqNo) && (u4SeqNo <= u4WinEnd))
	 ...
	 || ((u4SeqNo <= u4WinEnd) && (u4WinEnd < u4WinStart))) {
		...
		if (prReorderQueParm->fgIsWaitingForPktWithSsn) {
			...
			prReorderQueParm->u2WinStart = (u16)u4SeqNo;
			prReorderQueParm->u2WinEnd =
				((prReorderQueParm->u2WinStart) +
				 (prReorderQueParm->u2WinSize) - 1) %
				MAX_SEQ_NO_COUNT;
	...
	(((u4WinStart < u4WinEnd) && (u4WinEnd < u4SeqNo) &&
	 ...
	 || ((u4WinEnd < u4SeqNo) && (u4SeqNo < u4WinStart) &&
	     ((u4SeqNo + MAX_SEQ_NO_COUNT) <
	      (u4WinStart + HALF_SEQ_NO_COUNT)))) {
		u16 u2Delta, u2BeforeWinEnd;
		u32 u4BeforeCount, u4MissingCount;
		...
		prReorderQueParm->u2WinEnd = (u16)u4SeqNo;
		...
		if (prReorderQueParm->u2WinEnd >= u2BeforeWinEnd) {
			...
			u2Delta = MAX_SEQ_NO_COUNT -
				  (u2BeforeWinEnd - prReorderQueParm->u2WinEnd);
		}

		u4MissingCount =
			u2Delta - (u4BeforeCount -
				  ...
				  OUT P_QUE_T prReturnedQue)
{
	...
	P_HW_MAC_RX_DESC_T prRxStatus;
	u8 u8AmsduSubframeIdx; /* RX reorder for one MSDU in AMSDU issue */
	...
	/* Determine the insert position */
		...
		do {
			...
			if (((prExaminedQueuedSwRfb->u2SSN) ==
				...
				/* if first is duplicated, drop subsequent
			...
			/* Case 3: Insert point not found. Check the next SW_RFB
...
#endif
		/* Update the Reorder Queue Parameters according to the found
		 * insert position */
		if (prExaminedQueuedSwRfb == NULL) {
			...
			((P_QUE_ENTRY_T)prSwRfb)->prNext =
				(P_QUE_ENTRY_T)prExaminedQueuedSwRfb;
			     ...
			     OUT P_QUE_T prReturnedQue)
{
	...
	u32 rCurrentTime, *prMissTimeout;
	P_HW_MAC_RX_DESC_T prRxStatus;
	u8 fgIsAmsduSubframe; /* RX reorder for one MSDU in AMSDU issue */

	prReorderQue = &(prReorderQueParm->rReOrderQue);

	fgMissing = false;
	rCurrentTime = 0;
	prMissTimeout = &g_arMissTimeout[prReorderQueParm->ucStaRecIdx]
	...
	while (true) {
...
#if CFG_SUPPORT_RX_AMSDU
		/* If SN + 1 come and last frame is first or middle, update
		 * winstart */
		...
		if (fgDequeuHead) {
			if (((P_QUE_ENTRY_T)prReorderedSwRfb)->prNext == NULL) {
				...
				prReorderQue->prHead =
					((P_QUE_ENTRY_T)prReorderedSwRfb)
					->prNext;
				(((P_QUE_ENTRY_T)prReorderedSwRfb)->prNext)
				->prPrev = NULL;
			}
			prReorderQue->u4NumElem--;
	...
	prReorderQueParm->u2WinEnd = (((prReorderQueParm->u2WinStart) +
				       (prReorderQueParm->u2WinSize) - 1) %
				      MAX_SEQ_NO_COUNT);
}
			    ...
			    OUT P_QUE_T prReturnedQue)
{
	...
	while (true) {
		...
		/* SN < WinStart, so the head packet shall be indicated (do not
		 * advance the window) */
		else if (qmCompareSnIsLessThan(
				 (u32)(prReorderedSwRfb->u2SSN),
				 (u32)(prReorderQueParm->u2WinStart))) {
			fgDequeuHead = true;
		}
		/* SN > WinStart, break to update WinEnd */
		else {
				  ...
				  IN unsigned long ulParamPtr)
{
	...
	QUEUE_REMOVE_HEAD(&prAdapter->rRxCtrl.rFreeSwRfbList, prSwRfb,
			  P_SW_RFB_T);
	...
	if (prSwRfb) {
		...
		prEvent->ucSeqNum = 0;
		prEvent->u2PacketLength = sizeof(WIFI_EVENT_T) +
					  sizeof(EVENT_CHECK_REORDER_BUBBLE_T);

		prCheckReorderEvent =
			(P_EVENT_CHECK_REORDER_BUBBLE_T)prEvent->aucBuffer;
	...
	if (prReorderQueParm->u2FirstBubbleSn == prReorderQueParm->u2WinStart) {
		...
		if (QUEUE_IS_NOT_EMPTY(prReturnedQue)) {
			...
			while (prSwRfb) {
				...
				prSwRfb = (P_SW_RFB_T)QUEUE_GET_NEXT_ENTRY(
					(P_QUE_ENTRY_T)prSwRfb);
			}
			  ...
			  IN u32 u4EventBufLen)
{
	...
	if (!qmAddRxBaEntry(prAdapter, prStaRec->ucIndex, (u8)u4Tid,
			  ...
			  IN u32 u4EventBufLen)
{
	...
	qmDelRxBaEntry(prAdapter, prStaRec->ucIndex, prEventRxDelBa->ucTid,
		       true);
}
		  ...
		  IN u16 u2WinStart, IN u16 u2WinSize)
{
	...
	if (qmLookupRxBaEntry(prAdapter, ucStaRecIdx, ucTid)) {
		qmDelRxBaEntry(prAdapter, ucStaRecIdx, ucTid,
			       true); /* prQM->ucRxBaCount--
		                       */
	}
	...
	/* If a free-to-use entry is found, configure it and associate it with
	 * the STA_REC */
	...
	if (prRxBaEntry) {
		...
		prRxBaEntry->u2WinEnd =
			((u2WinStart + u2WinSize - 1) % MAX_SEQ_NO_COUNT);
#if CFG_SUPPORT_RX_AMSDU
		/* RX reorder for one MSDU in AMSDU issue */
		...
		/* This shall not happen because FW should keep track of the
		 * usage of RX BA entries */
		    ...
		    IN u8 fgFlushToHost)
{
	...
	if (prRxBaEntry) {
		prFlushedPacketList =
			qmFlushStaRxQueue(prAdapter, ucStaRecIdx, ucTid);
		...
		/* Update RX BA entry state. Note that RX queue flush is not
		 * done here */
			...
			IN u8 *pucIE, IN u16 u2IELength)
{
	...
	/* If the device does not support QoS or if WMM is not supported by the
	 * peer, exit. */
	...
	/* Determine whether QoS is enabled with the association */
		...
		IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
			switch (IE_ID(pucIE)) {
			case ELEM_ID_VENDOR:
				mqmParseAssocReqWmmIe(prAdapter, pucIE,
						      prStaRec);

#if CFG_SUPPORT_MTK_SYNERGY
				if (rlmParseCheckMTKOuiIE(prAdapter, pucIE,
							  &u4Flags))
			...
			case ELEM_ID_HT_CAP:
				/* Some client won't put the WMM IE if client is
				 * 802.11n */
			...
			IN u8 *pucIE, IN u16 u2IELength)
{
	...
	/* Determine whether QoS is enabled with the association */
		...
		IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
			...
			case ELEM_ID_HT_CAP:
				/* Some AP won't put the WMM IE if client is
				 * 802.11n */
				if (IE_LEN(pucIE) == (sizeof(IE_HT_CAP_T) - 2))
					prStaRec->fgIsQoS = true;
				break;
		   ...
		   IN u8 *pucIE, IN u16 u2IELength)
{
	...
	for (i = 0; i < BSS_INFO_NUM; i++) {
		...
		if (IS_BSS_ACTIVE(prBssInfo)) {
			...
			if (fgNewParameter) {
				/* DBGLOG(QM, INFO, ("Update EDCA parameter for
			   ...
			   IN u8 fgForceOverride)
{
	...
	do {
		...
		/* Check the Parameter Set Count to determine whether EDCA
			    ...
			    IN P_BSS_INFO_T prBssInfo)
{
	...
	for (eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++) {
		...
		if (prAcQueParams->ucIsACMSet !=
		    ((prWmmAcParams->ucAciAifsn & WMM_ACIAIFSN_ACM) ? true :
		     false))
			return false;

		/* AIFSN */
		if (prAcQueParams->u2Aifsn !=
		    (prWmmAcParams->ucAciAifsn & WMM_ACIAIFSN_AIFSN))
			return false;
		...
		if (prAcQueParams->u2CWmin !=
		    (BIT(prWmmAcParams->ucEcw & WMM_ECW_WMIN_MASK) - 1))
			return false;
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief This function is used for parsing EDCA parameters specified in the WMM
 * Parameter IE
 *
 * \param[in] prAdapter           Adapter pointer
		       ...
		       OUT P_AC_QUE_PARMS_T prAcQueParams)
{
	...
	prAcQueParams->ucIsACMSet =
		(prAcParam->ucAciAifsn & WMM_ACIAIFSN_ACM) ? true : false;

	prAcQueParams->u2Aifsn = (prAcParam->ucAciAifsn & WMM_ACIAIFSN_AIFSN);

	prAcQueParams->u2CWmax = BIT((prAcParam->ucEcw & WMM_ECW_WMAX_MASK) >>
				     WMM_ECW_WMAX_OFFSET) -
				 1;
 ...
 *
 * \param[in] prAdapter       Adapter pointer
 * \param[in]  prScanResult   The scan result which shall be parsed to obtain
 * needed info \param[out] prStaRec       The obtained info is stored in the
 * STA_REC
 *
 * \return none
 */
			  ...
			  OUT P_STA_RECORD_T prStaRec)
{
	...
	if (prStaRec->ucDesiredPhyTypeSet &
	    (PHY_TYPE_SET_802_11N | PHY_TYPE_SET_802_11AC))
		fgIsHtVht = true;
				 ...
				 P_STA_RECORD_T prStaRec, u8 *pucOutBuf)
{
	...
	return mqmFillWmmInfoIE(pucOutBuf, fgSupportUapsd,
...
void mqmGenerateWmmInfoIE(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo)
{
	...
	u4Length = mqmGenerateWmmInfoIEByStaRec(
		prAdapter, prBssInfo, prStaRec,
		((u8 *)prMsduInfo->prPacket + prMsduInfo->u2FrameLength));
			   ...
			   IN P_MSDU_INFO_T prMsduInfo)
{
	...
	u8 aucACI[] = { WMM_ACI_AC_BE, WMM_ACI_AC_BK, WMM_ACI_AC_VI,
			WMM_ACI_AC_VO };
	...
	prIeWmmParam = (P_IE_WMM_PARAM_T)((u8 *)prMsduInfo->prPacket +
	...
	prIeWmmParam->ucQosInfo =
		(prBssInfo->ucWmmParamSetCount & WMM_QOS_INFO_PARAM_SET_CNT);
	...
	for (eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++) {
		...
		/* DBGLOG(QM, LOUD, ("MQM: eAci=%d, ACM = %d, Aifsn = %d, CWmin
		 * = %d, CWmax = %d, TxopLimit = %d\n", */
		...
		prAcParam->ucEcw = (prBssInfo->aucCWminLog2ForBcast[eAci] &
				    WMM_ECW_WMIN_MASK);
		/* ECW Max */
		prAcParam->ucEcw |= ((prBssInfo->aucCWmaxLog2ForBcast[eAci]
				      << WMM_ECW_WMAX_OFFSET) &
				     WMM_ECW_WMAX_MASK);
				 ...
				 u8 *pOutBuf)
{
	...
	if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.fgTdlsBufferSTASleep)) {
		...
		for (eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++) {
			...
			/* DBGLOG(TDLS, INFO, "eAci = %d, ACM = %d, Aifsn = %d,
	 ...
	 * The STAUT sends a TDLS Setup/Response/Confirm Frame, to STA 2, via
	 * the AP, with all four AC flags set to 1 in QoS Info Field
	 */
	if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.fgTdlsBufferSTASleep)) {
...
u8 isProbeResponse(IN P_MSDU_INFO_T prMgmtTxMsdu)
{
	...
	prWlanHdr =
		(P_WLAN_MAC_HEADER_T)((unsigned long)prMgmtTxMsdu->prPacket +
...
qmGetFrameAction(IN P_ADAPTER_T prAdapter, IN u8 ucBssIndex, IN u8 ucStaRecIdx,
		 IN P_MSDU_INFO_T prMsduInfo,
		 IN ENUM_FRAME_TYPE_IN_CMD_Q_T eFrameType, IN u16 u2FrameLength)
{
	ENUM_FRAME_ACTION_T eFrameAction = FRAME_ACTION_TX_PKT;
	P_BSS_INFO_T prBssInfo;
	P_STA_RECORD_T prStaRec;
	u8 ucTC = nicTxGetFrameResourceType(eFrameType, prMsduInfo);
	u16 u2FreeResource = nicTxGetResource(prAdapter, ucTC);
	u8 ucReqResource;
				     ...
				     IN u32 u4EventBufLen)
{
	...
	if (prEventBssStatus->ucBssIndex > MAX_BSS_INDEX) {
		DBGLOG(QM,
		       ERROR,
		       "qmHandleEventBssAbsencePresence: (ucBssIndex = %d) out-of-bound\n",
		       prEventBssStatus->ucBssIndex);
		return;
	}
	prBssInfo =
		GET_BSS_INFO_BY_INDEX(prAdapter, prEventBssStatus->ucBssIndex);
	...
	/* DBGLOG(QM, TRACE, ("qmHandleEventBssAbsencePresence (ucNetTypeIdx=%d,
	 * fgIsAbsent=%d, FreeQuota=%d)\n", */
	...
	DBGLOG(QM, INFO, "NAF=%d,%d,%d, CH[%d], BN[%d]\n",
	       ...
	       prBssInfo->ucBssFreeQuota, prBssInfo->ucPrimaryChannel,
	       prBssInfo->eDBDCBand);
				  ...
				  IN u32 u4EventBufLen)
{
	...
	prEventStaChangePsMode =
		(P_EVENT_STA_CHANGE_PS_MODE_T)(prEvent->aucBuffer);
	prStaRec = QM_GET_STA_REC_PTR_FROM_INDEX(
		prAdapter, prEventStaChangePsMode->ucStaRecIdx);
	/* ASSERT(prStaRec); */
	...
	if (prStaRec) {
		...
		qmUpdateFreeQuota(prAdapter, prStaRec,
				  prEventStaChangePsMode->ucUpdateMode,
				  prEventStaChangePsMode->ucFreeQuota);

		/* DBGLOG(QM, TRACE, ("qmHandleEventStaChangePsMode
		 * (ucStaRecIdx=%d, fgIsInPs=%d)\n", */
				     ...
				     IN u32 u4EventBufLen)
{
	...
	prEventStaUpdateFreeQuota =
		(P_EVENT_STA_UPDATE_FREE_QUOTA_T)(prEvent->aucBuffer);
	prStaRec = QM_GET_STA_REC_PTR_FROM_INDEX(
		prAdapter, prEventStaUpdateFreeQuota->ucStaRecIdx);
	/* 2013/08/30
...
u32 qmDumpQueueStatus(IN P_ADAPTER_T prAdapter, IN u8 *pucBuf, IN u32 u4Max)
{
	...
	u32 i, u4TotalBufferCount, u4TotalPageCount;
	u32 u4CurBufferCount, u4CurPageCount;
	u32 u4Len = 0;
	u32 u4TotalGlobalQueLen = 0;
