...
const u16 au2RateCCKLong[CCK_RATE_NUM] = {
...
const u16 au2RateCCKShort[CCK_RATE_NUM] = {
	RATE_CCK_1M_LONG, /* RATE_1M_INDEX = 0 */
...
const u16 au2RateOFDM[OFDM_RATE_NUM] = {
...
const u16 au2RateHTMixed[HT_RATE_NUM] = {
...
const u16 au2RateHTGreenField[HT_RATE_NUM] = {
...
const u16 au2RateVHT[VHT_RATE_NUM] = {
...
const EMU_MAC_RATE_INFO_T arMcsRate2PhyRate[] = {
	/* Phy Rate Code,           BW20,  BW20 SGI, BW40, BW40 SGI, BW80, BW80
	 * SGI, BW160, BW160 SGI */
...
u32 nicRateCode2DataRate(IN u16 u2RateCode, IN u8 ucBandwidth, IN u8 ucGI)
{
	...
	if ((u2TxMode == TX_MODE_HT_GF) || (u2TxMode == TX_MODE_HT_MM) ||
		...
		u4PhyRateIn100Kbps =
			(nicGetHwRateByPhyRate(ucPhyRate & BITS(0, 3))) * 5;
	} else {
		ASSERT(false);
	}
	return u4PhyRateIn100Kbps;
	...
	for (i = RATE_1M_SW_INDEX; i <= RATE_54M_SW_INDEX; i++) {
		if (u2RateSet & BIT(i)) {
			...
			u4Status = nicRateIndex2RateCode(
				ucRatePreamble, ucRateIndex, &u2CurRateCode);

			if (u4Status != WLAN_STATUS_SUCCESS)
				continue;

			u4CurPhyRate = nicRateCode2DataRate(
				u2CurRateCode, MAC_BW_20, MAC_GI_NORMAL);
