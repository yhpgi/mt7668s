 ...
 * Traffic Class */
const u8 aucPriorityParam2TC[] = { TC1_INDEX, TC0_INDEX, TC0_INDEX, TC1_INDEX,
...
u8 fgIsBusAccessFailed = false;
u32 g_au4RxMpduCnt[ENUM_BAND_NUM] = { 0 };
u32 g_au4FcsError[ENUM_BAND_NUM] = { 0 };
u32 g_au4RxFifoCnt[ENUM_BAND_NUM] = { 0 };
u32 g_au4AmpduTxSfCnt[ENUM_BAND_NUM] = { 0 };
u32 g_au4AmpduTxAckSfCnt[ENUM_BAND_NUM] = { 0 };
...
PFN_OID_HANDLER_FUNC apfnOidSetHandlerAllowedInRFTest[] = {
    wlanoidRftestSetTestMode, wlanoidRftestSetAbortTestMode,
    wlanoidRftestSetAutoTest, wlanoidSetMcrWrite, wlanoidSetEepromWrite
};

/* OID query handlers allowed in RF test mode */
PFN_OID_HANDLER_FUNC
    apfnOidQueryHandlerAllowedInRFTest[] = { wlanoidRftestQueryAutoTest,
                                             wlanoidQueryMcrRead,
 ...
 * objects. If fails, return NULL. \retval NULL If it fails, NULL is returned.
                             ...
                             IN P_REG_INFO_T prRegInfo)
{
    ...
    do {
        ...
        if (u4Status == WLAN_STATUS_SUCCESS) {
            ...
            pu4WHISR =
                (u32 *)kalMemAlloc(sizeof(u32), PHY_MEM_TYPE);
            if (!pu4WHISR) {
            ...
            HAL_READ_INTR_STATUS(prAdapter, sizeof(u32),
                                 (u8 *)pu4WHISR);
            ...
            if (pu4WHISR) {
                kalMemFree(pu4WHISR, PHY_MEM_TYPE, sizeof(u32));
            }
        ...
        cnmTimerInitTimer(prAdapter, &prAdapter->rOidTimeoutTimer,
        ...
        if ((ENUM_REGISTRY_FIXED_RATE_T)(prRegInfo->u4FixedRate) <
            ...
            prAdapter->rWifiVar.eRateSetting =
                (ENUM_REGISTRY_FIXED_RATE_T)(prRegInfo
                                             ->u4FixedRate);
        } else {
        ...
        kalMemZero(prAdapter->ucRxLBList, sizeof(u8) * 4);
#endif
...
WLAN_STATUS wlanCheckWifiFunc(IN P_ADAPTER_T prAdapter, IN u8 fgRdyChk)
{
    ...
    while (true) {
        if (fgRdyChk) {
            HAL_WIFI_FUNC_READY_CHECK(
                prAdapter, WIFI_FUNC_READY_BITS, &fgResult);
        } else {
            HAL_WIFI_FUNC_OFF_CHECK(prAdapter, WIFI_FUNC_READY_BITS,
                                    &fgResult);
    ...
    while (prQueueEntry) {
        ...
        case COMMAND_TYPE_SECURITY_FRAME:
            ...
            if (prMsduInfo->u2FrameLength > 1408) {
                DBGLOG(INIT, INFO,
                       "Security frame length is too long [%d]\n",
                       prMsduInfo->u2FrameLength);
            }
#endif

            eFrameAction = qmGetFrameAction(
                prAdapter, prMsduInfo->ucBssIndex,
                prMsduInfo->ucStaRecIndex, NULL,
                FRAME_TYPE_802_1X, prCmdInfo->u2InfoBufLen);
            break;
        ...
        case COMMAND_TYPE_MANAGEMENT_FRAME:
            ...
            if (prMsduInfo->u2FrameLength > 1408) {
                DBGLOG(INIT, INFO,
                       "Management frame length is too long [%d]\n",
                       prMsduInfo->u2FrameLength);
            }
#endif
            ...
            eFrameAction = qmGetFrameAction(
                prAdapter, prMsduInfo->ucBssIndex,
                prMsduInfo->ucStaRecIndex, prMsduInfo,
                FRAME_TYPE_MMPDU, prMsduInfo->u2FrameLength);
            break;
 ...
 * immediately. \retval WLAN_STATUS_RESOURCE  : No resource for current command,
 * need to wait for previous frame finishing their transmission. \retval
 * WLAN_STATUS_FAILURE   : Get failure while access HIF or been rejected.
 */
/*----------------------------------------------------------------------------*/
    ...
    while (prQueueEntry) {
        ...
        if (tx_retry_cnt == 0) {
            if ((!prCmdInfo->fgSetQuery) ||
                ...
                QUEUE_INSERT_TAIL(prTempCmdDoneQue,
                                  prQueueEntry);
            }
        }
        ...
        if (tx_retry_cnt < NIC_TX_RESOURCE_POLLING_TIMEOUT) {
            ...
            if (prWifiCmd == NULL) {
                DBGLOG(INIT,
                       ERROR,
                       "CMD cannot send, pucInfoBuffer is NULL\n");
            } else {
                DBGLOG(INIT,
                       ERROR,
        ...
        if (tx_retry_cnt) {
            ...
            if (prWifiCmd == NULL) {
                DBGLOG(INIT, ERROR,
                       "RETRY done, pucInfoBuffer is NULL\n");
            } else {
                DBGLOG(INIT,
                       STATE,
                       "RETRY[%d] TX CMD: ID[0x%02X] SEQ[%u]!\n",
                       tx_retry_cnt,
                       prWifiCmd->ucCID,
                       prWifiCmd->ucSeqNum);
            }
...
void wlanClearTxCommandQueue(IN P_ADAPTER_T prAdapter)
{
    ...
    while (prQueueEntry) {
        ...
        nicTxReleaseResource(
            prAdapter, nicTxGetCmdResourceType(prCmdInfo), 1, true);
...
void wlanClearTxOidCommand(IN P_ADAPTER_T prAdapter)
{
    ...
    while (prQueueEntry) {
        ...
        if (prCmdInfo->fgIsOid) {
            ...
            nicTxReleaseResource(prAdapter,
...
void wlanClearDataQueue(IN P_ADAPTER_T prAdapter)
{
    if (HAL_IS_TX_DIRECT()) {
        ...
        nicTxReleaseMsduResource(
            prAdapter, (P_MSDU_INFO_T)QUEUE_GET_HEAD(prDataPort0));
        nicTxReleaseMsduResource(
            prAdapter, (P_MSDU_INFO_T)QUEUE_GET_HEAD(prDataPort1));

        /* <3> Return sk buffer */
        nicTxReturnMsduInfo(prAdapter,
                            (P_MSDU_INFO_T)QUEUE_GET_HEAD(prDataPort0));
        nicTxReturnMsduInfo(prAdapter,
                            (P_MSDU_INFO_T)QUEUE_GET_HEAD(prDataPort1));
        ...
        for (i = 0; i < TX_PORT_NUM; i++) {
            nicTxReleaseMsduResource(
                prAdapter,
                (P_MSDU_INFO_T)QUEUE_GET_HEAD(prDataPort[i]));
            nicTxReturnMsduInfo(
                prAdapter,
                (P_MSDU_INFO_T)QUEUE_GET_HEAD(prDataPort[i]));
        }
        ...
        if (fgIsNeedHandler) {
            if (prMsduInfo->pfTxDoneHandler != NULL) {
                prMsduInfo->pfTxDoneHandler(
                    prAdapter, prMsduInfo, rTxDoneStatus);
            }
        } else {
 ...
 * information. \param[IN] u4QueryBufferLen Specifies the number of bytes at
 * pvInfoBuf. \param[OUT] pu4QueryInfoLen  Points to the number of bytes it
 * written or is needed.
 *
                     ...
                     OUT u32 *pu4QryInfoLen)
{
    ...
    status = pfnOidQryHandler(prAdapter, pvInfoBuf, u4InfoBufLen,
                              pu4QryInfoLen);
 ...
 * miniport maintains for particular object identifiers, such as changes in
 * multicast addresses.
 *
 ...
 * for the set. \param[IN] u4InfoBufLen  Specifies the number of bytes at
 * prSetBuffer. \param[OUT] pu4SetInfoLen Points to the number of bytes it read
 * or is needed.
 *
 * \retval WLAN_STATUS_xxx Different WLAN_STATUS code returned by different
 * handlers.
                   ...
                   IN u32 u4InfoBufLen, OUT u32 *pu4SetInfoLen)
{
     ...
     * by itself
     */
    status = pfnOidSetHandler(prAdapter, pvInfoBuf, u4InfoBufLen,
                              pu4SetInfoLen);
...
WLAN_STATUS wlanSendDummyCmd(IN P_ADAPTER_T prAdapter, IN u8 fgIsReqTxRsrc)
{
    ...
    prCmdInfo->u2InfoBufLen = (u16)CMD_HDR_SIZE;
    ...
    if (fgIsReqTxRsrc) {
        if (wlanSendCommand(prAdapter, prCmdInfo) !=
            WLAN_STATUS_SUCCESS) {
            DBGLOG(INIT, ERROR,
                                    ...
                                    IN u8 ucPowerMode)
{
    ...
    prCmdInfo = cmdBufAllocateCmdInfo(
        prAdapter, (CMD_HDR_SIZE + sizeof(CMD_NIC_POWER_CTRL)));
    if (!prCmdInfo) {
        DBGLOG(INIT, ERROR, "Allocate CMD_INFO_T ==> FAILED.\n");
        return WLAN_STATUS_FAILURE;
    }
    ...
    prCmdInfo->eCmdType = COMMAND_TYPE_GENERAL_IOCTL;
    prCmdInfo->u2InfoBufLen =
        (u16)(CMD_HDR_SIZE + sizeof(CMD_NIC_POWER_CTRL));
    prCmdInfo->pfCmdDoneHandler = NULL;
    prCmdInfo->pfCmdTimeoutHandler = NULL;
    prCmdInfo->fgIsOid = true;
    ...
    while (1) {
        ...
        if (nicTxAcquireResource(prAdapter, ucTC, true) ==
 ...
 *                             True: wlan stays awake and keeps working in full
 * power state False: wlan may go to sleep and consumes less power.
 *
 * \return WLAN_STATUS_SUCCESS
 * \return WLAN_STATUS_FAILURE
 */
...
WLAN_STATUS wlanKeepFullPwr(IN P_ADAPTER_T prAdapter, IN u8 fgEnable)
{
    ...
    return wlanSendSetQueryCmd(prAdapter, CMD_ID_KEEP_FULL_PWR, true, false,
                               false, NULL, NULL,
                               sizeof(struct CMD_KEEP_FULL_PWR_T),
                               (u8 *)&rCmdKeepFullPwr, NULL, 0);
}
                               ...
                               OUT u32 *pu4Len, OUT u32 *pu4DataMode)
{
    ...
    prFwHead =
        (fw_image_tailer_t *)(pvFwImageMapFile + u4FwImageFileLength -
                              sizeof(fw_image_tailer_t));
    if (ucTotSecNum == 1) {
    ...
    *pu4Len = (prTailer->len + LEN_4_BYTE_CRC);
    if (prTailer->feature_set & DOWNLOAD_CONFIG_ENCRYPTION_MODE) {
        ...
        u4DataMode |= (prTailer->feature_set &
    ...
    *pu4DataMode = u4DataMode;
    ...
    if (ucCurSecNum == 0) {
        ...
        DBGLOG(INIT, INFO, "date[%s] version[%c%c%c%c%c%c%c%c%c%c]\n",
               prTailer->ram_built_date, prTailer->ram_version[0],
               prTailer->ram_version[1], prTailer->ram_version[2],
               prTailer->ram_version[3], prTailer->ram_version[4],
               prTailer->ram_version[5], prTailer->ram_version[6],
               prTailer->ram_version[7], prTailer->ram_version[8],
               prTailer->ram_version[9]);
    }

    /* Backup to FW version info */
    if (eDlIdx == IMG_DL_IDX_N9_FW) {
        kalMemCopy(&prAdapter->rVerInfo.rN9tailer, prTailer,
                   sizeof(tailer_format_t));
    } else {
        kalMemCopy(&prAdapter->rVerInfo.rCR4tailer, prTailer,
                   sizeof(tailer_format_t));
    }
}
#if CFG_SUPPORT_COMPRESSION_FW_OPTION
    ...
    OUT u32 *pu4CRC, OUT u32 *pu4UncompressedLength)
{
    ...
    prFwHead =
        (fw_image_tailer_t_2 *)(pvFwImageMapFile + u4FwImageFileLength -
                                sizeof(fw_image_tailer_t_2));
    if (ucTotSecNum == 1) {
        prTailer = &prFwHead->dlm_info;
    }else{
        prTailer = &prFwHead->ilm_info;
    ...
    *pu4DataMode = u4DataMode;
    ...
    if (ucCurSecNum == 0) {
        DBGLOG(INIT, INFO,
               "%s INFO: chip_info[%u:E%u] feature[0x%02X]\n",
               (eDlIdx == IMG_DL_IDX_N9_FW) ? "N9" : "CR4",
               prTailer->chip_info, prTailer->eco_code,
               prTailer->feature_set);
        DBGLOG(INIT, INFO, "date[%s] version[%c%c%c%c%c%c%c%c%c%c]\n",
               prTailer->ram_built_date, prTailer->ram_version[0],
               prTailer->ram_version[1], prTailer->ram_version[2],
    ...
    /* Backup to FW version info */
    if (eDlIdx == IMG_DL_IDX_N9_FW) {
        kalMemCopy(&prAdapter->rVerInfo.rN9Compressedtailer, prTailer,
                   sizeof(tailer_format_t_2));
        prAdapter->rVerInfo.fgIsN9CompressedFW = true;
    } else {
        kalMemCopy(&prAdapter->rVerInfo.rCR4Compressedtailer, prTailer,
                   sizeof(tailer_format_t_2));
                             ...
                             OUT u32 *pu4Len, OUT u32 *pu4DataMode)
{
    if (eDlIdx == IMG_DL_IDX_PATCH) {
        wlanImageSectionGetPatchInfo(prAdapter, pvFwImageMapFile,
                                     ...
                                     ucCurSecNum, eDlIdx,
                                     pu4StartOffset, pu4Addr, pu4Len,
                                     pu4DataMode);
    } else {
        wlanImageSectionGetFwInfo(prAdapter, pvFwImageMapFile,
                                  u4FwImageFileLength, ucTotSecNum,
                                  ucCurSecNum, eDlIdx, pu4StartOffset,
                                  pu4Addr, pu4Len, pu4DataMode);
    }
}
#if CFG_SUPPORT_COMPRESSION_FW_OPTION
                                       ...
                                       IN ENUM_IMG_DL_IDX_T eDlIdx)
{
    ...
    prCheckInfo = (fw_image_tailer_check *)(pvFwImageMapFile +
                                            u4FwImageFileLength -
                                            sizeof(fw_image_tailer_check));
    DBGLOG(INIT, INFO, "feature_set %d\n", prCheckInfo->feature_set);
    ucCompression =
        (u8)((prCheckInfo->feature_set & COMPRESSION_OPTION_MASK) >>
    ...
    OUT P_INIT_CMD_WIFI_DECOMPRESSION_START prFwImageInFo)
{
    ...
    u32 u4StartOffset = 0;
    u32 u4DataMode = 0;
    u32 u4Addr, u4Len, u4BlockSize, u4CRC, u4UnCompressedLength;
    WLAN_STATUS u4Status = WLAN_STATUS_SUCCESS;
    u8 *pucSecBuf, *pucStartPtr;
    u32 u4offset = 0, u4ChunkSize;
    /* 3a. parse file header for decision of divided firmware download or
     * not */
    for (i = 0; i < ucSectionNumber; ++i) {
        if (wlanImageSectionCheckFwCompressInfo(
                prAdapter, pvFwImageMapFile, u4FwImageFileLength,
                eDlIdx) == true) {
            wlanImageSectionGetCompressFwInfo(
                prAdapter, pvFwImageMapFile,
            ...
            pucStartPtr = (u8 *)pvFwImageMapFile + u4StartOffset;
            while (i4TotalLen) {
                ...
                DBGLOG(INIT, INFO, "u4ChunkSize Length %d!\n",
                       u4ChunkSize);
                if (wlanImageSectionConfig(
                        prAdapter, (u4Addr + u4offset),
                        u4ChunkSize, u4DataMode,
                ...
                for (j = 0; j < u4ChunkSize;
                    ...
                    pucSecBuf = (u8 *)pucStartPtr +
                                u4FileOffset + j;
                    if (wlanImageSectionDownload(
                            prAdapter, u4ImgSecSize,
                            pucSecBuf) !=
                        WLAN_STATUS_SUCCESS) {
                        DBGLOG(INIT,
                               ERROR,
            ...
            *pucIsCompressed = true;
            ...
            wlanImageSectionGetInfo(prAdapter, pvFwImageMapFile,
                                    u4FwImageFileLength,
                                    ucSectionNumber, i, eDlIdx,
                                    &u4StartOffset, &u4Addr, &u4Len,
                                    &u4DataMode);
            pucStartPtr = (u8 *)pvFwImageMapFile + u4StartOffset;
            ...
            for (j = 0; j < u4Len; j += CMD_PKT_SIZE_FOR_IMAGE) {
                ...
                pucSecBuf =
                    (u8 *)pucStartPtr + u4FileOffset + j;
                if (wlanImageSectionDownload(
                        prAdapter, u4ImgSecSize,
                        pucSecBuf) != WLAN_STATUS_SUCCESS) {
                    DBGLOG(INIT,
                           ERROR,
                           "Firmware scatter download failed!\n");
                    u4Status = WLAN_STATUS_FAILURE;
                                          ...
                                          IN ENUM_IMG_DL_IDX_T eDlIdx)
{
    ...
    for (i = 0; i < ucSectionNumber; ++i) {
        wlanImageSectionGetInfo(prAdapter, pvFwImageMapFile,
                                u4FwImageFileLength, ucSectionNumber, i,
                                eDlIdx, &u4StartOffset, &u4Addr, &u4Len,
                                &u4DataMode);
        ...
        if (wlanImageSectionConfig(prAdapter, u4Addr, u4Len, u4DataMode,
                                   eDlIdx) != WLAN_STATUS_SUCCESS) {
            DBGLOG(INIT, ERROR,
                   "Firmware download configuration failed!\n");
        ...
        for (j = 0; j < u4Len; j += CMD_PKT_SIZE_FOR_IMAGE) {
            ...
            pucSecBuf = (u8 *)pucStartPtr + u4FileOffset + j;
                                     ...
                                     OUT u8 *pucSeqNum)
{
    ...
    prCmdInfo = cmdBufAllocateCmdInfo(
        prAdapter, sizeof(INIT_HIF_TX_HEADER_T) +
        sizeof(INIT_CMD_PATCH_SEMA_CONTROL));
                                   ...
                                   IN ENUM_IMG_DL_IDX_T eDlIdx)
{
    ...
    prInitCmdDownloadConfig =
        (P_INIT_CMD_DOWNLOAD_CONFIG)(prInitHifTxHeader->rInitWifiCmd
                                     .aucBuffer);
                                     ...
                                     IN u32 u4ImgSecSize, IN u8 *pucImgSecBuf)
{
    ...
    /* 1. Allocate CMD Info Packet and its Buffer. */
    prCmdInfo = cmdBufAllocateCmdInfo(
        prAdapter, sizeof(INIT_HIF_TX_HEADER_T) + u4ImgSecSize);
    ...
    prCmdInfo->u2InfoBufLen =
        sizeof(INIT_HIF_TX_HEADER_T) + (u16)u4ImgSecSize;
    ...
    /* 3. Setup DOWNLOAD_BUF */
    kalMemCopy(prInitHifTxHeader->rInitWifiCmd.aucBuffer, pucImgSecBuf,
               u4ImgSecSize);
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief This function is called to confirm previously firmware download is
 * done without error
 *
 * @param prAdapter      Pointer to the Adapter structure.
 *
...
WLAN_STATUS wlanImageQueryStatus(IN P_ADAPTER_T prAdapter)
{
    ...
    do {
        if (kalIsCardRemoved(prAdapter->prGlueInfo) == true ||
            fgIsBusAccessFailed == true) {
            u4Status = WLAN_STATUS_FAILURE;
        } else if (nicRxWaitResponse(
                       prAdapter, 0, aucBuffer,
                       sizeof(INIT_HIF_RX_HEADER_T) +
                       sizeof(INIT_EVENT_PENDING_ERROR),
                       &u4RxPktLength) != WLAN_STATUS_SUCCESS) {
            u4Status = WLAN_STATUS_FAILURE;
            ...
            if (prInitHifRxHeader->rInitWifiEvent.ucEID !=
                ...
                prEventPendingError =
                    (P_INIT_EVENT_PENDING_ERROR)(
                        ...
                        rInitWifiEvent
                        .aucBuffer);
                if (prEventPendingError->ucStatus != 0) {  /* 0
                                                            * for
                                                            * download
                                                            * success
                                                            */
                                           ...
                                           IN u8 ucCmdSeqNum)
{
    ...
    u8 ucPortIdx = IMG_DL_STATUS_PORT_IDX;
    ...
    do {
        if (kalIsCardRemoved(prAdapter->prGlueInfo) == true ||
            ...
            if (prInitHifRxHeader->rInitWifiEvent.ucEID !=
                ...
                pr_info("Unexpected EVENT ID, get 0x%0x\n",
                        prInitHifRxHeader->rInitWifiEvent.ucEID);
                u4Status = WLAN_STATUS_FAILURE;
            } else if (prInitHifRxHeader->rInitWifiEvent.ucSeqNum !=
                       ucCmdSeqNum) {
                pr_info("Unexpected SeqNum %d, %d\n", ucCmdSeqNum,
                        prInitHifRxHeader->rInitWifiEvent.ucSeqNum);
                u4Status = WLAN_STATUS_FAILURE;
            } else {
                prEventCmdResult =
                               ...
                               IN u32 u4StartAddress, IN u8 ucPDA)
{
    ...
    prInitCmdWifiStart =
        (P_INIT_CMD_WIFI_START)(prInitHifTxHeader->rInitWifiCmd
                                .aucBuffer);
    ...
    IN u8 ucPDA, IN P_INIT_CMD_WIFI_DECOMPRESSION_START prFwImageInFo)
{
    ...
    prInitCmdWifiStart =
        (P_INIT_CMD_WIFI_DECOMPRESSION_START)(prInitHifTxHeader
                                              ->rInitWifiCmd
                                              .aucBuffer);
    prInitCmdWifiStart->u4Override = 0;
    if (fgEnable) {
...
#if CFG_SUPPORT_COMPRESSION_FW_OPTION
        rDlStatus = wlanImageSectionDownloadStage(
            prAdapter, prFwBuffer, u4FwSize, 2, IMG_DL_IDX_N9_FW,
            &fgIsCompressed, &rFwImageInFo);
        if (fgIsCompressed == true) {
            rCfgStatus = wlanCompressedFWConfigWifiFunc(
                prAdapter, false, 0, PDA_N9, &rFwImageInFo);
        } else {
            rCfgStatus =
                wlanConfigWifiFunc(prAdapter, false, 0, PDA_N9);
        }
#else
        rDlStatus = wlanImageSectionDownloadStage(
            prAdapter, prFwBuffer, u4FwSize, 2, IMG_DL_IDX_N9_FW);
...
#if CFG_SUPPORT_COMPRESSION_FW_OPTION
        rDlStatus = wlanImageSectionDownloadStage(
            prAdapter, prFwBuffer, u4FwSize, CR4_FWDL_SECTION_NUM,
            IMG_DL_IDX_CR4_FW, &fgIsCompressed, &rFwImageInFo);
        prAdapter->fgIsCr4FwDownloaded = true;
        if (fgIsCompressed == true) {
            rCfgStatus = wlanCompressedFWConfigWifiFunc(
                prAdapter, false, 0, PDA_CR4, &rFwImageInFo);
        } else {
            rCfgStatus = wlanConfigWifiFunc(prAdapter, false, 0,
                                            PDA_CR4);
        }
#else
        rDlStatus = wlanImageSectionDownloadStage(prAdapter, prFwBuffer,
                                                  u4FwSize,
                                                  CR4_FWDL_SECTION_NUM,
                                                  IMG_DL_IDX_CR4_FW);
        prAdapter->fgIsCr4FwDownloaded = true;
...
WLAN_STATUS wlanDownloadPatch(IN P_ADAPTER_T prAdapter)
{
    ...
    kalFirmwareImageMapping(prAdapter->prGlueInfo, &prFwBuffer, &u4FwSize,
                            IMG_DL_IDX_PATCH);
    if (prFwBuffer == NULL) {
        DBGLOG(INIT, WARN, "FW[%u] load error!\n", IMG_DL_IDX_PATCH);
        return WLAN_STATUS_FAILURE;
    ...
    if (wlanPatchIsDownloaded(prAdapter)) {
        wlanImageSectionGetInfo(prAdapter, prFwBuffer, u4FwSize, 1, 1,
                                IMG_DL_IDX_PATCH, &u4StartOffset,
    ...
    do {
#if CFG_SUPPORT_COMPRESSION_FW_OPTION
        wlanImageSectionDownloadStage(prAdapter, prFwBuffer, u4FwSize,
                                      1, IMG_DL_IDX_PATCH,
                                      &ucIsCompressed, NULL);
#else
        wlanImageSectionDownloadStage(prAdapter, prFwBuffer, u4FwSize,
...
WLAN_STATUS wlanGetPatchInfo(IN P_ADAPTER_T prAdapter)
{
    ...
    wlanImageSectionGetInfo(prAdapter, prFwBuffer, u4FwSize, 1, 1,
                            IMG_DL_IDX_PATCH, &u4StartOffset, &u4Addr,
                            &u4Len, &u4DataMode);
...
WLAN_STATUS wlanSetChipEcoInfo(IN P_ADAPTER_T prAdapter)
{
    ...
    if (wlanAccessRegister(prAdapter, TOP_HVR, &hw_version, 0, 0) !=
        ...
        /* success */
        nicSetChipHwVer((u8)(GET_HW_VER(hw_version) & 0xFF));
        nicSetChipFactoryVer((u8)((GET_HW_VER(hw_version) >> 8) & 0xF));
        nicSetChipSwVer((u8)GET_FW_VER(sw_version));
                               ...
                               IN u8 ucSetQuery)
{
    ...
    prInitCmdAccessReg =
        (P_INIT_CMD_ACCESS_REG)(prInitHifTxHeader->rInitWifiCmd
                                .aucBuffer);
    ...
    u4Status = wlanAccessRegisterStatus(prAdapter, ucCmdSeqNum, ucSetQuery,
                                        aucBuffer, sizeof(aucBuffer));
    if (ucSetQuery == 0) {
        prInitHifRxHeader = (P_INIT_HIF_RX_HEADER_T)aucBuffer;
        prInitEventAccessReg =
            (P_INIT_EVENT_ACCESS_REG)
                       ...
                       IN PFN_OID_HANDLER_FUNC pfnOidHandler, IN u32 u4Timeout)
{
    ...
    u4NumOfElem =
        sizeof(apfnOidWOTimeoutCheck) / sizeof(PFN_OID_HANDLER_FUNC);
    ...
    cnmTimerStartTimer(prAdapter, &(prAdapter->rOidTimeoutTimer),
                       u4OidTimeout);
...
WLAN_STATUS wlanUpdateNetworkAddress(IN P_ADAPTER_T prAdapter)
{
    ...
    if (kalRetrieveNetworkAddress(prAdapter->prGlueInfo, &rMacAddr) ==
        false ||
...
WLAN_STATUS wlanUpdateBasicConfig(IN P_ADAPTER_T prAdapter)
{
    ...
    prCmdInfo = cmdBufAllocateCmdInfo(
        prAdapter, CMD_HDR_SIZE + sizeof(CMD_BASIC_CONFIG_T));
...
u8 wlanProcessTxFrame(IN P_ADAPTER_T prAdapter, IN P_NATIVE_PACKET prPacket)
{
    ...
    if (kalQoSFrameClassifierAndPacketInfo(prAdapter->prGlueInfo, prPacket,
                                           &rTxPacketInfo)) {
        ...
        if (rTxPacketInfo.u2Flag) {
            if (rTxPacketInfo.u2Flag & BIT(ENUM_PKT_1X)) {
                 ...
                 * prStaRec) && !kalIs24Of4Packet(prPacket))
                            ...
                            IN P_NATIVE_PACKET prPacket)
{
    ...
    u4PacketLen = (u32)GLUE_GET_PKT_FRAME_LEN(prPacket);
    ...
    if (prCmdInfo && prMsduInfo) {
        ...
        prCmdInfo->eCmdType = COMMAND_TYPE_SECURITY_FRAME;
        prCmdInfo->u2InfoBufLen = (u16)u4PacketLen;
                             ...
                             IN u32 u4EventBufLen)
{
    ...
    if (GET_BSS_INFO_BY_INDEX(prAdapter, prMsduInfo->ucBssIndex)
        ->eNetworkType == NETWORK_TYPE_AIS &&
        prAdapter->rWifiVar.rAisSpecificBssInfo.fgCounterMeasure) {
        P_STA_RECORD_T prSta =
            cnmGetStaRecByIndex(prAdapter, prMsduInfo->ucBssIndex);
        ...
        if (prSta) {
            ...
            if (authSendDeauthFrame(
                    ...
                    GET_BSS_INFO_BY_INDEX(
                        prAdapter, prMsduInfo->ucBssIndex),
                    prSta, (P_SW_RFB_T)NULL,
                    REASON_CODE_MIC_FAILURE,
                    (PFN_TX_DONE_HANDLER)NULL
                    /* secFsmEventDeauthTxDone left upper layer
                     * handle the 60 timer */
                    ) != WLAN_STATUS_SUCCESS) {
                ASSERT(false);
            }
...
void wlanClearScanningResult(IN P_ADAPTER_T prAdapter)
{
    ...
    if (kalGetMediaStateIndicated(prAdapter->prGlueInfo) ==
        ...
        for (i = 0; i < prAdapter->rWlanInfo.u4ScanResultNum; i++) {
            if (EQUAL_MAC_ADDR(
                ...
                if (prAdapter->rWlanInfo.arScanResult[i]
                    .u4IELength > 0) {
    ...
    if (fgKeepCurrOne == true) {
        ...
        prAdapter->rWlanInfo.u4ScanIEBufferUsage = ALIGN_4(
            prAdapter->rWlanInfo.arScanResult[0].u4IELength);
    } else {
...
void wlanClearBssInScanningResult(IN P_ADAPTER_T prAdapter, IN u8 *arBSSID)
{
    ...
    while (1) {
        ...
        if (EQUAL_MAC_ADDR(arBSSID, prAdapter->rWlanInfo.arScanResult[i]
                           .arMacAddress)) {
...
void wlanEnableATGO(IN P_ADAPTER_T prAdapter)
{
    ...
    prMsgConnReq = (P_MSG_P2P_CONNECTION_REQUEST_T)cnmMemAlloc(
        prAdapter, RAM_TYPE_MSG, sizeof(MSG_P2P_CONNECTION_REQUEST_T));
    if (!prMsgConnReq) {
        ASSERT(false);
        return;
    }
    ...
    mboxSendMsg(prAdapter, MBOX_ID_0, (P_MSG_HDR_T)prMsgConnReq,
                MSG_SEND_METHOD_BUF);
}
#endif
...
void wlanPrintVersion(IN P_ADAPTER_T prAdapter)
{
    ...
    aucBuf[4] = '\0';
    dev_info(prAdapter->prGlueInfo->prDev,
    ...
    dev_info(prAdapter->prGlueInfo->prDev,
             ...
             prTailer->ram_built_date, prTailer->chip_info,
             prTailer->eco_code + 1);
    ...
    aucDate[16] = '\0';
    dev_info(prAdapter->prGlueInfo->prDev,
             "Patch platform %s version 0x%04X %s", aucBuf,
             prVerInfo->rPatchHeader.u4PatchVersion, aucDate);

    dev_info(prAdapter->prGlueInfo->prDev, "Drv version %u.%u[DEC]\n",
...
WLAN_STATUS wlanQueryNicCapability(IN P_ADAPTER_T prAdapter)
{
    ...
    P_WIFI_CMD_T prWifiCmd;
    u32 u4RxPktLength;
    u8 aucBuffer[sizeof(WIFI_EVENT_T) + sizeof(EVENT_NIC_CAPABILITY_T)];
    ...
    /* 1. Allocate CMD Info Packet and its Buffer */
    prCmdInfo = cmdBufAllocateCmdInfo(
        prAdapter, CMD_HDR_SIZE + sizeof(EVENT_NIC_CAPABILITY_T));
    if (!prCmdInfo) {
        DBGLOG(INIT, ERROR, "Allocate CMD_INFO_T ==> FAILED.\n");
        return WLAN_STATUS_FAILURE;
    }
    ...
    prAdapter->fgIsHw5GBandDisabled =
        (u8)prEventNicCapability->ucHw5GBandDisabled;
    prAdapter->fgIsEepromUsed = (u8)prEventNicCapability->ucEepromUsed;
    prAdapter->fgIsEmbbededMacAddrValid =
        (u8)(!IS_BMCAST_MAC_ADDR(prEventNicCapability->aucMacAddr) &&
    ...
    prAdapter->rWifiVar.ucStaVhtBfee &=
        (!(prEventNicCapability->ucHwNotSupportAC));
    prAdapter->rWifiVar.ucStaVhtMuBfee &=
        (!(prEventNicCapability->ucHwNotSupportAC));
                           ...
                           P_PARAM_MCR_RW_STRUCT_T prMcrRdInfo)
{
    ...
    P_WIFI_CMD_T prWifiCmd;
    u32 u4RxPktLength;
    u8 aucBuffer[sizeof(WIFI_EVENT_T) + sizeof(CMD_ACCESS_REG)];
    P_HW_MAC_RX_DESC_T prRxStatus;
    P_WIFI_EVENT_T prEvent;
    ...
    /* 1. Allocate CMD Info Packet and its Buffer */
    prCmdInfo = cmdBufAllocateCmdInfo(
        prAdapter, CMD_HDR_SIZE + sizeof(CMD_ACCESS_REG));
    ...
    /* compose CMD_BUILD_CONNECTION cmd pkt */
    prCmdInfo->eCmdType = COMMAND_TYPE_GENERAL_IOCTL;
    prCmdInfo->u2InfoBufLen = (u16)(CMD_HDR_SIZE + sizeof(CMD_ACCESS_REG));
    prCmdInfo->pfCmdDoneHandler = NULL;
...
static s32 wlanCal6628EfuseForm(IN P_ADAPTER_T prAdapter, s32 au4Input)
{
    ...
    auTxPwrOffset_Round = (u8)au4TxPwrOffset_Round;
                                       ...
                                       IN P_REG_INFO_T prRegInfo)
{
    ...
    if (prRegInfo->prOldEfuseMapping->uc5GChannelOffsetVaild) {
        ...
        wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_CHANNEL_PWR_OFFSET,
                            true, false, false, NULL, NULL,
                            sizeof(rCmdPowerOffset),
                            (u8 *)&rCmdPowerOffset, NULL, 0);
    ...
    if (prRegInfo->prOldEfuseMapping->uc11AcTxPwrValid) {
        ...
        wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_80211AC_TX_PWR, true,
                            false, false, NULL, NULL,
                            sizeof(CMD_TX_AC_PWR_T), (u8 *)&rCmdAcPwr,
                            NULL, 0);
                                    ...
                                    IN P_REG_INFO_T prRegInfo)
{
    ...
    if (prAdapter->rVerInfo.u2Part1CfgOwnVersion == 0x0001) {
        ...
        if (prRegInfo->ucTxPwrValid != 0) {
            ...
            nicUpdateTxPower(
                prAdapter,
                (P_CMD_TX_PWR_T)(&(prRegInfo->rTxPwr)));
        }
    }
    ...
    if (prRegInfo->prOldEfuseMapping) {
        ...
        if (prRegInfo->prOldEfuseMapping->ucChannelOffsetVaild) {
            ...
            kalMemZero(&rCmdPowerOffset,
                       sizeof(CMD_POWER_OFFSET_T));
            ...
            wlanSendSetQueryCmd(prAdapter,
                                CMD_ID_SET_CHANNEL_PWR_OFFSET, true,
                                false, false, NULL, NULL,
                                sizeof(rCmdPowerOffset),
                                (u8 *)&rCmdPowerOffset, NULL, 0);
            /* dumpMemory8(&rCmdPowerOffset,9); */
...
#else
    wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_PHY_PARAM, true, false, false,
                        NULL, NULL, sizeof(CMD_PHY_PARAM_T),
                        (u8 *)(prRegInfo->aucEFUSE), NULL, 0);
#endif
    /*RSSI path compasation */
    if (prRegInfo->ucRssiPathCompasationUsed) {
        CMD_RSSI_PATH_COMPASATION_T rCmdRssiPathCompasation;
        kalMemZero(&rCmdRssiPathCompasation,
                   sizeof(CMD_RSSI_PATH_COMPASATION_T));
        ...
        wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_PATH_COMPASATION,
                            ...
                            (u8 *)&rCmdRssiPathCompasation, NULL, 0);
    }
#if CFG_SUPPORT_RDD_TEST_MODE
    rRddParam.ucRddTestMode = (u8)prRegInfo->u4RddTestMode;
    rRddParam.ucRddShutCh = (u8)prRegInfo->u4RddShutFreq;
    rRddParam.ucRddStartCh =
        (u8)nicFreq2ChannelNum(prRegInfo->u4RddStartFreq);
    rRddParam.ucRddStopCh =
        (u8)nicFreq2ChannelNum(prRegInfo->u4RddStopFreq);
    rRddParam.ucRddDfs = (u8)prRegInfo->u4RddDfs;
    ...
    prAdapter->rWifiVar.rConnSettings.u2CountryCode =
        (((u16)prRegInfo->au2CountryCode[0]) << 8) |
        (((u16)prRegInfo->au2CountryCode[1]) & BITS(0, 7));
    ...
    wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_NVRAM_SETTINGS, true, false,
                        false, NULL, NULL, sizeof(rCmdNvramSettings),
                        (u8 *)&rCmdNvramSettings, NULL, 0);
        ...
        if (prMsduInfo->fgIs802_1x) {
            ...
            if (prStaRec) {
                if (!prStaRec->fgIsValid) {
                    DBGLOG(INIT,
                           WARN,
                           "%s: Free MSDU of 1X PKT [Len: %d] due to StaRec is still invalid\n",
                           __func__,
                           prSkb->len);
                    prMsduInfo->prPacket = NULL;
                    cnmPktFree(prAdapter, prMsduInfo);
...
u32 wlanGetTxPendingFrameCount(IN P_ADAPTER_T prAdapter)
{
    ...
    u4Num = kalGetTxPendingFrameCount(prAdapter->prGlueInfo) +
            (u32)GLUE_GET_REF_CNT(prTxCtrl->i4PendingFwdFrameCount);
                          ...
                          IN u32 u4QueryBufferLen, OUT u32 *pu4QueryInfoLen)
{
    ...
    do {
        ...
        prQueryBssStatistics =
            (P_PARAM_GET_BSS_STATISTICS)pvQueryBuffer;
...
void wlanDumpBssStatistics(IN P_ADAPTER_T prAdapter, u8 ucBssIdx)
{
    ...
    for (eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++) {
        DBGLOG(SW4, INFO,
               ...
               arLLStats[eAci].u4TxMsdu, arLLStats[eAci].u4RxMsdu,
               arLLStats[eAci].u4TxDropMsdu,
               arLLStats[eAci].u4TxFailMsdu);
    }
}
        ...
        *pu4QueryInfoLen = sizeof(PARAM_GET_STA_STATISTICS);
        ...
        for (eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++) {
            ...
            prQueryStaStatistics->arLinkStatistics[eAci].u4RxMsdu =
                prStaRec->arLinkStatistics[eAci].u4RxMsdu;
            prQueryStaStatistics->arLinkStatistics[eAci]
            .u4TxDropMsdu =
                prStaRec->arLinkStatistics[eAci].u4TxDropMsdu;
        }
        ...
        for (ucIdx = TC0_INDEX; ucIdx <= TC3_INDEX; ucIdx++) {
            ...
            /* Reset */
            prQM->au4QmTcResourceEmptyCounter[prStaRec->ucBssIndex]
            [ucIdx] = 0;
        }
        ...
        if (prAdapter->u4FwCompileFlag0 &
            ...
            rResult = wlanSendSetQueryCmd(
                prAdapter, CMD_ID_GET_STA_STATISTICS, false,
                true, true, nicCmdEventQueryStaStatistics,
                nicOidCmdTimeoutCommon,
                sizeof(CMD_GET_STA_STATISTICS_T),
                (u8 *)&rQueryCmdStaStatistics, pvQueryBuffer,
                u4QueryBufferLen);
...
WLAN_STATUS wlanQueryNicCapabilityV2(IN P_ADAPTER_T prAdapter)
{
    ...
    if (prAdapter->u4FwFeatureFlag0 & FEATURE_FLAG0_NIC_CAPABILITY_V2) {
        ...
        prWifiCmd->u2Length = prCmdInfo->u2InfoBufLen -
                              (u16)OFFSET_OF(WIFI_CMD_T, u2Length);
        ...
        /* get event */
        while (true) {
            if (nicRxWaitResponse(prAdapter, 1, prEventBuff,
                                  CFG_RX_MAX_PKT_SIZE,
                                  &u4RxPktLength) !=
                WLAN_STATUS_SUCCESS) {
                DBGLOG(INIT, WARN,
...
void wlanSetNicResourceParameters(IN P_ADAPTER_T prAdapter)
{
    ...
    for (idx = 0; idx < TC_NUM; idx++) {
        ...
        memset(string, 0, sizeof(string) / sizeof(u8));
        snprintf(string, sizeof(string) / sizeof(u8), "Tc%xPage", idx);

        /* update the final value */
        prWifiVar->au4TcPageCount[idx] = (u32)wlanCfgGetUint32(
            prAdapter, string, prWifiVar->au4TcPageCount[idx]);
    }
    ...
    for (idx = 0; idx < TC_NUM; idx++) {
        ...
        memset(string, 0, sizeof(string) / sizeof(u8));
        snprintf(string, sizeof(string) / sizeof(u8), "Tc%xGrt", idx);

        /* update the final value */
        prQM->au4GuaranteedTcResource[idx] = (u32)wlanCfgGetUint32(
            prAdapter, string, prQM->au4GuaranteedTcResource[idx]);
    }
#endif
#endif
}
 ...
 * @param ucBssIndex                     Index of BSS
 *
 * @return void *                         pointer of network interface structure
 */
/*----------------------------------------------------------------------------*/
...
void wlanInitFeatureOption(IN P_ADAPTER_T prAdapter)
{
    ...
    prWifiVar->ucQoS =
        (u8)wlanCfgGetUint32(prAdapter, "Qos", FEATURE_ENABLED);

    prWifiVar->ucStaHt =
        (u8)wlanCfgGetUint32(prAdapter, "StaHT", FEATURE_ENABLED);
    prWifiVar->ucStaVht =
        (u8)wlanCfgGetUint32(prAdapter, "StaVHT", FEATURE_ENABLED);

    prWifiVar->ucApHt =
        (u8)wlanCfgGetUint32(prAdapter, "ApHT", FEATURE_ENABLED);
    prWifiVar->ucApVht =
        (u8)wlanCfgGetUint32(prAdapter, "ApVHT", FEATURE_ENABLED);

    prWifiVar->ucP2pGoHt =
        (u8)wlanCfgGetUint32(prAdapter, "P2pGoHT", FEATURE_ENABLED);
    prWifiVar->ucP2pGoVht =
        (u8)wlanCfgGetUint32(prAdapter, "P2pGoVHT", FEATURE_ENABLED);

    prWifiVar->ucP2pGcHt =
        (u8)wlanCfgGetUint32(prAdapter, "P2pGcHT", FEATURE_ENABLED);
    prWifiVar->ucP2pGcVht =
        (u8)wlanCfgGetUint32(prAdapter, "P2pGcVHT", FEATURE_ENABLED);

    prWifiVar->ucAmpduRx =
        (u8)wlanCfgGetUint32(prAdapter, "AmpduRx", FEATURE_ENABLED);
    prWifiVar->ucAmpduTx =
        (u8)wlanCfgGetUint32(prAdapter, "AmpduTx", FEATURE_ENABLED);

    prWifiVar->ucAmsduInAmpduRx = (u8)wlanCfgGetUint32(
        prAdapter, "AmsduInAmpduRx", FEATURE_ENABLED);
    prWifiVar->ucAmsduInAmpduTx = (u8)wlanCfgGetUint32(
        prAdapter, "AmsduInAmpduTx", FEATURE_ENABLED);
    prWifiVar->ucHtAmsduInAmpduRx = (u8)wlanCfgGetUint32(
        prAdapter, "HtAmsduInAmpduRx", FEATURE_DISABLED);
    prWifiVar->ucHtAmsduInAmpduTx = (u8)wlanCfgGetUint32(
        prAdapter, "HtAmsduInAmpduTx", FEATURE_DISABLED);
    prWifiVar->ucVhtAmsduInAmpduRx = (u8)wlanCfgGetUint32(
        prAdapter, "VhtAmsduInAmpduRx", FEATURE_ENABLED);
    prWifiVar->ucVhtAmsduInAmpduTx = (u8)wlanCfgGetUint32(
        prAdapter, "VhtAmsduInAmpduTx", FEATURE_ENABLED);

    prWifiVar->ucTspec =
        (u8)wlanCfgGetUint32(prAdapter, "Tspec", FEATURE_DISABLED);

    prWifiVar->ucUapsd =
        (u8)wlanCfgGetUint32(prAdapter, "Uapsd", FEATURE_ENABLED);
    prWifiVar->ucStaUapsd =
        (u8)wlanCfgGetUint32(prAdapter, "StaUapsd", FEATURE_DISABLED);
    prWifiVar->ucApUapsd =
        (u8)wlanCfgGetUint32(prAdapter, "ApUapsd", FEATURE_DISABLED);
    prWifiVar->ucP2pUapsd =
        (u8)wlanCfgGetUint32(prAdapter, "P2pUapsd", FEATURE_ENABLED);
    prWifiVar->u4RegP2pIfAtProbe = (u8)wlanCfgGetUint32(
        prAdapter, "RegP2pIfAtProbe", FEATURE_DISABLED);
    prWifiVar->ucP2pShareMacAddr = (u8)wlanCfgGetUint32(
        prAdapter, "P2pShareMacAddr", FEATURE_DISABLED);

    prWifiVar->ucTxShortGI =
        (u8)wlanCfgGetUint32(prAdapter, "SgiTx", FEATURE_ENABLED);
    prWifiVar->ucRxShortGI =
        (u8)wlanCfgGetUint32(prAdapter, "SgiRx", FEATURE_ENABLED);

    prWifiVar->ucTxLdpc =
        (u8)wlanCfgGetUint32(prAdapter, "LdpcTx", FEATURE_ENABLED);
    prWifiVar->ucRxLdpc =
        (u8)wlanCfgGetUint32(prAdapter, "LdpcRx", FEATURE_ENABLED);

    prWifiVar->ucTxStbc =
        (u8)wlanCfgGetUint32(prAdapter, "StbcTx", FEATURE_ENABLED);
    prWifiVar->ucRxStbc =
        (u8)wlanCfgGetUint32(prAdapter, "StbcRx", FEATURE_ENABLED);
    prWifiVar->ucRxStbcNss =
        (u8)wlanCfgGetUint32(prAdapter, "StbcRxNss", 1);

    prWifiVar->ucTxGf =
        (u8)wlanCfgGetUint32(prAdapter, "GfTx", FEATURE_ENABLED);
    prWifiVar->ucRxGf =
        (u8)wlanCfgGetUint32(prAdapter, "GfRx", FEATURE_ENABLED);

    prWifiVar->ucMCS32 =
        (u8)wlanCfgGetUint32(prAdapter, "MCS32", FEATURE_DISABLED);

    prWifiVar->ucSigTaRts =
        (u8)wlanCfgGetUint32(prAdapter, "SigTaRts", FEATURE_DISABLED);
    prWifiVar->ucDynBwRts =
        (u8)wlanCfgGetUint32(prAdapter, "DynBwRts", FEATURE_DISABLED);
    prWifiVar->ucTxopPsTx =
        (u8)wlanCfgGetUint32(prAdapter, "TxopPsTx", FEATURE_DISABLED);

    prWifiVar->ucStaHtBfee =
        (u8)wlanCfgGetUint32(prAdapter, "StaHTBfee", FEATURE_DISABLED);
    prWifiVar->ucStaVhtBfee =
        (u8)wlanCfgGetUint32(prAdapter, "StaVHTBfee", FEATURE_ENABLED);
    prWifiVar->ucStaVhtMuBfee = (u8)wlanCfgGetUint32(
        prAdapter, "StaVHTMuBfee", FEATURE_ENABLED);
    prWifiVar->ucStaHtBfer =
        (u8)wlanCfgGetUint32(prAdapter, "StaHTBfer", FEATURE_DISABLED);
    prWifiVar->ucStaVhtBfer =
        (u8)wlanCfgGetUint32(prAdapter, "StaVHTBfer", FEATURE_DISABLED);

    /* 0: disabled
     * 1: Tx done event to driver
     * 2: Tx status to FW only
     */
    prWifiVar->ucDataTxDone =
        (u8)wlanCfgGetUint32(prAdapter, "DataTxDone", 0);
    prWifiVar->ucDataTxRateMode = (u8)wlanCfgGetUint32(
        prAdapter, "DataTxRateMode", DATA_RATE_MODE_AUTO);
    prWifiVar->u4DataTxRateCode =
        wlanCfgGetUint32(prAdapter, "DataTxRateCode", 0x0);

    prWifiVar->ucApWpsMode =
        (u8)wlanCfgGetUint32(prAdapter, "ApWpsMode", 0);
    DBGLOG(INIT, INFO, "ucApWpsMode = %u\n", prWifiVar->ucApWpsMode);

    prWifiVar->ucThreadScheduling =
        (u8)wlanCfgGetUint32(prAdapter, "ThreadSched", 0);
    prWifiVar->ucThreadPriority = (u8)wlanCfgGetUint32(
        prAdapter, "ThreadPriority", WLAN_THREAD_TASK_PRIORITY);
    prWifiVar->cThreadNice = (s8)wlanCfgGetInt32(prAdapter, "ThreadNice",
                                                 WLAN_THREAD_TASK_NICE);

    prAdapter->rQM.u4MaxForwardBufferCount = (u32)wlanCfgGetUint32(
        prAdapter, "ApForwardBufferCnt", QM_FWD_PKT_QUE_THRESHOLD);

    /* AP channel setting
     * 0: auto
     */
    prWifiVar->ucApChannel =
        (u8)wlanCfgGetUint32(prAdapter, "ApChannel", 0);
     ...
     * 3: SCB
     */
    prWifiVar->ucApSco = (u8)wlanCfgGetUint32(prAdapter, "ApSco", 0);
    prWifiVar->ucP2pGoSco = (u8)wlanCfgGetUint32(prAdapter, "P2pGoSco", 0);
     ...
     */
    prWifiVar->ucStaBandwidth =
        (u8)wlanCfgGetUint32(prAdapter, "StaBw", MAX_BW_160MHZ);
    prWifiVar->ucSta2gBandwidth =
        (u8)wlanCfgGetUint32(prAdapter, "Sta2gBw", MAX_BW_40MHZ);
    prWifiVar->ucSta5gBandwidth =
        (u8)wlanCfgGetUint32(prAdapter, "Sta5gBw", MAX_BW_80MHZ);
    prWifiVar->ucP2p2gBandwidth =
        (u8)wlanCfgGetUint32(prAdapter, "P2p2gBw", MAX_BW_20MHZ);
    prWifiVar->ucP2p5gBandwidth =
        (u8)wlanCfgGetUint32(prAdapter, "P2p5gBw", MAX_BW_80MHZ);
    prWifiVar->ucApBandwidth =
        (u8)wlanCfgGetUint32(prAdapter, "ApBw", MAX_BW_160MHZ);
    prWifiVar->ucAp2gBandwidth =
        (u8)wlanCfgGetUint32(prAdapter, "Ap2gBw", MAX_BW_40MHZ);
    prWifiVar->ucAp5gBandwidth =
        (u8)wlanCfgGetUint32(prAdapter, "Ap5gBw", MAX_BW_80MHZ);
    prWifiVar->ucApChnlDefFromCfg = (u8)wlanCfgGetUint32(
     ...
     * 2: 11k
     */
    prWifiVar->ucRxMaxMpduLen = (u8)wlanCfgGetUint32(
        prAdapter, "RxMaxMpduLen", VHT_CAP_INFO_MAX_MPDU_LEN_3K);
    /* Max Tx AMSDU in AMPDU length *in BYTES**/
    prWifiVar->u4TxMaxAmsduInAmpduLen =
        wlanCfgGetUint32(prAdapter, "TxMaxAmsduInAmpduLen", 4096);

    prWifiVar->ucStaDisconnectDetectTh =
        (u8)wlanCfgGetUint32(prAdapter, "StaDisconnectDetectTh", 0);
    prWifiVar->ucApDisconnectDetectTh =
        (u8)wlanCfgGetUint32(prAdapter, "ApDisconnectDetectTh", 0);
    prWifiVar->ucP2pDisconnectDetectTh =
        (u8)wlanCfgGetUint32(prAdapter, "P2pDisconnectDetectTh", 0);

    prWifiVar->ucTcRestrict =
        (u8)wlanCfgGetUint32(prAdapter, "TcRestrict", 0xFF);
    /* Max Tx dequeue limit: 0 => auto */
    prWifiVar->u4MaxTxDeQLimit =
        (u32)wlanCfgGetUint32(prAdapter, "MaxTxDeQLimit", 0x0);
    prWifiVar->ucAlwaysResetUsedRes =
        (u32)wlanCfgGetUint32(prAdapter, "AlwaysResetUsedRes", 0x0);

#if CFG_SUPPORT_MTK_SYNERGY
    prWifiVar->ucMtkOui =
        (u8)wlanCfgGetUint32(prAdapter, "MtkOui", FEATURE_ENABLED);
    prWifiVar->u4MtkOuiCap =
        (u32)wlanCfgGetUint32(prAdapter, "MtkOuiCap", 0);
    ...
    prWifiVar->ucCmdRsvResource = (u8)wlanCfgGetUint32(
        prAdapter, "TxCmdRsv", QM_CMD_RESERVED_THRESHOLD);
    prWifiVar->u4MgmtQueueDelayTimeout = (u32)wlanCfgGetUint32(
        prAdapter, "TxMgmtQueTO", QM_MGMT_QUEUED_TIMEOUT);  /* ms */

    /* Performance related */
    prWifiVar->u4HifIstLoopCount =
        (u32)wlanCfgGetUint32(prAdapter, "IstLoop", CFG_IST_LOOP_COUNT);
    prWifiVar->u4Rx2OsLoopCount =
        (u32)wlanCfgGetUint32(prAdapter, "Rx2OsLoop", 4);
    prWifiVar->u4HifTxloopCount =
        (u32)wlanCfgGetUint32(prAdapter, "HifTxLoop", 1);
    prWifiVar->u4TxFromOsLoopCount =
        (u32)wlanCfgGetUint32(prAdapter, "OsTxLoop", 1);
    prWifiVar->u4TxRxLoopCount =
        (u32)wlanCfgGetUint32(prAdapter, "Rx2ReorderLoop", 1);
    prWifiVar->u4TxIntThCount = (u32)wlanCfgGetUint32(prAdapter, "IstTxTh",
                                                      HIF_IST_TX_THRESHOLD);

    prWifiVar->u4NetifStopTh =
        (u32)wlanCfgGetUint32(prAdapter, "NetifStopTh",
                              CFG_TX_STOP_NETIF_PER_QUEUE_THRESHOLD);
    prWifiVar->u4NetifStartTh =
        (u32)wlanCfgGetUint32(prAdapter, "NetifStartTh",
                              CFG_TX_START_NETIF_PER_QUEUE_THRESHOLD);
    prWifiVar->ucTxBaSize = (u8)wlanCfgGetUint32(prAdapter, "TxBaSize", 64);
    prWifiVar->ucRxHtBaSize =
        (u8)wlanCfgGetUint32(prAdapter, "RxHtBaSize", 64);
    prWifiVar->ucRxVhtBaSize =
        (u8)wlanCfgGetUint32(prAdapter, "RxVhtBaSize", 64);

    /* Tx Buffer Management */
    prWifiVar->ucExtraTxDone =
        (u32)wlanCfgGetUint32(prAdapter, "ExtraTxDone", 1);
    prWifiVar->ucTxDbg = (u32)wlanCfgGetUint32(prAdapter, "TxDbg", 0);

    kalMemZero(prWifiVar->au4TcPageCount,
               sizeof(prWifiVar->au4TcPageCount));

    prWifiVar->au4TcPageCount[TC0_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc0Page", NIC_TX_PAGE_COUNT_TC0);
    prWifiVar->au4TcPageCount[TC1_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc1Page", NIC_TX_PAGE_COUNT_TC1);
    prWifiVar->au4TcPageCount[TC2_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc2Page", NIC_TX_PAGE_COUNT_TC2);
    prWifiVar->au4TcPageCount[TC3_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc3Page", NIC_TX_PAGE_COUNT_TC3);
    prWifiVar->au4TcPageCount[TC4_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc4Page", NIC_TX_PAGE_COUNT_TC4);
    prWifiVar->au4TcPageCount[TC5_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc5Page", NIC_TX_PAGE_COUNT_TC5);

#if QM_ADAPTIVE_TC_RESOURCE_CTRL
    /*MinRsrv*/
    prQM->au4MinReservedTcResource[TC0_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc0MinRsv", QM_MIN_RESERVED_TC0_RESOURCE);
    prQM->au4MinReservedTcResource[TC1_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc1MinRsv", QM_MIN_RESERVED_TC1_RESOURCE);
    prQM->au4MinReservedTcResource[TC2_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc2MinRsv", QM_MIN_RESERVED_TC2_RESOURCE);
    prQM->au4MinReservedTcResource[TC3_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc3MinRsv", QM_MIN_RESERVED_TC3_RESOURCE);
    prQM->au4MinReservedTcResource[TC4_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc4MinRsv", QM_MIN_RESERVED_TC4_RESOURCE);
    prQM->au4MinReservedTcResource[TC5_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc5MinRsv", QM_MIN_RESERVED_TC5_RESOURCE);

#if (HIF_TX_RSRC_WMM_ENHANCE == 1)
    prQM->au4MinReservedTcResource[TC6_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc6MinRsv", QM_MIN_RESERVED_TC6_RESOURCE);
    prQM->au4MinReservedTcResource[TC7_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc7MinRsv", QM_MIN_RESERVED_TC7_RESOURCE);
    prQM->au4MinReservedTcResource[TC8_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc8MinRsv", QM_MIN_RESERVED_TC8_RESOURCE);
    prQM->au4MinReservedTcResource[TC9_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc9MinRsv", QM_MIN_RESERVED_TC9_RESOURCE);
    prQM->au4MinReservedTcResource[TC10_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc10MinRsv", QM_MIN_RESERVED_TC10_RESOURCE);

    prQM->au4MinReservedTcResource[TC11_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc11MinRsv", QM_MIN_RESERVED_TC11_RESOURCE);
    prQM->au4MinReservedTcResource[TC12_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc12MinRsv", QM_MIN_RESERVED_TC12_RESOURCE);
    prQM->au4MinReservedTcResource[TC13_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc13MinRsv", QM_MIN_RESERVED_TC13_RESOURCE);
    prQM->au4MinReservedTcResource[TC14_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc14MinRsv", QM_MIN_RESERVED_TC14_RESOURCE);
    prQM->au4MinReservedTcResource[TC15_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc15MinRsv", QM_MIN_RESERVED_TC15_RESOURCE);
    prQM->au4MinReservedTcResource[TC16_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc16MinRsv", QM_MIN_RESERVED_TC16_RESOURCE);
#endif

    /*Guaranteed*/
    prQM->au4GuaranteedTcResource[TC0_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc0Grt", QM_GUARANTEED_TC0_RESOURCE);
    prQM->au4GuaranteedTcResource[TC1_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc1Grt", QM_GUARANTEED_TC1_RESOURCE);
    prQM->au4GuaranteedTcResource[TC2_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc2Grt", QM_GUARANTEED_TC2_RESOURCE);
    prQM->au4GuaranteedTcResource[TC3_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc3Grt", QM_GUARANTEED_TC3_RESOURCE);
    prQM->au4GuaranteedTcResource[TC4_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc4Grt", QM_GUARANTEED_TC4_RESOURCE);
    prQM->au4GuaranteedTcResource[TC5_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc5Grt", QM_GUARANTEED_TC5_RESOURCE);

#if (HIF_TX_RSRC_WMM_ENHANCE == 1)
    prQM->au4GuaranteedTcResource[TC6_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc6Grt", QM_GUARANTEED_TC6_RESOURCE);
    prQM->au4GuaranteedTcResource[TC7_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc7Grt", QM_GUARANTEED_TC7_RESOURCE);
    prQM->au4GuaranteedTcResource[TC8_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc8Grt", QM_GUARANTEED_TC8_RESOURCE);
    prQM->au4GuaranteedTcResource[TC9_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc9Grt", QM_GUARANTEED_TC9_RESOURCE);
    prQM->au4GuaranteedTcResource[TC10_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc10Grt", QM_GUARANTEED_TC10_RESOURCE);
    prQM->au4GuaranteedTcResource[TC11_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc11Grt", QM_GUARANTEED_TC11_RESOURCE);
    prQM->au4GuaranteedTcResource[TC12_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc12Grt", QM_GUARANTEED_TC12_RESOURCE);
    prQM->au4GuaranteedTcResource[TC13_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc13Grt", QM_GUARANTEED_TC13_RESOURCE);
    prQM->au4GuaranteedTcResource[TC14_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc14Grt", QM_GUARANTEED_TC14_RESOURCE);
    prQM->au4GuaranteedTcResource[TC15_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc15Grt", QM_GUARANTEED_TC15_RESOURCE);
    prQM->au4GuaranteedTcResource[TC16_INDEX] = (u32)wlanCfgGetUint32(
        prAdapter, "Tc16Grt", QM_GUARANTEED_TC16_RESOURCE);
#endif

    prQM->u4TimeToAdjustTcResource = (u32)wlanCfgGetUint32(
        prAdapter, "TcAdjustTime", QM_INIT_TIME_TO_ADJUST_TC_RSC);
    prQM->u4TimeToUpdateQueLen = (u32)wlanCfgGetUint32(
        prAdapter, "QueLenUpdateTime", QM_INIT_TIME_TO_UPDATE_QUE_LEN);
    prQM->u4QueLenMovingAverage = (u32)wlanCfgGetUint32(
        prAdapter, "QueLenMovingAvg", QM_QUE_LEN_MOVING_AVE_FACTOR);
    prQM->u4ExtraReservedTcResource = (u32)wlanCfgGetUint32(
        prAdapter, "TcExtraRsv", QM_EXTRA_RESERVED_RESOURCE_WHEN_BUSY);
#endif

    /* Stats log */
    prWifiVar->u4StatsLogTimeout = (u32)wlanCfgGetUint32(
        prAdapter, "StatsLogTO", WLAN_TX_STATS_LOG_TIMEOUT);
    prWifiVar->u4StatsLogDuration = (u32)wlanCfgGetUint32(
        prAdapter, "StatsLogDur", WLAN_TX_STATS_LOG_DURATION);

    prWifiVar->ucDhcpTxDone =
        (u8)wlanCfgGetUint32(prAdapter, "DhcpTxDone", 1);
    prWifiVar->ucArpTxDone =
        (u8)wlanCfgGetUint32(prAdapter, "ArpTxDone", 1);

    prWifiVar->ucMacAddrOverride =
        (u8)wlanCfgGetInt32(prAdapter, "MacOverride", 0);
    if (wlanCfgGet(prAdapter, "MacAddr", prWifiVar->aucMacAddrStr,
                   "00:0c:e7:66:32:e1", 0)) {
        DBGLOG(INIT, INFO, "get MacAddr fail, use defaul\n");
    }

    prWifiVar->ucCtiaMode = (u8)wlanCfgGetUint32(prAdapter, "CtiaMode", 0);
    ...
    prWifiVar->ucTpTestMode =
        (u8)wlanCfgGetUint32(prAdapter, "TpTestMode", 0);

#if CFG_SUPPORT_DBDC
    prWifiVar->ucDbdcMode =
        (u8)wlanCfgGetUint32(prAdapter, "DbdcMode", DBDC_MODE_DYNAMIC);
#endif
#if (CFG_EFUSE_BUFFER_MODE_DELAY_CAL == 1)
    prWifiVar->ucEfuseBufferModeCal =
        (u8)wlanCfgGetUint32(prAdapter, "EfuseBufferModeCal", 0);
#endif
    prWifiVar->ucCalTimingCtrl = (u8)wlanCfgGetUint32(
        prAdapter, "CalTimingCtrl", 0  /* power on full cal */ );
    prWifiVar->ucWow =
        (u8)wlanCfgGetUint32(prAdapter, "Wow", FEATURE_DISABLED);
    prWifiVar->ucOffload =
        (u8)wlanCfgGetUint32(prAdapter, "Offload", FEATURE_DISABLED);
    prWifiVar->ucAdvPws =
        (u8)wlanCfgGetUint32(prAdapter, "AdvPws", FEATURE_ENABLED);
    prWifiVar->ucWowOnMdtim =
        (u8)wlanCfgGetUint32(prAdapter, "WowOnMdtim", 1);
    prWifiVar->ucWowOffMdtim =
        (u8)wlanCfgGetUint32(prAdapter, "WowOffMdtim", 3);
    prWifiVar->ucWowPwsMode = (u8)wlanCfgGetUint32(prAdapter, "WowPwsMode",
    ...
    prWifiVar->ucEnforce2G = (u8)wlanCfgGetUint32(prAdapter, "Enforce2G",
                                                  Param_PowerModeMax);
    prWifiVar->ucEnforce5G = (u8)wlanCfgGetUint32(prAdapter, "Enforce5G",
                                                  Param_PowerModeMax);
#else
    prWifiVar->ucEnforcePSMode = (u8)wlanCfgGetUint32(
        prAdapter, "EnforcePSMode", Param_PowerModeMax);
    prWifiVar->ucEnforceCAM2G =
        (u8)wlanCfgGetUint32(prAdapter, "EnforceCAM2G", 0);
#endif

#if CFG_SUPPORT_REPLAY_DETECTION
    prWifiVar->ucRpyDetectOffload = (u8)wlanCfgGetUint32(
        prAdapter, "rpydetectoffload", FEATURE_ENABLED);
#endif

#if CFG_WOW_SUPPORT
    prAdapter->rWowCtrl.fgWowEnable =
        (u8)wlanCfgGetUint32(prAdapter, "WowEnable", FEATURE_ENABLED);
    prAdapter->rWowCtrl.ucScenarioId =
        (u8)wlanCfgGetUint32(prAdapter, "WowScenarioId", 0);
    prAdapter->rWowCtrl.ucBlockCount =
        (u8)wlanCfgGetUint32(prAdapter, "WowPinCnt", 1);
    prAdapter->rWowCtrl.astWakeHif[0].ucWakeupHif =
        (u8)wlanCfgGetUint32(prAdapter, "WowHif", ENUM_HIF_TYPE_GPIO);
    prAdapter->rWowCtrl.astWakeHif[0].ucGpioPin =
        (u8)wlanCfgGetUint32(prAdapter, "WowGpioPin", 0xFF);
    prAdapter->rWowCtrl.astWakeHif[0].ucTriggerLvl =
        (u8)wlanCfgGetUint32(prAdapter, "WowTriggerLevel", 3);
    ...
    prWifiVar->u4SwTestMode = (u8)wlanCfgGetUint32(prAdapter, "Sigma",
                                                   ENUM_SW_TEST_MODE_NONE);
    prWifiVar->ucCtrlFlagAssertPath = (u8)wlanCfgGetUint32(
        prAdapter, "AssertPath", DBG_ASSERT_PATH_DEFAULT);
    prWifiVar->ucCtrlFlagDebugLevel = (u8)wlanCfgGetUint32(
        prAdapter, "AssertLevel", DBG_ASSERT_CTRL_LEVEL_DEFAULT);
    prWifiVar->u4ScanCtrl = (u8)wlanCfgGetUint32(
        prAdapter, "ScanCtrl", SCN_CTRL_DEFAULT_SCAN_CTRL);
    prWifiVar->ucScanChannelListenTime =
        (u8)wlanCfgGetUint32(prAdapter, "ScnChListenTime", 0);

    /* Wake lock related configuration */
    prWifiVar->u4WakeLockRxTimeout = wlanCfgGetUint32(
        prAdapter, "WakeLockRxTO", WAKE_LOCK_RX_TIMEOUT);
    prWifiVar->u4WakeLockThreadWakeup = wlanCfgGetUint32(
        prAdapter, "WakeLockThreadTO", WAKE_LOCK_THREAD_WAKEUP_TIMEOUT);

    prWifiVar->ucSmartRTS = (u8)wlanCfgGetUint32(prAdapter, "SmartRTS", 0);
    /* add more cfg from RegInfo */
    prWifiVar->u4UapsdAcBmp =
        (u32)wlanCfgGetUint32(prAdapter, "UapsdAcBmp", 0);
    prWifiVar->u4MaxSpLen = (u32)wlanCfgGetUint32(prAdapter, "MaxSpLen", 0);
    prWifiVar->fgDisOnlineScan =
        (u32)wlanCfgGetUint32(prAdapter, "DisOnlineScan", 0);
    prWifiVar->fgDisBcnLostDetection =
        (u32)wlanCfgGetUint32(prAdapter, "DisBcnLostDetection", 0);
    prWifiVar->fgDisRoaming =
        (u32)wlanCfgGetUint32(prAdapter, "DisRoaming", 0);
    prWifiVar->fgEnArpFilter = (u32)wlanCfgGetUint32(
        prAdapter, "EnArpFilter", FEATURE_ENABLED);

    /* Driver Flow Control Dequeue Quota. Now is only used by DBDC */
    prWifiVar->uDeQuePercentEnable =
        (u8)wlanCfgGetUint32(prAdapter, "DeQuePercentEnable", 1);
    prWifiVar->u4DeQuePercentVHT80Nss1 =
        (u32)wlanCfgGetUint32(prAdapter, "DeQuePercentVHT80NSS1",
                              QM_DEQUE_PERCENT_VHT80_NSS1);
    prWifiVar->u4DeQuePercentVHT40Nss1 =
        (u32)wlanCfgGetUint32(prAdapter, "DeQuePercentVHT40NSS1",
                              QM_DEQUE_PERCENT_VHT40_NSS1);
    prWifiVar->u4DeQuePercentVHT20Nss1 =
        (u32)wlanCfgGetUint32(prAdapter, "DeQuePercentVHT20NSS1",
                              QM_DEQUE_PERCENT_VHT20_NSS1);
    prWifiVar->u4DeQuePercentHT40Nss1 = (u32)wlanCfgGetUint32(
        prAdapter, "DeQuePercentHT40NSS1", QM_DEQUE_PERCENT_HT40_NSS1);
    prWifiVar->u4DeQuePercentHT20Nss1 = (u32)wlanCfgGetUint32(
        prAdapter, "DeQuePercentHT20NSS1", QM_DEQUE_PERCENT_HT20_NSS1);

    /* Support TDLS 5.5.4.2 optional case */
    prWifiVar->fgTdlsBufferSTASleep = (u8)wlanCfgGetUint32(
        prAdapter, "TdlsBufferSTASleep", FEATURE_DISABLED);
    /* Support USB Whole chip reset recover */
    prWifiVar->fgChipResetRecover = (u8)wlanCfgGetUint32(
        prAdapter, "ChipResetRecover", FEATURE_ENABLED);

#if CFG_SUPPORT_ANT_SELECT
    prWifiVar->ucSpeIdxCtrl =
        (u8)wlanCfgGetUint32(prAdapter, "SpeIdxCtrl", 2);
    prWifiVar->ucSpeIdxCtrl2G =
        (u8)wlanCfgGetUint32(prAdapter, "SpeIdxCtrl2G", 2);
#endif
#ifdef CFG_SUPPORT_ADJUST_JOIN_CH_REQ_INTERVAL
    prWifiVar->u4AisJoinChReqIntervel =
        (u32)wlanCfgGetUint32(prAdapter, "AisJoinChReqIntervel",
    ...
    prWifiVar->rRssiPathCompasation.c2GRssiCompensation =
        (u8)wlanCfgGetUint32(prAdapter, "2GRssiComp", 0);
    prWifiVar->rRssiPathCompasation.c5GRssiCompensation =
        (u8)wlanCfgGetUint32(prAdapter, "5GRssiComp", 0);
#endif
    prWifiVar->ucEd2GNonEU =
        (s32)wlanCfgGetInt32(prAdapter, "Ed2GNonEU", 0);
    prWifiVar->ucEd5GNonEU =
        (s32)wlanCfgGetInt32(prAdapter, "Ed5GNonEU", 0);
    prWifiVar->ucEd2GEU = (s32)wlanCfgGetInt32(prAdapter, "Ed2GEU", 0);
    prWifiVar->ucEd5GEU = (s32)wlanCfgGetInt32(prAdapter, "Ed5GEU", 0);
    prWifiVar->ucDelayTimeOfDisconnect =
        (u8)wlanCfgGetUint32(prAdapter, "DelayTimeofDisconnect",
                             AIS_DELAY_TIME_OF_DISCONNECT_SEC);
#if CFG_KEY_ERROR_STATISTIC_RECOVERY
    prWifiVar->u4BmcKeyErrorTh =
        (s32)wlanCfgGetInt32(prAdapter, "BmcKeyErrorTh", 0);
#endif
}
...
void wlanCfgSetSwCtrl(IN P_ADAPTER_T prAdapter)
{
    ...
    for (i = 0; i < WLAN_CFG_SET_SW_CTRL_LEN_MAX; i++) {
        ...
        /* get nothing */
        if (wlanCfgGet(prAdapter, aucKey, aucValue, "", 0) !=
            WLAN_STATUS_SUCCESS) {
            continue;
        }
...
void wlanCfgSetDebugLevel(IN P_ADAPTER_T prAdapter)
{
    ...
    for (i = 0; i < WLAN_CFG_SET_DEBUG_LEVEL_LEN_MAX; i++) {
        ...
        if (u4DbgIdx == 0xFFFFFFFF) {
            ...
            DBGLOG(INIT, INFO,
                   "Set DBG module[%lu] log level to [0x%02x]!",
...
void wlanCfgSetCountryCode(IN P_ADAPTER_T prAdapter)
{
    ...
    if (wlanCfgGet(prAdapter, "Country", aucValue, "", 0) ==
        WLAN_STATUS_SUCCESS) {
        prAdapter->rWifiVar.rConnSettings.u2CountryCode =
            (((u16)aucValue[0]) << 8) | ((u16)aucValue[1]);
...
s32 wlanCfgGetInt32(IN P_ADAPTER_T prAdapter, const s8 *pucKey, s32 i4ValueDef)
{
    ...
    wlanCfgRecordValue(prAdapter, pucKey, (u32)i4Value);
                             ...
                             u8 times)
{
    ...
    char ch;
    s32 i = 0, j = 0;
    u32 bufferindex = 0, base = 0;
    u32 sum = 0, startOffset = 0;
    ...
    if (buffer == NULL || args_size[ED_STRING_SITE] == 0 ||
    ...
    DBGLOG(INIT, INFO, "\n");
    memcpy(((P_CMD_FORMAT_V1_T)buffer) + times, &cmd_v1,
    ...
    for (i = 0; i < WLAN_CFG_ENTRY_NUM_MAX; i++) {
        ...
        if (prWlanCfgEntry) {
            ...
            rCmd_v1.itemValueLength =
                kalStrLen(prWlanCfgEntry->aucValue);
            ...
            kalMemCopy(((P_CMD_FORMAT_V1_T)rCmdV1Header.buffer) +
                       ucTimes,
                         ...
                         u32 u4ConfigBufLen, u8 isFwConfig)
{
    ...
    s8 **ppcArgs;
    s32 i4Nargs;
    s8 arcArgv_size[WLAN_CFG_ARGV_MAX];
    u8 ucTimes = 0;
    ...
    u8 *pucCurrBuf = ucTmp;
    u32 u4CurrSize = ARRAY_SIZE(ucTmp);
    u32 u4RetSize = 0;
        ...
        if (prBssInfo->eNetworkType == NETWORK_TYPE_AIS) {
            ...
            if (prBssInfo->eCurrentOPMode == OP_MODE_ACCESS_POINT) {
                ...
                kalP2PGCIndicateConnectionStatus(
                    prGlueInfo,
                    (u8)prBssInfo->u4PrivateData, NULL,
                                  ...
                                  IN P_MSDU_INFO_T prMsduInfo)
{
    ...
    if (prStaRec) {
        u4DeltaTime = (u32)(prPktProfile->rHifTxDoneTimestamp -
                             ...
                             IN ENUM_TX_PROFILING_TAG_T eTag)
{
    ...
    case TX_PROF_TAG_DRV_ENQUE:
        ...
        if (wlanTxLifetimeIsTargetMsdu(prAdapter, prMsduInfo)) {
            ...
            /* Packet enqueue time */
            prPktProfile->rEnqueueTimestamp = (u32)kalGetTimeTick();
        }
        break;

    case TX_PROF_TAG_DRV_DEQUE:
        if (prPktProfile->fgIsValid) {
            prPktProfile->rDequeueTimestamp = (u32)kalGetTimeTick();
        }
        break;
                                ...
                                IN u32 u4DurationInMs)
{
    ...
    rResult = wlanSendSetQueryCmd(prAdapter, CMD_ID_STATS_LOG, true, false,
                                  ...
                                  nicOidCmdTimeoutCommon,
                                  sizeof(CMD_STATS_LOG_T),
                                  (u8 *)&rStatsLogCmd, NULL, 0);
    ...
    for (ucStaRecIdx = 0; ucStaRecIdx < CFG_STA_REC_NUM; ucStaRecIdx++) {
        ...
        if (pucAddr) {
            if (prTempStaRec->fgIsInUse &&
                EQUAL_MAC_ADDR(prTempStaRec->aucMacAddr, pucAddr)) {
                            ...
                            u8 fgSuspend)
{
     ...
     * setting or not */
    rStatus = kalIoctl(prGlueInfo, wlanoidSetDhcpOffladInfo,
                       (void *)&rDhcpSetCmd, sizeof(rDhcpSetCmd), false,
                         ...
                         u8 fgSuspend)
{
    ...
    if (prGlueInfo->prAdapter->rWifiVar.ucWow &&
        ...
        if (prGlueInfo->prAdapter->rWifiVar.ucAdvPws) {
            /* cfg enable + wow disable + adv pws enable => Wow Off
             * mdtim */
            rSuspendCmd.ucMdtim =
        ...
        if (prGlueInfo->prAdapter->rWifiVar.ucAdvPws) {
            /* cfg disable + adv pws enable => MT6632 case => Wow
             * Off mdtim */
            rSuspendCmd.ucMdtim =
                prGlueInfo->prAdapter->rWifiVar.ucWowOffMdtim;
    ...
    rStatus = kalIoctl(prGlueInfo, wlanoidNotifyFwSuspend,
                       (void *)&rSuspendCmd, sizeof(rSuspendCmd), false,
                       false, true, &u4SetInfoLen);
                           ...
                           IN u32 u4QueryBufferLen, OUT u32 *pu4QueryInfoLen)
{
    ...
    do {
        ...
        rResult = wlanSendSetQueryCmd(
            prAdapter, CMD_ID_GET_LTE_CHN, false, true, true,
            /* Query ID */
            ...
            (u8 *)&rQuery_LTE_SAFE_CHN, pvQueryBuffer,
            u4QueryBufferLen);
 ...
 */
/*----------------------------------------------------------------------------*/
static void wlanAddDirtinessToAffectedChannels(P_ADAPTER_T prAdapter,
                                               ...
                                               u8 ucCoveredRange)
{
    ...
    ucLeftNeighborChannel =
        ucLeftestCoveredChannel ? ucLeftestCoveredChannel - 1 : 0;
...
wlanCalculateAllChannelDirtiness(IN P_ADAPTER_T prAdapter)
{
    ...
    LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
        ...
        /* dirtiness index1 */
        wlanCalculateChannelDirtiness(prAdapter, prBssDesc, u4Dirtiness,
                                      true);
                      ...
                      IN u8 ucBssIndex)
{
    ...
    u8 ucSpeIdxCtrl = prAdapter->rWifiVar.ucSpeIdxCtrl;
    ...
    if (eBand == BAND_2G4 && !prAdapter->rWifiVar.ucSpeIdxCtrl2G) {
    ...
    if (!prAdapter->rWifiVar.fgDbDcModeEn) {
        ...
        if (eBand == BAND_2G4) {
            if (IS_WIFI_2G4_SISO(prAdapter)) {
                ...
                ucRetValSpeIdx =
                    wlanAntPathFavorSelect(eWfPathFavor);
            }
        } else if (eBand == BAND_5G) {
                      ...
                      u32 u4Sel)
{
    ...
    rSwCtrlInfo.u4Data = ((u4EdVal2G & 0xFF) | ((u4EdVal5G & 0xFF) << 16) |
                          (u4Sel << 31));
...
WLAN_STATUS wlanUpdateRssiComp(IN P_ADAPTER_T prAdapter)
{
    ...
    return wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_PATH_COMPASATION, true,
                               false, false, NULL, NULL,
                               sizeof(rCmdRssiPathCompasation),
                               (u8 *)&rCmdRssiPathCompasation, NULL, 0);
}
...
int wlanSuspendRekeyOffload(P_GLUE_INFO_T prGlueInfo, IN u8 ucRekeyMode)
{
    ...
    /* if enable => enable FW rekey offload. if disable, let rekey back to
     * supplicant */
    ...
    if (ucRekeyMode == GTK_REKEY_CMD_MODE_OFFLOAD_ON) {
        DBGLOG(RSN, INFO, "kek\n");
        DBGLOG_MEM8(RSN, ERROR, (u8 *)prGlueInfo->rWpaInfo.aucKek,
                    NL80211_KEK_LEN);
        DBGLOG(RSN, INFO, "kck\n");
        DBGLOG_MEM8(RSN, ERROR, (u8 *)prGlueInfo->rWpaInfo.aucKck,
                    NL80211_KCK_LEN);
        DBGLOG(RSN, INFO, "replay count\n");
        DBGLOG_MEM8(RSN, ERROR, (u8 *)prGlueInfo->rWpaInfo.aucReplayCtr,
    ...
    if ((ucRekeyMode == GTK_REKEY_CMD_MODE_SET_BCMC_PN) &&
        ...
        if (prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex >
            MAX_BSS_INDEX) {
            DBGLOG(SW4, ERROR,
                   "Invalid BssInfo index[%u], skip dump!\n",
                   prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex);
        ...
        DBGLOG_MEM8(RSN, INFO, (u8 *)prGtkData->aucReplayCtr,
                    NL80211_REPLAY_CTR_LEN);
        kalMemCopy(prGtkData->aucReplayCtr,
...
void wlanSuspendPmHandle(P_GLUE_INFO_T prGlueInfo)
{
    ...
    if (!prGlueInfo->prAdapter->rWifiVar.ucEapolOffload) {
        ...
        if ((ucKekZeroCnt == NL80211_KCK_LEN) ||
            ...
            wlanSuspendRekeyOffload(
                prGlueInfo, GTK_REKEY_CMD_MODE_RPY_OFFLOAD_OFF);
        }
    ...
    if (prGlueInfo->prAdapter->rWifiVar.ucWow &&
        ...
        if (kalGetMediaStateIndicated(prGlueInfo) ==
            PARAM_MEDIA_STATE_CONNECTED) {
            /* AIS bss enter wow power mode, default fast
             * power-saving */
            ...
            nicConfigPowerSaveWowProfile(prGlueInfo->prAdapter, idx,
                                         ePwrMode, false, true);
            DBGLOG(HAL, STATE,
                   "Suspend wow power save idx:%d, mode:%d\n", idx,
                   ePwrMode);
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief This function is to restore power-saving mode command when AIS leave
 * wow But ignore GC/GO/AP role
 *
 * @param prGlueInfo                     Pointer of prGlueInfo Data Structure
 *
 * @return void
...
void wlanResumePmHandle(P_GLUE_INFO_T prGlueInfo)
{
    ...
    /* if wifi.cfg EAPOL offload is disble, we disable FW offload when leave
     * wow */
    if (!prGlueInfo->prAdapter->rWifiVar.ucEapolOffload) {
        /*
         * check if KCK, KEK not sync from supplicant.
        ...
        /* as resume performed, reset BC/MC KeyRSC, to prevent incorrect
         * replay detection. */
        for (ucKeyIdx = 0; ucKeyIdx < 4; ucKeyIdx++) {
    ...
    if (prGlueInfo->prAdapter->rWifiVar.ucWow &&
        ...
        if (kalGetMediaStateIndicated(prGlueInfo) ==
            ...
            /* resume AIS power-saving cmd when leave wow state,
             * ignore ePwrMode input */
            nicConfigPowerSaveWowProfile(
                prGlueInfo->prAdapter,
                prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex,
...
void disconnect_sta(P_ADAPTER_T prAdapter, P_STA_RECORD_T sta_rec)
{
    ...
    case STA_TYPE_LEGACY_AP:
        ...
        ais_abort_msg = (P_MSG_AIS_ABORT_T)cnmMemAlloc(
...
#define DISCONNECT_STATUS    WLAN_STATUS_MEDIA_DISCONNECT
            ...
            if (kalGetMediaStateIndicated(glue_info) ==
                PARAM_MEDIA_STATE_CONNECTED) {
                kalIndicateStatusAndComplete(
                    glue_info, DISCONNECT_STATUS, NULL, 0);
            }
#undef DISCONNECT_STATUS
    ...
    case STA_TYPE_P2P_GO:
        p2p_abot_msg = (P_MSG_P2P_CONNECTION_ABORT_T)cnmMemAlloc(
        ...
        if (p2p_abot_msg) {
            ...
            p2p_bss_info = GET_BSS_INFO_BY_INDEX(
                prAdapter, sta_rec->ucBssIndex);
