...
u8 g_bCaptureDone = false;
u8 g_bIcapEnable = false;
u16 g_u2DumpIndex;
u8 g_fgFowardBcn2Supplicant = false;
...
static void rlmFillHtCapIE(P_ADAPTER_T prAdapter,
			   P_BSS_INFO_T prBssInfo,
			   P_MSDU_INFO_T prMsduInfo);

static void rlmFillExtCapIE(P_ADAPTER_T prAdapter,
			    P_BSS_INFO_T prBssInfo,
			    P_MSDU_INFO_T prMsduInfo);

static void rlmFillHtOpIE(P_ADAPTER_T prAdapter,
			  P_BSS_INFO_T prBssInfo,
			  P_MSDU_INFO_T prMsduInfo);

static u8 rlmRecIeInfoForClient(P_ADAPTER_T prAdapter,
				P_BSS_INFO_T prBssInfo,
				u8 *pucIE,
				u16 u2IELength);
...
static u8 rlmRecBcnInfoForClient(P_ADAPTER_T prAdapter,
				 P_BSS_INFO_T prBssInfo,
				 P_SW_RFB_T prSwRfb,
				 u8 *pucIE,
				 u16 u2IELength);
...
static void rlmFillVhtCapIE(P_ADAPTER_T prAdapter,
			    P_BSS_INFO_T prBssInfo,
			    P_MSDU_INFO_T prMsduInfo);
...
void rlmFsmEventInit(P_ADAPTER_T prAdapter)
{
	...
	for (i = 0; i < BSS_INFO_NUM; i++) {
		...
		cnmTimerInitTimer(prAdapter, &prBssInfo->rTxQuietTimer,
				  (PFN_MGMT_TIMEOUT_FUNC)rrmTxQuietTimeout,
...
void rlmReqGeneratePowerCapIE(P_ADAPTER_T prAdapter, P_MSDU_INFO_T prMsduInfo)
{
	...
	/* We should add power capability IE in assoc/reassoc req if the
	 * spectrum management bit is set to 1 in Capability Infor field, or the
	 * connection will be rejected by Marvell APs in some TGn items.
	 * (e.g. 5.2.32). Spectrum management related feature (802.11h) is for
	 * 5G band.
	 */
				 ...
				 P_MSDU_INFO_T prMsduInfo)
{
	...
	/* We should add supported channels IE in assoc/reassoc req if the
	 * spectrum management bit is set to 1 in Capability Infor field, or the
	 * connection will be rejected by Marvell APs in some TGn items.
	 * (e.g. 5.2.3). Spectrum management related feature (802.11h) is for 5G
	 * band.
	 */
	if (!prBssInfo || prBssInfo->eBand != BAND_5G)
		return;
...
void rlmRspGenerateErpIE(P_ADAPTER_T prAdapter, P_MSDU_INFO_T prMsduInfo)
{
	...
	if (RLM_NET_IS_11GN(prBssInfo) && prBssInfo->eBand == BAND_2G4 &&
		...
		prErpIe = (P_IE_ERP_T)(((u8 *)prMsduInfo->prPacket) +
			 ...
			 IN u32 *pu4Cap)
{
	...
	do {
		...
		kalMemCopy(pu4Cap, prMtkOuiIE->aucCapability, sizeof(u32));
			   ...
			   P_MSDU_INFO_T prMsduInfo)
{
	...
	prHtCap = (P_IE_HT_CAP_T)(((u8 *)prMsduInfo->prPacket) +
				  prMsduInfo->u2FrameLength);
	...
	if (prBssInfo->ucNss <
	    wlanGetSupportNss(prAdapter, prBssInfo->ucBssIndex)) {
		prHtCap->u2HtCapInfo &= ~HT_CAP_INFO_SM_POWER_SAVE; /*Set as
		                                                     * static
		                                                     * power
		                                                     * save */
	}
			    ...
			    P_MSDU_INFO_T prMsduInfo)
{
	...
	prExtCap = (P_EXT_CAP_T)(((u8 *)prMsduInfo->prPacket) +
				 prMsduInfo->u2FrameLength);
	...
	if (prAdapter->rWifiVar.ucAvailablePhyTypeSet & PHY_TYPE_SET_802_11AC) {
		/* Note: For AIS connecting state, structure in BSS_INFO will
		 * not be inited */
			  ...
			  P_MSDU_INFO_T prMsduInfo)
{
	...
	prHtOp = (P_IE_HT_OP_T)(((u8 *)prMsduInfo->prPacket) +
				prMsduInfo->u2FrameLength);
	...
	} else if (prBssInfo->eObssHtProtectMode ==
		   HT_PROTECT_MODE_NON_MEMBER) {
		prHtOp->u2Info2 = (u8)HT_PROTECT_MODE_NON_MEMBER;
	} else {
		/* It may be SYS_PROTECT_MODE_NONE or SYS_PROTECT_MODE_20M */
		prHtOp->u2Info2 = (u8)prBssInfo->eHtProtectMode;
	}
				       ...
				       P_MSDU_INFO_T prMsduInfo, u8 fgIsMaxCap)
{
	...
	prVhtOpMode =
		(P_IE_VHT_OP_MODE_NOTIFICATION_T)(((u8 *)prMsduInfo->prPacket) +
						  prMsduInfo->u2FrameLength);

	kalMemZero((void *)prVhtOpMode, sizeof(IE_VHT_OP_MODE_NOTIFICATION_T));

	prVhtOpMode->ucId = ELEM_ID_OP_MODE;
	prVhtOpMode->ucLength =
		sizeof(IE_VHT_OP_MODE_NOTIFICATION_T) - ELEM_HDR_LEN;
			    ...
			    P_MSDU_INFO_T prMsduInfo)
{
	...
	prVhtCap = (P_IE_VHT_CAP_T)(((u8 *)prMsduInfo->prPacket) +
				    prMsduInfo->u2FrameLength);
	...
	if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.ucStaVhtBfee)) {
		...
		if (prStaRec &&
			...
			prVhtCap->u4VhtCapInfo |=
				(((u32)prStaRec->ucVhtCapNumSoundingDimensions)
					<<
			 ...
			 * it requires STAUT to set its maximum STS capability
			 * here
			 */
			prVhtCap->u4VhtCapInfo |=
		    ...
		    P_MSDU_INFO_T prMsduInfo)
{
	...
	prVhtOp = (P_IE_VHT_OP_T)(((u8 *)prMsduInfo->prPacket) +
				  prMsduInfo->u2FrameLength);
	...
	prVhtOp->ucVhtOperation[0] =
		prBssInfo->ucVhtChannelWidth; /* (UINT8)VHT_OP_CHANNEL_WIDTH_80;
	                                       */
...
/* P_ENUM_CHANNEL_WIDTH_P peChannelWidth will be treated as u16 * pointer
		    ...
		    u8 *peChannelWidth, u8 *pucS1, u8 *pucPrimaryCh)
{
	...
	if (*peChannelWidth > CW_20_40MHZ) {
		...
		ucCurrentBandwidth = (u8)*peChannelWidth + ucOffset;
	} else {
		/*case BW20 BW40 */
	...
	if (ucCurrentBandwidth > ucMaxBandwidth) {
				...
				if (ucCurrentBandwidth == MAX_BW_160MHZ) {
					if ((*pucPrimaryCh >= 36) &&
						...
						*pucS1 = 122;
						   ...
						   (*pucPrimaryCh <= 144)) {
						*pucS1 = 138; /*160 downgrade
						               * should not in
						               * this case*/
					} else if ((*pucPrimaryCh >= 149) &&
						   (*pucPrimaryCh <= 161)) {
						*pucS1 = 155; /*160 downgrade
						               * should not in
						               * this case*/
					} else {
						DBGLOG(RLM,
		...
		DBGLOG(RLM,
		       INFO,
		       "Modify ChannelWidth (%d) and Extend (%d)\n",
		       *peChannelWidth,
		       *peExtend);
	}
}
			  ...
			  u8 ucChannelWidth)
{
	...
	if (ucChannelWidth == MAX_BW_80_80_MHZ) {
		...
		if (prBssInfo->ucVhtPeerChannelWidth ==
		    VHT_OP_CHANNEL_WIDTH_160) {
			prBssInfo->ucVhtChannelFrequencyS1 =
				(prBssInfo->ucVhtPeerChannelFrequencyS1 >
				 prBssInfo->ucPrimaryChannel) ?
				(prBssInfo->ucVhtPeerChannelFrequencyS1 -
				 8) :
				(prBssInfo->ucVhtPeerChannelFrequencyS1 +
				 8);
		} else if (prBssInfo->ucVhtPeerChannelWidth ==
				...
				u8 *pucIE, u16 u2IELength)
{
	...
	u8 ucERP, ucPrimaryChannel;
	P_WIFI_VAR_T prWifiVar = &prAdapter->rWifiVar;
#if CFG_SUPPORT_QUIET
	u8 fgHasQuietIE = false;
#endif
	u8 IsfgHtCapChange = false;

#if CFG_SUPPORT_802_11AC
	P_IE_VHT_OP_T prVhtOp;
	P_IE_VHT_CAP_T prVhtCap;
	P_IE_OP_MODE_NOTIFICATION_T
		prOPModeNotification; /* Operation Mode Notification */
	u8 fgHasOPModeIE = false;
	u8 ucVhtOpModeChannelWidth = 0;
	u8 ucVhtOpModeRxNss = 0;
	u8 ucVhtCapMcsOwnNotSupportOffset;
	u8 ucMaxBwAllowed;
	u8 ucInitVhtOpMode = 0;
#endif
	 ...
	 * corresponding BssInfo supports 802.11n, i.e., RLM_NET_IS_11N()
	 */
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		...
		case ELEM_ID_HT_CAP:
			...
			if ((prStaRec->u2HtCapInfo &
			    ...
			    (prHtCap->u2HtCapInfo &
			     HT_CAP_INFO_SM_POWER_SAVE)) {
				IsfgHtCapChange = true; /* Purpose : To detect
				                         * SMPS change */
			}
		...
		case ELEM_ID_HT_OP:
			 ...
			 * by its secondary channel, but its DS IE is correct
			 * 20110610
			 */
			...
			if ((prBssInfo->ucHtOpInfo1 & HT_OP_INFO1_SCO) !=
			    CHNL_EXT_RES) {
				prBssInfo->eBssSCO =
					(ENUM_CHNL_EXT_T)(prBssInfo->ucHtOpInfo1
							  &
							  HT_OP_INFO1_SCO);
			}
			...
			prBssInfo->eHtProtectMode =
				(ENUM_HT_PROTECT_MODE_T)(prBssInfo->u2HtOpInfo2
							 &
		...
		case ELEM_ID_VHT_CAP:
			...
			/*Set Vht Rx Mcs Map upon peer's capability and our
			 * capability */
			prStaRec->u2VhtRxMcsMap =
				prVhtCap->rVhtSupportedMcsSet.u2RxMcsMap;
			if (wlanGetSupportNss(prAdapter, prStaRec->ucBssIndex) <
			    8) {
				...
				prStaRec->u2VhtRxMcsMap |= BITS(
					ucVhtCapMcsOwnNotSupportOffset, 15);
			}
		...
		case ELEM_ID_OP_MODE:
			if (!RLM_NET_IS_11AC(prBssInfo) ||
			    IE_LEN(pucIE) !=
			    (sizeof(IE_OP_MODE_NOTIFICATION_T) - 2))
				break;
			prOPModeNotification =
				(P_IE_OP_MODE_NOTIFICATION_T)pucIE;
		...
		case ELEM_ID_WIDE_BAND_CHANNEL_SWITCH:
			if (!RLM_NET_IS_11AC(prBssInfo) ||
			    IE_LEN(pucIE) !=
			    (sizeof(IE_WIDE_BAND_CHANNEL_T) - 2))
				break;
			DBGLOG(RLM,
			       INFO,
		...
		case ELEM_ID_CH_SW_ANNOUNCEMENT:
...
#if CFG_DFS_NEWCH_DFS_FORCE_DISCONNECT
			max_count = rlmDomainGetActiveChannelCount(
				NL80211_BAND_5GHZ) +
			...
			DBGLOG(RLM,
			       INFO,
			       "[DFS][CSA][CLIENT] Switch to new channel: new ChNum = [%d]\n",
			       prCSAIE->ucNewChannelNum);
			if ((Channel) &&
				...
				DBGLOG(RLM,
				       INFO,
				       "[DFS][CSA][CLIENT] New channel is un-supported channel!");
			} else {
				DBGLOG(RLM,
				       INFO,
				       "[DFS][CSA][CLIENT] New channel is non-DFS channel!");
			}
#endif
			break;
		...
		case ELEM_ID_SCO:
			if (IE_LEN(pucIE) !=
			    (sizeof(IE_SECONDARY_OFFSET_T) - 2))
				break;
		 ...
		 */
		case ELEM_ID_QUIET:
			rrmQuietHandleQuietIE(prAdapter, prBssInfo,
					      (P_IE_QUIET_T)pucIE);
			g_fgFowardBcn2Supplicant = true;
			fgHasQuietIE = true;
			break;
	 ...
	 * changed with the channel bandwidth setting of Operation Mode field.
	...
	if (fgHasOPModeIE == true) {
		...
		if (ucVhtOpModeChannelWidth == VHT_OP_MODE_CHANNEL_WIDTH_20) {
			...
			prBssInfo->ucHtOpInfo1 |= HT_OP_INFO1_STA_CHNL_WIDTH;
			prStaRec->u4VhtCapInfo &=
				~VHT_CAP_INFO_MAX_SUP_CHANNEL_WIDTH_MASK;
			prStaRec->u2HtCapInfo |= HT_CAP_INFO_SUP_CHNL_WIDTH;
		} else if (ucVhtOpModeChannelWidth ==
			   VHT_OP_MODE_CHANNEL_WIDTH_160_80P80) {
			prBssInfo->ucHtOpInfo1 |= HT_OP_INFO1_STA_CHNL_WIDTH;
			...
			if (prBssInfo->ucVhtChannelWidth ==
				...
				prStaRec->u4VhtCapInfo |=
					VHT_CAP_INFO_MAX_SUP_CHANNEL_WIDTH_SET_160;
			} else if (prBssInfo->ucVhtChannelWidth ==
				   VHT_OP_CHANNEL_WIDTH_80P80) {
				prStaRec->u4VhtCapInfo |=
					VHT_CAP_INFO_MAX_SUP_CHANNEL_WIDTH_SET_160_80P80;
			}
		...
		if (prStaRec->ucStaState == STA_STATE_3) {
			DBGLOG(RLM, INFO, "Update OpMode to 0x%x",
			       prStaRec->ucVhtOpMode);
			DBGLOG(RLM, INFO,
			       "to FW due to OpMode Notificaition\n");
			cnmStaSendUpdateCmd(prAdapter, prStaRec, NULL, false);
		}
	...
	if (!HAS_CH_SWITCH_PARAMS(prCSAParams, prBssDesc) &&
	    prBssInfo->fgHasStopTx
#if CFG_SUPPORT_QUIET
	...
	rlmReviseMaxBw(prAdapter, prBssInfo->ucBssIndex, &prBssInfo->eBssSCO,
		       (u8 *)&prBssInfo->ucVhtChannelWidth,
		       &prBssInfo->ucVhtChannelFrequencyS1,
		       &prBssInfo->ucPrimaryChannel);
					 ...
					 u16 u2IELength)
{
	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		...
		case ELEM_ID_HT_OP: {
			...
			if ((prHtOp->ucInfo1 & HT_OP_INFO1_SCO) !=
			    CHNL_EXT_RES) {
				eSCO = (ENUM_CHNL_EXT_T)(prHtOp->ucInfo1 &
							 HT_OP_INFO1_SCO);
			}
			break;
		}
	 ...
	 * same update procedure. We should give it the entry pointer of desired
	 * channel list.
	 */
			...
			u16 u2IELength)
{
	 ...
	 * Response, user should use another function, rlmRecIeInfoForClient()
	 */
	...
	/*Avoid wrong primary channel info in HT operation IE info when accept
	 ...
	 * which shall be invoked afterwards. Update channel, bandwidth and
	 * protection mode by nicUpdateBss()
	 */
}
			 ...
			 P_BSS_INFO_T prBssInfo)
{
	...
	prCmdBody->ucRfBand = (u8)prBssInfo->eBand;
	prCmdBody->ucPrimaryChannel = prBssInfo->ucPrimaryChannel;
	prCmdBody->ucRfSco = (u8)prBssInfo->eBssSCO;
	prCmdBody->ucErpProtectMode = (u8)prBssInfo->fgErpProtectMode;
	prCmdBody->ucHtProtectMode = (u8)prBssInfo->eHtProtectMode;
	prCmdBody->ucGfOperationMode = (u8)prBssInfo->eGfOperationMode;
	prCmdBody->ucTxRifsMode = (u8)prBssInfo->eRifsOperationMode;
	...
	if (RLM_NET_PARAM_VALID(prBssInfo)) {
		DBGLOG(RLM,
		       ...
		       prCmdBody->ucRfSco,
		       prCmdBody->ucErpProtectMode,
		       prCmdBody->ucHtProtectMode,
		       prCmdBody->ucHtOpInfo1,
		       prCmdBody->ucUseShortSlotTime,
...
void rlmSyncOperationParams(P_ADAPTER_T prAdapter, P_BSS_INFO_T prBssInfo)
{
	...
	prCmdBody = (P_CMD_SET_BSS_RLM_PARAM_T)cnmMemAlloc(
		prAdapter, RAM_TYPE_BUF, sizeof(CMD_SET_BSS_RLM_PARAM_T));
	...
	if (rStatus != WLAN_STATUS_PENDING) {
		DBGLOG(RLM,
		       WARN,
		       "rlmSyncOperationParams set cmd fail\n");
	}
			...
			u16 u2IELength)
{
	...
	P_IE_VHT_CAP_T prVhtCap;
	u8 ucVhtCapMcsOwnNotSupportOffset;
	/* u8 ucVhtCapMcsPeerNotSupportOffset; */
	P_IE_OP_MODE_NOTIFICATION_T
		prOPModeNotification; /* Operation Mode Notification */
#endif
			     ...
			     u8 *pOutBuf)
{
	...
	u8 i;
	u8 ucMaxBw;
	u8 supportNss = wlanGetSupportNss(prAdapter, prBssInfo->ucBssIndex);
				  ...
				  IN PFN_TX_DONE_HANDLER pfTxDoneHandler)
{
	...
	prTxFrame =
		(P_ACTION_TPC_REPORT_FRAME)((unsigned long)(prMsduInfo
							    ->prPacket) +
				   ...
				   IN PFN_TX_DONE_HANDLER pfTxDoneHandler)
{
	...
	prTxFrame =
		(P_ACTION_SM_REPORT_FRAME)((unsigned long)(prMsduInfo->prPacket)
					   +
					   MAC_TX_RESERVED_FIELD);
...
void rlmProcessSpecMgtAction(P_ADAPTER_T prAdapter, P_SW_RFB_T prSwRfb)
{
	...
	u2IELength = prSwRfb->u2PacketLen -
		     (u16)OFFSET_OF(ACTION_SM_REQ_FRAME, aucInfoElem[0]);
	...
	case ACTION_CHNL_SWITCH:
		...
		IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
			...
			case ELEM_ID_WIDE_BAND_CHANNEL_SWITCH:
				...
				prWideBandChannelIE =
					(P_IE_WIDE_BAND_CHANNEL_T)pucIE;
				prCSAParams->ucVhtBw =
					prWideBandChannelIE->ucNewChannelWidth;
				prCSAParams->ucVhtS1 =
			...
			case ELEM_ID_CH_SW_ANNOUNCEMENT:
				...
				if (prChannelSwitchAnnounceIE
					...
					/* Need to stop data transmission
					 * immediately */
					if (!prBssInfo->fgHasStopTx) {
						prBssInfo->fgHasStopTx = true;
#if CFG_SUPPORT_TDLS
						/* TDLS peers */
						TdlsTxCtrl(prAdapter, prBssInfo,
							   false);
#endif
						/* AP */
						qmSetStaRecTxAllowed(prAdapter,
				...
				max_count = rlmDomainGetActiveChannelCount(
					...
					NL80211_BAND_2GHZ);
				for (i = 0; i < max_count; i++) {
					Channel = rlmDomainGetActiveChannels() +
						  i;
				...
				if ((Channel) &&
				    (Channel->flags & IEEE80211_CHAN_RADAR)) {
					prCSAParams->fgBeaconNewChannelIsDFS =
						true;
					DBGLOG(RLM,
					       INFO,
					       "[DFS][CSA][CLIENT] New channel is DFS channel!");
				} else if (!Channel) {
					prCSAParams->fgNewChannelIsDisabled =
						true;
					DBGLOG(RLM,
					       INFO,
					       "[DFS][CSA][CLIENT] New channel is un-supported channel!");
			...
			case ELEM_ID_SCO:
				...
				prSecondaryOffsetIE =
					(P_IE_SECONDARY_OFFSET_T)pucIE;

				DBGLOG(RLM, INFO, "[CSA Mgt] SCO [%d]->[%d]\n",
				       prBssInfo->eBssSCO,
				       prSecondaryOffsetIE->ucSecondaryOffset);

				prCSAParams->eSco =
					(ENUM_CHNL_EXT_T)prSecondaryOffsetIE
					->ucSecondaryOffset;
				break;
			       ...
			       IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	do {
		...
		DBGLOG(P2P, INFO, "CSA TX Done Status: %d, seqNo: %d\n",
...
void rlmSendChannelSwitchFrame(IN P_ADAPTER_T prAdapter, u8 ucBssIndex)
{
	...
	u2EstimatedFrameLen =
		MAC_TX_RESERVED_FIELD + sizeof(ACTION_CHANNEL_SWITCH_FRAME);

	/* Alloc MSDU_INFO */
	prMsduInfo =
		(P_MSDU_INFO_T)cnmMgtPktAlloc(prAdapter, u2EstimatedFrameLen);
	...
	prTxFrame->aucInfoElem[2] = prAdapter->rWifiVar.ucChannelSwitchMode;
	prTxFrame->aucInfoElem[3] = prAdapter->rWifiVar.ucNewChannelNumber;
	prTxFrame->aucInfoElem[4] = prAdapter->rWifiVar.ucChannelSwitchCount;
	...
	TX_SET_MMPDU(prAdapter, prMsduInfo, prBssInfo->ucBssIndex,
		     ...
		     sizeof(ACTION_CHANNEL_SWITCH_FRAME), pfTxDoneHandler,
		     MSDU_RATE_MODE_AUTO);
...
void rlmCsaTimeout(IN P_ADAPTER_T prAdapter, unsigned long ulParamPtr)
{
	...
	DBGLOG(RLM, EVENT,
	       "[CSA] CSA timeout and prepare to switch to new channel(%d)\n",
	       prCSAParams->ucCsaNewCh);

	if (prCSAParams->fgNewChannelIsDisabled) {
		prCSAParams->fgNewChannelIsDisabled = false;
		/* do aisBSSlinkdown directly here without sending CSA
		 * notification */
		DBGLOG(RLM,
		       EVENT,
		       "[CSA] Disconnect with the AP due to the new channel is un-supported\n");
	...
	if (prBssDesc) {
		DBGLOG(RLM, INFO,
		       ...
		       prCSAParams->ucCsaNewCh, prBssInfo->eBssSCO);
		...
		kalUpdateBssChannel(prAdapter->prGlueInfo, prBssDesc->aucSSID,
				    prBssDesc->ucSSIDLen, prBssDesc->aucBSSID,
...
#else
		DBGLOG(RLM, EVENT,
		       "[CSA] Bypass SAP channel switch triggered by driver\n");
#endif
	}
...
#if CFG_DFS_NEWCH_DFS_FORCE_DISCONNECT
	if (prCSAParams->fgBeaconNewChannelIsDFS ||
	    prCSAParams->fgActionNewChannelIsDFS) {
		prCSAParams->fgBeaconNewChannelIsDFS = false;
		prCSAParams->fgActionNewChannelIsDFS = false;
		aisBssLinkDown(prAdapter);
	} else
				    ...
				    u8 ucNss)
{
	...
	TX_SET_MMPDU(prAdapter, prMsduInfo, prBssInfo->ucBssIndex,
		     prStaRec->ucIndex, WLAN_MAC_MGMT_HEADER_LEN,
		     sizeof(ACTION_OP_MODE_NOTIFICATION_FRAME), NULL,
		     MSDU_RATE_MODE_AUTO);
			     ...
			     u8 ucNss)
{
	...
	/* Calculate MSDU buffer length */
	u2EstimatedFrameLen =
		MAC_TX_RESERVED_FIELD + sizeof(ACTION_SM_POWER_SAVE_FRAME);
	...
	prTxFrame->ucSmPowerCtrl &=
		(~HT_SM_POWER_SAVE_CONTROL_SM_MODE); /* Static
	                                              * SM
	                                              * power
	                                              * save
	                                              * mode
	...
	TX_SET_MMPDU(prAdapter, prMsduInfo, prBssInfo->ucBssIndex,
		     prStaRec->ucIndex, WLAN_MAC_MGMT_HEADER_LEN,
		     sizeof(ACTION_SM_POWER_SAVE_FRAME), NULL,
		     MSDU_RATE_MODE_AUTO);
 ...
 *
 * \param[in] ucChannelWidth 0:20MHz, 1:Any channel width in the STA¡¦s
				    ...
				    P_STA_RECORD_T prStaRec, u8 ucChannelWidth)
{
	...
	/* Calculate MSDU buffer length */
	u2EstimatedFrameLen = MAC_TX_RESERVED_FIELD +
			      sizeof(ACTION_NOTIFY_CHANNEL_WIDTH_FRAME);
	...
	TX_SET_MMPDU(prAdapter, prMsduInfo, prBssInfo->ucBssIndex,
		     prStaRec->ucIndex, WLAN_MAC_MGMT_HEADER_LEN,
		     sizeof(ACTION_NOTIFY_CHANNEL_WIDTH_FRAME), NULL,
		     MSDU_RATE_MODE_AUTO);
			  ...
			  u8 ucChannelWidth, u8 ucNss)
{
	...
	u8 fgIsChangeVhtBw = true, fgIsChangeHtBw = true;
	...
	if (ucChannelWidth == prBssInfo->ucOpChangeChannelWidth) {
		...
		case MAX_BW_40MHZ:
			if (!(prBssInfo->ucHtPeerOpInfo1 &
			      HT_OP_INFO1_STA_CHNL_WIDTH) ||
			    (!prBssInfo->fg40mBwAllowed)) {
				DBGLOG(RLM,
				       INFO,
				       "Can't change to BW40: PeerOpBw[%d] fg40mBwAllowed[%d]\n",
				       (prBssInfo->ucHtPeerOpInfo1 &
					HT_OP_INFO1_STA_CHNL_WIDTH),
				       prBssInfo->fg40mBwAllowed);
				fgIsChangeVhtBw = false;
			}
		...
		default:
			DBGLOG(RLM, WARN,
			       "BW[%d] is invalid for OpMode change\n",
			       ucChannelWidth);
			fgIsChangeVhtBw = false;
		}
	}
	...
	if (fgIsChangeHtBw) {
		...
		if (ucChannelWidth == MAX_BW_20MHZ) {
			...
			prBssInfo->ucHtOpInfo1 |= HT_OP_INFO1_STA_CHNL_WIDTH;
			if ((prBssInfo->ucHtPeerOpInfo1 & HT_OP_INFO1_SCO) !=
			    CHNL_EXT_RES) {
				prBssInfo->eBssSCO =
					(ENUM_CHNL_EXT_T)(prBssInfo->
							  ucHtPeerOpInfo1 &
							  HT_OP_INFO1_SCO);
			}
		} else {
	...
	if (fgIsChangeVhtBw) {
		...
		rlmChangeVhtOpBwPara(prAdapter, ucBssIndex,
				     prBssInfo->ucOpChangeChannelWidth);

		DBGLOG(RLM, INFO,
		       "Update VHT Channel Width Info to w=%d s1=%d s2=%d\n",
		       prBssInfo->ucVhtChannelWidth,
		       prBssInfo->ucVhtChannelFrequencyS1,
		       prBssInfo->ucVhtChannelFrequencyS2);
	}
	...
	if (fgIsChangeHtBw) {
		...
		DBGLOG(RLM, INFO,
		       "Update HT Channel Width Info to bw=%d s=%d\n",
		       (prBssInfo->ucHtOpInfo1 & HT_OP_INFO1_STA_CHNL_WIDTH) >>
		       2,
		       prBssInfo->eBssSCO);
	}
	...
	if ((prBssInfo->ucNss != ucNss) || fgIsChangeVhtBw || fgIsChangeHtBw) {
		...
		if (prBssInfo->eCurrentOPMode ==
			...
			if (prBssInfo->prStaRecOfAP) {
				prStaRec = prBssInfo->prStaRecOfAP;
#if CFG_SUPPORT_802_11AC
				/* 2. Check if we can change OpMode and Send
				 * OPmode notification frame */
				if (prStaRec->ucDesiredPhyTypeSet &
				    PHY_TYPE_BIT_VHT) { /*Send VHT notification
					                 * frame*/
					/* <1> Notify VHT Nss and Channel Width
					 * change*/
					...
					DBGLOG(RLM,
					       INFO,
					       "Send VHT OPmode notification frame, BW=%d, Nss=%d\n",
					       ucChannelWidth,
					       prBssInfo->ucNss);
				} else
#endif
				if (prStaRec->ucDesiredPhyTypeSet &
				    PHY_TYPE_BIT_HT) {         /*Send HT
					                       * notification
					                       * frame*/
					...
					DBGLOG(RLM,
					       INFO,
					       "Send HT SM Power Save frame, Nss=%d\n",
					       prBssInfo->ucNss);
				}
				...
				if (fgIsChangeHtBw) {
					...
					DBGLOG(RLM,
					       INFO,
					       "Send HT Notify Channel Width frame, BW=%d\n",
					       ucChannelWidth);
				}
			} else {
				DBGLOG(RLM, WARN,
				       "Can't change OpMode at legacy mode\n");
			}
		} else if (prBssInfo->eCurrentOPMode == OP_MODE_ACCESS_POINT) {
			P_LINK_T prClientList;

			/* 4. Update BCN/Probe Resp IE to notify peers the OP is
			 * be changed */
			DBGLOG(RLM, INFO,
			       "Beacon content update with Bssidex(%d)\n",
			       prBssInfo->ucBssIndex);
				   ...
				   P_MSDU_INFO_T prMsduInfo)
{
	...
	prRrmEnabledCap =
		(P_IE_RRM_ENABLED_CAP_T)(((u8 *)prMsduInfo->prPacket) +
					 prMsduInfo->u2FrameLength);
				      ...
				      u16 u2PacketLen)
{
	...
	aisCollectNeighborAP(prAdapter, &prNeighborResponse->aucInfoElem[0],
			     u2PacketLen -
			     OFFSET_OF(ACTION_NEIGHBOR_REPORT_FRAME_T,
				       aucInfoElem),
			     0);
}
#endif
...
void rrmQuietIeNotExist(P_ADAPTER_T prAdapter, P_BSS_INFO_T prBssInfo)
{
	...
	if (prBssInfo->fgRequestQuietInterval) {
		...
		if (u4QuietDuration > 0) {
			if (!prBssInfo->fgHasStopTx) {
				prBssInfo->fgHasStopTx = true;
				qmSetStaRecTxAllowed(prAdapter, prStaRec,
						     false);
			}
			cnmTimerStartTimer(prAdapter, &prBssInfo->rTxQuietTimer,
					   u4QuietDuration);
			   ...
			   P_IE_QUIET_T pucQUIE)
{
	...
	DBGLOG(RLM, EVENT, "[QIE] Cnt[%d] Duration[%d] Offset[%d] Period[%d]\n",
	       pucQUIE->ucCount, pucQUIE->u2Duration, pucQUIE->u2Offset,
	       pucQUIE->ucPeriod);

	prBssInfo->fgRequestQuietInterval = true;
	u4NextQuietTime = (u32)(pucQUIE->ucCount * prBssInfo->u2BeaconInterval +
				pucQUIE->u2Offset);
...
void rrmTxQuietTimeout(P_ADAPTER_T prAdapter, unsigned long ulParamPtr)
{
	...
	if (prBssInfo->fgIsInQuietInterval) {
		...
		if (prBssInfo->ucQuietPeriod) {
			prBssInfo->fgRequestQuietInterval = true;
			u4QuietPeriod =
				(u32)(prBssInfo->ucQuietPeriod *
				      prBssInfo->u2BeaconInterval +
				      prBssInfo->u2QuietOffset);
...
void rlmBfStaRecPfmuUpdate(P_ADAPTER_T prAdapter, P_STA_RECORD_T prStaRec)
{
	...
	u32 u4SetBufferLen = sizeof(CMD_STAREC_BF);
	...
	prStaRecBF = (P_CMD_STAREC_BF)cnmMemAlloc(prAdapter, RAM_TYPE_MSG,
						  u4SetBufferLen);
	...
	case MODE_VHT:
		...
		if (prStaRec->rTxBfPfmuStaInfo.fgETxBfCap) {
			...
			prStaRec->rTxBfPfmuStaInfo.ucNr =
				(ucBFerMaxNr < ucBFeeMaxNr) ? ucBFerMaxNr :
				ucBFeeMaxNr;
			prStaRec->rTxBfPfmuStaInfo.ucNc =
				((prStaRec->u2VhtRxMcsMap &
				  VHT_CAP_INFO_MCS_2SS_MASK) != BITS(2, 3)) ?
				1 :
				0;
		}
		break;
	...
	kalMemCopy(&prStaRecBF->rTxBfPfmuInfo, &prStaRec->rTxBfPfmuStaInfo,
	...
	rWlanStatus = wlanSendSetQueryExtCmd(
		...
		EXT_CMD_ID_STAREC_UPDATE, true, false, false,
...
void rlmETxBfTriggerPeriodicSounding(P_ADAPTER_T prAdapter)
{
	...
	rWlanStatus = wlanSendSetQueryExtCmd(
		...
		(u8 *)&rCmdTxBfActionInfo, &rTxBfActionInfo, u4SetBufferLen);
