...
static VOID cnmStaRecHandleEventPkt(
		P_ADAPTER_T prAdapter, P_CMD_INFO_T prCmdInfo, PUINT_8 pucEventBuf, IN UINT_32 u4EventBufLen);

static VOID cnmStaSendRemoveCmd(
		P_ADAPTER_T prAdapter, ENUM_STA_REC_CMD_ACTION_T eActionType, UINT_8 ucStaRecIndex, UINT_8 ucBssIndex);
		...
		P_ADAPTER_T prAdapter, P_STA_RECORD_T prStaRec, P_TXBF_PFMU_STA_INFO prTxBfPfmuStaInfo, BOOLEAN fgNeedResp)
{
	...
	DBGLOG(REQ, INFO, "Update StaRec[%u] WIDX[%u] State[%u] Type[%u] BssIdx[%u] AID[%u]\n", prCmdContent->ucStaIndex,
			prCmdContent->ucWlanIndex, prCmdContent->ucStaState, prCmdContent->ucStaType, prCmdContent->ucBssIndex,
	...
	for (i = 0; i < CFG_RX_MAX_BA_TID_NUM; i++) {
		if (prStaRec->aprRxReorderParamRefTbl[i]) {
			...
			DBGLOG(SW4, INFO, " Valid[%u] WinStart/End[%u/%u] WinSize[%u] ReOrderQueLen[%u]\n",
					prStaRec->aprRxReorderParamRefTbl[i]->fgIsValid, prStaRec->aprRxReorderParamRefTbl[i]->u2WinStart,
					prStaRec->aprRxReorderParamRefTbl[i]->u2WinEnd, prStaRec->aprRxReorderParamRefTbl[i]->u2WinSize,
...
cnmPeerAdd(P_ADAPTER_T prAdapter, PVOID pvSetBuffer, UINT_32 u4SetBufferLen, PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(CMD_PEER_ADD_T);
	...
	if (prStaRec == NULL) {
		prStaRec = cnmStaRecAlloc(
				prAdapter, STA_TYPE_DLS_PEER, (UINT_8)prAdapter->prAisBssInfo->ucBssIndex, prCmd->aucPeerMac);
...
cnmPeerUpdate(P_ADAPTER_T prAdapter, PVOID pvSetBuffer, UINT_32 u4SetBufferLen, PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(CMD_PEER_ADD_T);
	...
	if (prCmd->fgIsSupVht) {
		...
		prStaRec->u2VhtRxHighestSupportedDataRate = prCmd->rVHtCap.rVMCS.u2RxHighest;

		prStaRec->u2VhtTxMcsMap					  = prCmd->rVHtCap.rVMCS.u2TxMcsMap;
		prStaRec->u2VhtTxHighestSupportedDataRate = prCmd->rVHtCap.rVMCS.u2TxHighest;

		prStaRec->ucVhtOpMode = VHT_OP_MODE_CHANNEL_WIDTH_20 | VHT_OP_MODE_CHANNEL_WIDTH_80;
		/* no op mode IE, use HT/VHT cap to check BW */
		if (prCmd->fgIsSupHt && prAisBssInfo->fg40mBwAllowed && (prCmd->rHtCap.u2CapInfo & HT_CAP_INFO_SUP_CHNL_WIDTH))
			prStaRec->ucVhtOpMode |= VHT_OP_MODE_CHANNEL_WIDTH_40;
		if ((prCmd->rVHtCap.u4CapInfo & VHT_CAP_INFO_MAX_SUP_CHANNEL_WIDTH_SET_160) ||
		...
		if (((prCmd->rVHtCap.rVMCS.u2RxMcsMap & VHT_CAP_INFO_MCS_2SS_MASK) >> VHT_CAP_INFO_MCS_2SS_OFFSET) !=
