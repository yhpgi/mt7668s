...
#define MK16_TKIP(x, y)	   (((u16)(x) << 8) | (u16)(y))
		...
		IN u8 *pucSa, IN u8 *pucDa, IN u8 ucPriority, OUT u8 *pucMic)
{
	...
	au4Msg[1] = ((u32)pucDa[4]) | ((u32)pucDa[5] << 8) |
		    ((u32)pucSa[0] << 16) | ((u32)pucSa[1] << 24);
	...
	au4Msg[0] = (u32)ucPriority;

	l = l ^ au4Msg[0];
	tkipMicB(&l, &r);

	/* Michael message processing for MSDU data playload except the last
	 * octets which cannot be partitioned into a 32-bit word.
	 */
	for (i = 0; i < (u32)u4DataLen / 4; i++) {
		WLAN_GET_FIELD_32(pucData + i * 4, &au4Msg[0]);
		l = l ^ au4Msg[0];
		tkipMicB(&l, &r);
	}

	/* Michael message processing for the last uncomplete octets, if
	 * present, and the padding.
	 */
	switch (u4DataLen & 3) {
	case 1:
		au4Msg[0] = ((u32)pucData[u4DataLen - 1]) | 0x00005A00;
		break;

	case 2:
		au4Msg[0] = ((u32)pucData[u4DataLen - 2]) |
			    ((u32)pucData[u4DataLen - 1] << 8) | 0x005A0000;
		break;

	case 3:
		au4Msg[0] = ((u32)pucData[u4DataLen - 3]) |
			    ((u32)pucData[u4DataLen - 2] << 8) |
			    ((u32)pucData[u4DataLen - 1] << 16) | 0x5A000000;
		break;
			...
			IN u8 *pucMicKey)
{
	...
	tkipMicGen(pucMicKey, (u8 *)pucPayload, u2PayloadLen, pucSa, pucDa,
		   ucPriority, aucMic);

	kalMemCopy((u8 *)pucMic, &aucMic[0], WLAN_MAC_MIC_LEN);

	DBGLOG(RSN, LOUD, "Mic %02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x\n",
	       pucMic[0], pucMic[1], pucMic[2], pucMic[3], pucMic[4], pucMic[5],
	       pucMic[6], pucMic[7]);
}
 ...
 * \param[in]  pucMickey Pointer to MIC key
 * \param[out] pu4ResultFrameBodyLen Pointer to put the result frame body
 * length.
 *
...
u8 tkipMicDecapsulate(IN P_SW_RFB_T prSwRfb, IN u8 *pucMicKey)
{
	...
	/* if ((prRxStatus->ucKIdxSecMode & BITS(0,3)) !=
	...
	if (RXM_IS_QOS_DATA_FRAME(prSwRfb->u2FrameCtrl)) {
		ucPriority = (u8)((((P_WLAN_MAC_HEADER_QOS_T)prSwRfb->pvHeader)
				   ->u2QosCtrl) &
				  MASK_QC_TID);
	} else {
		ucPriority = 0;
...
u8 tkipMicDecapsulateInRxHdrTransMode(IN P_SW_RFB_T prSwRfb, IN u8 *pucMicKey)
{
	...
	if (prSkb) {
		...
		*(prSkb->data + 10) = 0x00;
		*(prSkb->data + 11) = 0x00;
		*(prSkb->data + 12) = *(u8 *)(prSwRfb->pvHeader + 12);
		*(prSkb->data + 13) = *(u8 *)(prSwRfb->pvHeader + 13);

		tkipMicGen(pucMicKey, prSkb->data + 6,
			   u2FrameBodyLen - WLAN_MAC_MIC_LEN + 8,
			   prSwRfb->pvHeader + 6, prSwRfb->pvHeader,
