...
VOID rlmObssInit(P_ADAPTER_T prAdapter)
{
	...
	for (i = 0; i < BSS_INFO_NUM; i++) {
		...
		cnmTimerInitTimer(
				prAdapter, &prBssInfo->rObssScanTimer, (PFN_MGMT_TIMEOUT_FUNC)rlmObssScanTimeout, (ULONG)prBssInfo);
	}
}
...
VOID rlmObssScanDone(P_ADAPTER_T prAdapter, P_MSG_HDR_T prMsgHdr)
{
	...
	if (prBssInfo->auc2G_20mReqChnlList[0] > 0 || prBssInfo->auc2G_NonHtChnlList[0] > 0) {
		...
		if (prMsduInfo) {
			prTxFrame = (P_ACTION_20_40_COEXIST_FRAME)((ULONG)(prMsduInfo->prPacket) + MAC_TX_RESERVED_FIELD);
