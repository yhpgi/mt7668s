...
void rlmObssScanDone(P_ADAPTER_T prAdapter, P_MSG_HDR_T prMsgHdr)
{
	...
	if (prBssInfo->auc2G_20mReqChnlList[0] > 0 ||
		...
		if (prMsduInfo) {
			prTxFrame =
				(P_ACTION_20_40_COEXIST_FRAME)((unsigned long)(
								       prMsduInfo
								       ->
								       prPacket)
							       +
			...
			if (prBssInfo->auc2G_NonHtChnlList[0] > 0) {
				...
				prTxFrame->rChnlReport.ucRegulatoryClass =
					81; /* 2.4GHz, ch1~13 */
				for (i = 0;
				     i < prBssInfo->auc2G_NonHtChnlList[0] &&
				     i < CHNL_LIST_SZ_2G;
				     i++)
					prTxFrame->rChnlReport
					.aucChannelList[i] =
						prBssInfo->auc2G_NonHtChnlList
						[i + 1];

				u2PayloadLen +=
					IE_SIZE(&prTxFrame->rChnlReport);
			}
...
void rlmObssTriggerScan(P_ADAPTER_T prAdapter, P_BSS_INFO_T prBssInfo)
{
	...
	prScanReqMsg = (P_MSG_SCN_SCAN_REQ)cnmMemAlloc(
		prAdapter, RAM_TYPE_MSG, sizeof(MSG_SCN_SCAN_REQ));
	ASSERT(prScanReqMsg);
