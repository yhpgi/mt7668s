...
static BOOLEAN roamingFsmIsNeedScan(IN P_ADAPTER_T prAdapter)
{
	...
	{
		if (EQUAL_SSID(
					prRoamBssDesc->aucSSID, prRoamBssDesc->ucSSIDLen, prAisBssInfo->aucSSID, prAisBssInfo->ucSSIDLen)) {
		...
		if (prBssDesc) {
			...
			if (prAisBssInfo->ucRoamSkipTimes == 0) {
				...
				wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_ROAMING_SKIP, TRUE, FALSE, FALSE, NULL, NULL,
						sizeof(CMD_ROAMING_SKIP_ONE_AP_T), (PUINT_8)&rCmdRoamingSkipOneAP, NULL, 0);
		} else
