			       ...
			       IN P_MSG_HDR_T prMsgHdr)
{
	...
	do {
		 ...
		 * p2pRoleFsmRunEventScanRequest free prMsgHdr before return, so
		 * prMsgHdr is needed to be NULL.
		 */
	} while (false);
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief    This function is call when channel is granted by CNM module from
 * FW.
 *
				    ...
				    IN P_MSG_HDR_T prMsgHdr)
{
	...
	do {
		...
		if (prMsgHdr != NULL) {
			prMsgWfdCfgSettings =
				(P_MSG_WFD_CONFIG_SETTINGS_CHANGED_T)prMsgHdr;
			prWfdCfgSettings =
		...
		DBGLOG(P2P, INFO,
		       ...
		       (u32)prWfdCfgSettings->u4WfdState,
		       (u32)prWfdCfgSettings->u4WfdFlag,
		       (u32)prWfdCfgSettings->u4WfdAdvancedFlag);
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief    This function is used to handle scan done event during Device
 * Discovery.
 *
 * \param[in] prAdapter  Pointer of ADAPTER_T
 *
