...
static u8 *apucDebugAAState[AA_STATE_NUM] = {
    (u8 *)DISP_STRING("AA_IDLE"),
    (u8 *)DISP_STRING("SAA_SEND_AUTH1"),
    (u8 *)DISP_STRING("SAA_WAIT_AUTH2"),
    (u8 *)DISP_STRING("SAA_SEND_AUTH3"),
    (u8 *)DISP_STRING("SAA_WAIT_AUTH4"),
    (u8 *)DISP_STRING("SAA_SEND_ASSOC1"),
    (u8 *)DISP_STRING("SAA_WAIT_ASSOC2"),
    (u8 *)DISP_STRING("AAA_SEND_AUTH2"),
    (u8 *)DISP_STRING("AAA_SEND_AUTH4"),
    (u8 *)DISP_STRING("AAA_SEND_ASSOC2"),
...
void saaFsmRunEventStart(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr){
    ...
    if (prStaRec->ucBssIndex < BSS_INFO_NUM) {
        ...
        if ((prAdapter->rWifiVar.ucAvailablePhyTypeSet &
            ...
            prBssInfo->fgAssoc40mBwAllowed = cnmBss40mBwPermitted(
...
void saaFsmRunEventRxAuth(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb){
    ...
    if ((authCheckRxAuthFrameStatus(
             prAdapter, prSwRfb, prStaRec->eAuthAssocSent,
        ...
        if (u2StatusCode == STATUS_CODE_SUCCESSFUL ||
             ...
             * layer*/
            if (saaFsmSendEventJoinComplete(
                    prAdapter, WLAN_STATUS_FAILURE, prStaRec,
...
WLAN_STATUS saaFsmRunEventRxAssoc(IN P_ADAPTER_T prAdapter,
    ...
    if ((assocCheckRxReAssocRspFrameStatus(prAdapter, prSwRfb,
                                           &u2StatusCode) ==
         WLAN_STATUS_SUCCESS) &&
        (!(IS_STA_IN_P2P(prStaRec)))) {
        cnmTimerStopTimer(prAdapter,
    ...
    do {
        if (eFrmType == FRM_DEAUTH) {
            if (prStaRec->ucStaState == STA_STATE_3) {
                ...
                if (saaFsmSendEventJoinComplete(
                        prAdapter, WLAN_STATUS_JOIN_ABORT,
                        prStaRec,
                        NULL) == WLAN_STATUS_RESOURCES) {
                    /* can set a timer and retry later */
                    DBGLOG(SAA,
...
WLAN_STATUS saaFsmRunEventRxDisassoc(IN P_ADAPTER_T prAdapter,
    ...
    #define CONNECTED_BSS(wdev) wdev->connected
            ...
            p2pRoleFsmRunEventRxDisassociation(prAdapter, prStaRec,
                                               prSwRfb);
        } else {
            ASSERT(0);
        }
