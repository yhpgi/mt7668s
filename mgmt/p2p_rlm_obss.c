...
static u8 rlmObssChnlLevelIn2G4(P_BSS_INFO_T prBssInfo,
				u8 ucPriChannel,
				ENUM_CHNL_EXT_T eExtend);

static u8 rlmObssChnlLevelIn5G(P_BSS_INFO_T prBssInfo,
			       u8 ucPriChannel,
			       ENUM_CHNL_EXT_T eExtend);
				...
				ENUM_CHNL_EXT_T eExtend)
{
	...
	for (i = 1;
	     i <= prBssInfo->auc2G_NonHtChnlList[0] && i <= CHNL_LIST_SZ_2G;
	     i++) {
	...
	for (i = 1;
	     i <= prBssInfo->auc2G_20mReqChnlList[0] && i <= CHNL_LIST_SZ_2G;
	     i++) {
	...
	for (i = 1;
	     i <= prBssInfo->auc2G_PriChnlList[0] && i <= CHNL_LIST_SZ_2G;
	     i++) {
	...
	for (i = 1;
	     i <= prBssInfo->auc2G_SecChnlList[0] && i <= CHNL_LIST_SZ_2G;
	     i++) {
			       ...
			       ENUM_CHNL_EXT_T eExtend)
{
	...
	for (i = 1;
	     i <= prBssInfo->auc5G_PriChnlList[0] && i <= CHNL_LIST_SZ_5G;
	     i++) {
	...
	for (i = 1;
	     i <= prBssInfo->auc5G_NonHtChnlList[0] && i <= CHNL_LIST_SZ_5G;
	     i++) {
	...
	for (i = 1;
	     i <= prBssInfo->auc5G_SecChnlList[0] && i <= CHNL_LIST_SZ_5G;
	     i++) {
			     ...
			     P_SW_RFB_T prSwRfb)
{
	...
	prMsduInfo =
		(P_MSDU_INFO_T)cnmMgtPktAlloc(prAdapter, PUBLIC_ACTION_MAX_LEN);
	if (prMsduInfo == NULL)
		return;
	...
	TX_SET_MMPDU(prAdapter, prMsduInfo, prBssInfo->ucBssIndex,
		     prSwRfb->ucStaRecIdx, WLAN_MAC_MGMT_HEADER_LEN,
		     WLAN_MAC_MGMT_HEADER_HTC_LEN + 5, NULL,
		     MSDU_RATE_MODE_AUTO);
