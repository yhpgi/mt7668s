...
static WLAN_STATUS wnmBTMQueryTxDone(
...
static WLAN_STATUS wnmBTMResponseTxDone(
		IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	P_BSS_TRANSITION_MGT_PARAM_T prBtm		  = &prAdapter->rWifiVar.rAisSpecificBssInfo.rBTMParam;
...
VOID wnmSendBTMResponseFrame(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	P_BSS_TRANSITION_MGT_PARAM_T prBtmParam	  = &prAdapter->rWifiVar.rAisSpecificBssInfo.rBTMParam;
	...
	if (!prMsduInfo)
		return;
	prTxFrame = (P_ACTION_BTM_RSP_FRAME_T)((unsigned long)(prMsduInfo->prPacket) + MAC_TX_RESERVED_FIELD);
	...
	TX_SET_MMPDU(prAdapter, prMsduInfo, prStaRec->ucBssIndex, prStaRec->ucIndex, WLAN_MAC_MGMT_HEADER_LEN,
			OFFSET_OF(ACTION_BTM_RSP_FRAME_T, aucOptInfo) + u2PayloadLen, wnmBTMResponseTxDone, MSDU_RATE_MODE_AUTO);
...
VOID wnmSendBTMQueryFrame(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	if (!prMsduInfo)
		return;
	prTxFrame = (P_ACTION_BTM_QUERY_FRAME_T)((unsigned long)(prMsduInfo->prPacket) + MAC_TX_RESERVED_FIELD);
	...
	/* 4 Update information of MSDU_INFO_T */
	TX_SET_MMPDU(prAdapter, prMsduInfo, prStaRec->ucBssIndex, prStaRec->ucIndex, WLAN_MAC_MGMT_HEADER_LEN,
			WLAN_MAC_MGMT_HEADER_LEN + 4 + prBtmParam->u2OurNeighborBssLen, wnmBTMQueryTxDone, MSDU_RATE_MODE_AUTO);
...
VOID wnmRecvBTMRequest(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if (!prRxFrame)
		return;
	if (prSwRfb->u2PacketLen < OFFSET_OF(ACTION_BTM_REQ_FRAME_T, aucOptInfo)) {
		DBGLOG(WNM, WARN, "BTM: Request frame length is less than a standard BTM frame\n");
		return;
	}
	prMsg = (P_MSG_AIS_BSS_TRANSITION_T)cnmMemAlloc(prAdapter, RAM_TYPE_MSG, sizeof(MSG_AIS_BSS_TRANSITION_T));
	if (!prMsg) {
	...
	if (ucRequestMode & BTM_REQ_MODE_BSS_TERM_INCLUDE) {
		P_SUB_IE_BSS_TERM_DURATION_T prBssTermDuration = (P_SUB_IE_BSS_TERM_DURATION_T)pucOptInfo;
	...
	if (ucRequestMode & BTM_REQ_MODE_CAND_INCLUDED_BIT) {
		...
		if (prSwRfb->u2PacketLen > u2TmpLen) {
			...
			DBGLOG(WNM, WARN, "BTM: Candidate Include bit is set, but no candidate list\n");
	}

	DBGLOG(WNM, INFO, "BTM: Req %d, VInt %d, DiscTimer %d, Token %d, TransType %d\n", prBtmParam->ucRequestMode,
			prRxFrame->ucValidityInterval, prBtmParam->u2DisassocTimer, prMsg->ucToken, eTransType);
